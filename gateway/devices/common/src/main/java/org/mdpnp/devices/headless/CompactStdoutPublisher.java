...
public final class CompactStdoutPublisher implements JsonPublisher {
    ...
    public synchronized void publish(Map<String, Object> event) throws IOException {
        ...
        String topic = str(event.get("topic"));
        if (!"vital".equals(eventType) && !"numeric".equals(eventType) && !"Numeric".equals(topic)) return;

        // Track device
        String did = firstNonEmpty(str(event.get("deviceId")), str(event.get("unique_device_identifier")));
        if (did != null && !did.isEmpty()) deviceId = did;
        String v = str(event.get("vendor"));
        if (v != null && !v.isEmpty()) vendor = v;

        // Store value by metric
        String metric = firstNonEmpty(str(event.get("metric")), str(event.get("metric_id")), str(event.get("vendor_metric_id")));

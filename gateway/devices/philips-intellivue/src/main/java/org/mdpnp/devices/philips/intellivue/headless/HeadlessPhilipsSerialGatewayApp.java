...
public final class HeadlessPhilipsSerialGatewayApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final MultiJsonPublisher sinks = buildSinks(a.stdout, a.jsonl, a.httpUrl, a.httpTimeoutMs, a.httpHeaders, a.allowInsecureHttp, a.outputFormat);
        ...
        System.err.println("Serial settings: " + a.serialBaud + " baud, 8 data bits, no parity, 1 stop bit, no flow control");
        while (running.get()) {
            ...
            try {
                ...
                serialBridge = new RS232Adapter(a.serial, serialSide, parserSide, threadGroup, networkLoop, a.serialBaud);
    ...
    private static MultiJsonPublisher buildSinks(Boolean stdout, String jsonl, String httpUrl, int httpTimeoutMs, Map<String, String> headers, boolean allowInsecureHttp, String outputFormat) throws Exception {
        ...
        if (enableStdout) { sinks.add(format(new StdoutJsonPublisher(), outputFormat)); }
    ...
    static final class Args {
        ...
        int serialBaud = RS232Adapter.DEFAULT_BAUD;
        Boolean stdout;
        ...
        static Args parse(String[] args) {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                else if ("--serial-baud".equals(k) || "--philips-baud".equals(k)) { a.serialBaud = Integer.parseInt(required(k, v)); i++; }
            ...
            if (a.serialBaud != 115200 && a.serialBaud != 19200) { throw new IllegalArgumentException("Philips fixed baudrate must be 115200 or 19200"); }
        ...
        static void usageAndExit() {
            System.out.println("Usage: HeadlessPhilipsSerialGatewayApp --serial <device> [--serial-baud 115200|19200] [--device-id id] [--jsonl file] [--output-format canonical] [--dead-letter-jsonl file] [--http-url url] [--allow-insecure-http true|false] [--http-header 'Authorization: Bearer TOKEN'] [--stdout true|false]");
            System.out.println("Example: ./gradlew :devices:philips-intellivue:runHeadlessPhilipsSerial --args=\"--serial /dev/philips_monitor_01 --gateway-id jetson_nicu_01 --bed-id bed_12 --device-id philips_monitor_01 --jsonl /tmp/philips-events.jsonl --dead-letter-jsonl /tmp/philips-dead-letter.jsonl\"");
            System.out.println("Serial settings: fixed baudrate protocol, 8N1, no flow control. Supported baud rates: 115200 and 19200.");
            System.exit(0);
        }
    }
}

...
public final class HeadlessDraegerGatewayApp {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final MultiJsonPublisher sinks = buildSinks(a.stdout, a.jsonl, a.httpUrl, a.httpTimeoutMs, a.httpHeaders, a.allowInsecureHttp, a.webPort, a.stdoutMode, a.outputFormat);
        ...
        while (running.get()) {
            ...
            try {
                t = openTransport(a);
                final HeadlessDraegerMedibus medibus = new HeadlessDraegerMedibus(t.in, t.out, a.gatewayId, a.bedId, a.deviceId, a.profile, queued);
                poller = startPoller(medibus, a.pollMs, a.lowFrequencyPollMs, a.profile);
                System.err.println("Draeger headless gateway connected: deviceId=" + a.deviceId + " profile=" + a.profile.id());
    ...
    static final class Args {
        ...
        static Args parse(String[] args) {
            ...
            for (int i=0; i<args.length; i++) {
                ...
                else if ("--low-frequency-poll-ms".equals(k)) { a.lowFrequencyPollMs = Long.parseLong(required(k,v)); i++; }
                ...
                else if ("--profile".equals(k) || "--draeger-profile".equals(k)) { a.profile = DraegerMedibusProfile.parse(required(k,v)); i++; }
            ...
            if (a.pollMs <= 0 || a.lowFrequencyPollMs <= 0) { throw new IllegalArgumentException("poll intervals must be > 0"); }
        ...
        static void usageAndExit() {
            System.out.println("Usage: HeadlessDraegerGatewayApp (--serial /dev/draeger_vent_01 [--serial-baud 19200] | --tcp-host <ip> --tcp-port <port>) [--profile v500|intensive-care|evita|savina|fabius] [--device-id id] [--jsonl file] [--output-format canonical] [--dead-letter-jsonl file] [--http-url url] [--allow-insecure-http true|false] [--http-header 'Authorization: Bearer TOKEN'] [--stdout true|false]");
            System.exit(0);
        }
    }
}

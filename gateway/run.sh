...
#   devices/draeger-medibus/build/install/draeger/bin/draeger --help
#   devices/philips-intellivue/build/install/philips/bin/philips-serial --help
#   runtime/build/install/gateway-runtime/bin/gateway-runtime --help
...
# --- Check build ---
if [ ! -d devices/draeger-medibus/build/install ] && [ ! -d devices/philips-intellivue/build/install ] && [ ! -d runtime/build/install ]; then
...
Modes (pick one):
  ...
  --web-only                           Dashboard/replay mode with no live driver
...
Device options (for --multi mode):
  ...
  --draeger-tcp <host:port>            Draeger TCP endpoint
...
Common options:
  ...
  --output-format canonical            Optional compatibility flag; canonical is always used
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --stdout)
            if [ -n "${2:-}" ] && [[ "${2:-}" != --* ]]; then STDOUT_FLAG="$2"; shift 2;
...
if [ -n "$OUTPUT_FORMAT" ] && [ "$OUTPUT_FORMAT" != "canonical" ]; then
    echo "ERROR: --output-format supports only canonical."
    exit 1
fi
...
if [ -n "$OUTPUT_FORMAT" ]; then OUTPUT_ARGS+=(--output-format "$OUTPUT_FORMAT"); fi
...
case "$MODE" in
    draeger)
        ...
        exec devices/draeger-medibus/build/install/draeger/bin/draeger \
    ...
    philips-serial)
        ...
        exec devices/philips-intellivue/build/install/philips/bin/philips-serial \
    ...
    philips-udp)
        ...
        exec devices/philips-intellivue/build/install/philips/bin/philips \
            --host "$PHILIPS_HOST" \
            --gateway-id "$GATEWAY_ID" --bed-id "$BED_ID" \
            --device-id "$PHILIPS_DEVICE_ID" \
            "${OUTPUT_ARGS[@]}" "${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}"
    ...
    multi)
        ...
        if [ -n "$DRAEGER_TCP_HOST" ]; then
            MULTI_ARGS+=(--draeger-tcp-host "$DRAEGER_TCP_HOST" --draeger-tcp-port "$DRAEGER_TCP_PORT" --draeger-device-id "$DRAEGER_DEVICE_ID")
            echo "Draeger TCP: $DRAEGER_TCP_HOST:$DRAEGER_TCP_PORT"
        fi
        exec runtime/build/install/gateway-runtime/bin/gateway-runtime \
            "${MULTI_ARGS[@]}" "${OUTPUT_ARGS[@]}" "${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}"
        ;;
    web-only)
        echo "Web-only replay/dashboard mode"
        exec runtime/build/install/gateway-runtime/bin/gateway-runtime \
            --gateway-id "$GATEWAY_ID" --bed-id "$BED_ID" \
            "${OUTPUT_ARGS[@]}" "${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}"
        ;;

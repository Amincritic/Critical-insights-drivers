...
# --- Build if needed ---
if [ ! -d devices/multidevice/build/install ]; then
...
# Copy JARs (multidevice includes all device JARs)
cp devices/multidevice/build/install/multidevice/lib/* "$DEST/lib/"

# Copy launchers
cp devices/multidevice/build/install/multidevice/bin/multidevice "$DEST/bin/"
cp devices/draeger/build/install/draeger/bin/draeger "$DEST/bin/"
cp devices/philips/build/install/philips/bin/philips "$DEST/bin/"
cp devices/philips/build/install/philips/bin/philips-serial "$DEST/bin/"
chmod +x "$DEST/bin/"*
...
# Deployment files
cp openice-multidevice.service "$DEST/"
...
if [ ! -f bin/draeger ] && [ ! -f bin/multidevice ]; then
...
case "$MODE" in
    ...
    multi)
        ...
        exec bin/multidevice "${MULTI_ARGS[@]}" "${OUTPUT_ARGS[@]}" "${EXTRA_ARGS[@]+"${EXTRA_ARGS[@]}"}" ;;
...
echo ""
echo "Contents:"
echo "  bin/           Launcher scripts (draeger, philips, philips-serial, multidevice)"

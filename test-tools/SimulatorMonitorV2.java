...
/**
 * Patient-monitor-style GUI V2 for the Draeger MEDIBUS V2 and Philips IntelliVue V2 simulators.
 *
 * Improvements over V1:
 * - Two top-level tabs separating Draeger and Philips into full-screen layouts
 * - Much larger fonts throughout
 * - Separated waveform/numeric/control panels per device
 * - Dark background like a real bedside monitor
 *
 * No external dependencies -- uses Java Swing only.
 *
 * Usage:
 *   javac test-tools/SimulatorMonitorV2.java
 *   java -cp test-tools SimulatorMonitorV2
 */
public class SimulatorMonitorV2 extends JFrame {
    ...
    // =====================================================================
    // Waveform displays (separate per device)
    // =====================================================================
    // Draeger: 3 traces (Paw, Flow, Vol)
    ...
    static final Color[] DR_TRACE_COLORS = {
        new Color(255, 255, 0),   // Paw - yellow
    ...
    // Philips: 4 traces (ECG, Pleth, ABP, Resp)
    ...
    static final Color[] PH_TRACE_COLORS = {
        ...
        new Color(255, 255, 0)    // Respiration - yellow
    };
    ...
    public SimulatorMonitorV2() {
        ...
        drWavePanel = new WaveformPanel(drWaveBuffers, DR_TRACE_COLORS, DR_TRACE_LABELS, drWaveLock, 3);
        phWavePanel = new WaveformPanel(phWaveBuffers, PH_TRACE_COLORS, PH_TRACE_LABELS, phWaveLock, 4);
    ...
    // =====================================================================
    // Waveform Panel -- custom drawing with sweep line (reusable)
    // =====================================================================
    ...
    private class WaveformPanel extends JPanel {
        ...
        WaveformPanel(float[][] buffers, Color[] colors, String[] labels, Object lock, int numTraces) {
    ...
    private JPanel buildDraegerNumericsPanel() {
        ...
        panel.add(buildNumericBox("VT",     lblDrVt,    "mL",       cyan,    3));
        panel.add(buildNumericBox("RR",     lblDrRr,    "/min",     lime,    3));
        panel.add(buildNumericBox("Paw",    lblDrPaw,   "mbar",     yellow,  3));
        panel.add(buildNumericBox("PEEP",   lblDrPeep,  "mbar",     orange,  3));
        panel.add(buildNumericBox("FiO2",   lblDrFio2,  "%",        lav,     3));
    ...
    private JPanel buildDraegerControlsArea() {
        ...
        // Bottom row: mode, waveforms, alarms
    ...
    private JPanel buildPhilipsNumericsPanel() {
        ...
        panel.add(buildNumericBox("HR",    lblPhHr,    "bpm",    green,  2));
        panel.add(buildNumericBox("SpO2",  lblPhSpo2,  "%",      cyan,   2));
        panel.add(buildNumericBox("PR",    lblPhPr,    "bpm",    cyan,   2));
        panel.add(buildNumericBox("RR",    lblPhRr,    "/min",   yellow, 2));
        panel.add(buildCompoundNumericBox("ABP", lblPhAbp, lblPhAbpM, "mmHg", red, 2));
        panel.add(buildCompoundNumericBox("NBP", lblPhNbp, lblPhNbpM, "mmHg", white, 2));
        panel.add(buildNumericBox("etCO2", lblPhEtco2, "mmHg",   orange, 2));
        panel.add(buildNumericBox("Temp",  lblPhTemp,  "\u00B0C", purple, 2));
    ...
    private JPanel buildNumericBox(String label, JLabel valueLabel, String unit, Color color, int borderWidth) {
        ...
        box.setMaximumSize(new Dimension(300, 58));
        box.setPreferredSize(new Dimension(260, 52));
    ...
    private JPanel buildCompoundNumericBox(String label, JLabel valLabel, JLabel meanLabel, String unit, Color color, int borderWidth) {
        ...
        box.setMaximumSize(new Dimension(300, 66));
        box.setPreferredSize(new Dimension(260, 60));
    ...
    private void generateDrWaveformSamples() {
        ...
        synchronized (drWaveLock) {
            for (int s = 0; s < samplesPerTick; s++) {
                ...
                // Airway Pressure
                drWaveBuffers[0][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = generatePawSample(breathFrac, pp, peep);
                // Flow
                drWaveBuffers[1][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = generateFlowSample(breathFrac);
                // Volume
                drWaveBuffers[2][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = generateVolumeSample(breathFrac, tv);
    ...
    private void generatePhWaveformSamples() {
        ...
        synchronized (phWaveLock) {
            for (int s = 0; s < samplesPerTick; s++) {
                ...
                phWaveBuffers[0][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = generateEcgSample(ecgBeatFrac);

                double plethBeatFrac = (phPlethPhaseLocal % beatInterval) / beatInterval;
                phWaveBuffers[1][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = generatePlethSample(plethBeatFrac);

                double abpBeatFrac = (phAbpPhaseLocal % beatInterval) / beatInterval;
                phWaveBuffers[2][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = generateAbpSample(abpBeatFrac, abpSys, abpDia);
                ...
                phWaveBuffers[3][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = respVal;
    ...
    private void updatePhNumerics() {
        ...
        lblPhPr.setText(String.valueOf(hr + 1));
    ...
    private void startDraeger() {
        ...
        draegerThread = new Thread(() -> runDraegerServer(port), "draeger-sim-v2");
    ...
    private void startPhilips() {
        ...
        philipsThread = new Thread(() -> runPhilipsServer(port), "philips-sim-v2");
    ...
    private void runPhilipsServer(int port) {
        try {
            ...
            broadcastExec = Executors.newSingleThreadScheduledExecutor(r -> {
                Thread t = new Thread(r, "ph-broadcast-v2");
                t.setDaemon(true);
                return t;
            });
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            SimulatorMonitorV2 monitor = new SimulatorMonitorV2();

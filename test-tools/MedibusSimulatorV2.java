 ...
 *
 * Usage:
 *   javac test-tools/MedibusSimulatorV2.java
 *
 *   # TCP mode
 ...
 *
 *   # Custom device model
 *   java -cp test-tools MedibusSimulatorV2 --tcp 9100 --model evita4
 *
...
public class MedibusSimulatorV2 {
    ...
    private void processCommand(byte[] buf, int len, OutputStream out) throws IOException {
        ...
        if (realtimeEnabled && realtimeOut != null) {
     ...
     * Returns the currently configured realtime transmission codes.
     * Per spec: response contains list of configured waveform code bytes.
     */
    ...
    private void sendRealtimeWaveformBurst(OutputStream out) throws IOException {
        if (!realtimeEnabled || realtimeConfiguredCodes.isEmpty()) return;
        ...
        for (int s = 0; s < samplesPerBurst; s++) {
            ...
            for (int code : realtimeConfiguredCodes) {
                ...
                // Realtime data: 2 bytes with high bit set (fast bytes)
                // High byte: 0x80 | (code & 0x1F) << 2 | (value >> 8 & 0x03)
                // Low byte:  0x80 | (value & 0x7F)
     ...
     * Returns available trend parameters as 2-byte ASCII HEX codes.
     */
    private void sendTrendDataStatus(OutputStream out) throws IOException {
        ByteArrayOutputStream data = new ByteArrayOutputStream();
        // List available trend parameters
     ...
     * Argument contains the trend parameter code (2 bytes ASCII HEX).
     * Response contains historical samples as 4-byte ASCII values.
     */
    private void sendTrendData(byte[] argument, OutputStream out) throws IOException {
        ...
        synchronized (trendSamples) {
            for (double[] sample : trendSamples) {
    ...
    private void addTextRecord(ByteArrayOutputStream data, int code, String text) throws IOException {
        data.write(asciiHexHi(code));
        data.write(asciiHexLo(code));
        int len = Math.min(text.length(), 127);
        data.write(len); // Length as raw byte
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                ...
                case "--help":
                    System.out.println("Usage: java MedibusSimulatorV2 [--tcp <port>] [--serial <path>] [--model <name>]");

import javax.swing.*;
import javax.swing.border.*;
import javax.swing.text.*;
import java.awt.*;
import java.awt.event.*;
import java.awt.image.BufferedImage;
import java.io.*;
import java.net.*;
import java.nio.*;
import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;
import java.util.*;
import java.util.concurrent.*;
import java.util.concurrent.atomic.*;
import javax.imageio.ImageIO;

/**
 * CriticalInsights Patient Monitor Simulator
 *
 * Production-ready patient monitor simulator GUI with Draeger MEDIBUS and Philips IntelliVue
 * protocol support. Features alarm bar, preset scenarios, ECG arrhythmia simulation,
 * artifact/noise injection, screenshots, vital sign trend sparklines, fullscreen mode,
 * NBP cycling, SpO2 signal quality, context menus, and keyboard shortcuts.
 *
 * No external dependencies -- uses Java Swing only.
 *
 * Usage:
 *   ../gateway/gradlew installDist
 *   ../scripts/run-simulator-gui.sh
 */
public class CriticalInsightsMonitor extends JFrame {

    // =====================================================================
    // Protocol Constants -- Draeger MEDIBUS
    // =====================================================================
    static final int ESC = 0x1B;
    static final int SOH = 0x01;
    static final int CR  = 0x0D;
    static final int DC1 = 0x11;
    static final int DC3 = 0x13;
    static final int NAK = 0x15;
    static final int ETX = 0x03;

    static final int RT_SYNC_BYTE = 0xD0;
    static final int RT_SYNC = 0xC6;
    static final int RT_SYNC_INSP_START = 0xC0;

    static final int CMD_NOP              = 0x30;
    static final int CMD_ICC              = 0x51;
    static final int CMD_STOP             = 0x55;
    static final int CMD_REQ_DEVICE_ID    = 0x52;
    static final int CMD_REQ_DATETIME     = 0x28;
    static final int CMD_REQ_DATA_CP1     = 0x24;
    static final int CMD_REQ_DATA_CP2     = 0x2B;
    static final int CMD_REQ_LOW_ALRM_CP1 = 0x25;
    static final int CMD_REQ_HI_ALRM_CP1  = 0x26;
    static final int CMD_REQ_ALARMS_CP3   = 0x23;
    static final int CMD_REQ_ALARMS_CP1   = 0x27;
    static final int CMD_REQ_ALARMS_CP2   = 0x2E;
    static final int CMD_REQ_SETTINGS     = 0x29;
    static final int CMD_REQ_TEXT_MSG     = 0x2A;
    static final int CMD_CONFIGURE_RESP   = 0x4A;
    static final int CMD_REQ_RT_CONFIG    = 0x53;
    static final int CMD_CONFIG_RT        = 0x54;
    static final int CMD_REQ_TREND_STATUS = 0x6C;
    static final int CMD_REQ_TREND_DATA   = 0x6D;

    static final int RT_CODE_AIRWAY_PRESSURE = 0x00;
    static final int RT_CODE_FLOW            = 0x01;
    static final int RT_CODE_VOLUME          = 0x03;
    static final int RT_CODE_EXP_VOLUME      = 0x24;

    // =====================================================================
    // Protocol Constants -- Philips IntelliVue
    // =====================================================================
    static final int BROADCAST_PORT    = 24005;
    static final int CN_SPDU_SI        = 0x0D;
    static final int AC_SPDU_SI        = 0x0E;
    static final int FN_SPDU_SI        = 0x09;
    static final int DN_SPDU_SI        = 0x0A;
    static final int DATA_EXPORT_SPDU  = 0xE1;

    static final int ROIV_APDU  = 0x0001;
    static final int RORS_APDU  = 0x0002;
    static final int ROLRS_APDU = 0x0005;

    static final int CMD_CONFIRMED_EVENT  = 0x0001;
    static final int CMD_GET              = 0x0003;
    static final int CMD_SET              = 0x0004;
    static final int CMD_CONFIRMED_SET    = 0x0005;
    static final int CMD_CONFIRMED_ACTION = 0x0007;

    static final int NOM_PART_OBJ   = 1;
    static final int NOM_PART_SCADA = 2;
    static final int NOM_PART_DIM   = 4;
    static final int NOM_MOC_VMS_MDS          = 0x0021;
    static final int NOM_MOC_VMO_METRIC_NU    = 0x0006;
    static final int NOM_MOC_VMO_METRIC_SA_RT = 0x0009;
    static final int NOM_MOC_VMO_AL_MON       = 0x0036;
    static final int NOM_MOC_PT_DEMOG         = 0x002A;
    static final int NOM_MOC_VMO_METRIC_ENUM  = 0x000A;
    static final int NOM_ACT_POLL_MDIB_DATA     = 0x0C16; // 3094
    static final int NOM_ACT_POLL_MDIB_DATA_EXT = 0xF13B; // 61755
    static final int NOM_NOTI_MDS_CREAT  = 0x0D06;

    static final int NOM_ATTR_NU_VAL_OBS         = 0x0950;
    static final int NOM_ATTR_ID_LABEL           = 0x0927;
    static final int NOM_ATTR_ID_LABEL_STRING     = 0x0928;
    static final int NOM_ATTR_METRIC_SPECN        = 0x093F;
    static final int NOM_ATTR_SA_VAL_OBS          = 0x096E;
    static final int NOM_ATTR_SA_SPECN            = 0x096D;
    static final int NOM_ATTR_SA_FIXED_VAL_SPECN  = 0x0A16;
    static final int NOM_ATTR_ID_TYPE             = 0x092F;
    static final int NOM_ATTR_SYS_TYPE            = 0x0986;
    static final int NOM_ATTR_ID_MODEL            = 0x0998;
    static final int NOM_ATTR_TIME_ABS            = 0x0987;
    static final int NOM_ATTR_DEV_AL_COND         = 0x0916;
    static final int NOM_ATTR_AL_MON_T_AL_LIST    = 0x0904;
    static final int NOM_ATTR_AL_MON_P_AL_LIST    = 0x0902;
    static final int NOM_ATTR_ENUM_OBS_VAL        = 0x0920;
    static final int NOM_ATTR_PT_NAME_GIVEN       = 0x0996;
    static final int NOM_ATTR_PT_NAME_FAMILY      = 0x0997;
    static final int NOM_ATTR_PT_ID               = 0x0984;
    static final int NOM_ATTR_PT_DOB              = 0x0958;
    static final int NOM_ATTR_PT_SEX              = 0x0993;
    static final int NOM_ATTR_PT_HEIGHT           = 0x0960;
    static final int NOM_ATTR_PT_WEIGHT           = 0x0994;
    static final int NOM_ATTR_PT_BSA              = 0x0995;

    static final int NOM_ECG_CARD_BEAT_RATE       = 0x4182;
    static final int NOM_PULS_OXIM_SAT_O2        = 0x4BB8;
    static final int NOM_PULS_OXIM_PULS_RATE     = 0x480A;
    static final int NOM_RESP_RATE               = 0x500A;
    static final int NOM_PRESS_BLD_ART_ABP_SYS   = 0x4A15;
    static final int NOM_PRESS_BLD_ART_ABP_DIA   = 0x4A16;
    static final int NOM_PRESS_BLD_ART_ABP_MEAN  = 0x4A17;
    static final int NOM_PRESS_BLD_NONINV_SYS    = 0x4A05;
    static final int NOM_PRESS_BLD_NONINV_DIA    = 0x4A06;
    static final int NOM_PRESS_BLD_NONINV_MEAN   = 0x4A07;
    static final int NOM_CO2_ET                  = 0x50B0;
    static final int NOM_TEMP_BLD                = 0xE014;
    static final int NOM_PRESS_BLD_VEN_CENT      = 0x4A44;
    static final int NOM_PRESS_BLD_ART_PULM_SYS  = 0x4A1D;
    static final int NOM_PRESS_BLD_ART_PULM_DIA  = 0x4A1E;
    static final int NOM_PRESS_BLD_ART_PULM_MEAN = 0x4A1F;

    static final int NOM_ECG_ELEC_POTL_II_SA  = 0x0102;
    static final int NOM_PLETH_PULS_OXIM_SA   = 0x4BB4;
    static final int NOM_PRESS_BLD_ART_ABP_SA = 0x4A14;
    static final int NOM_RESP_SA              = 0x5000;

    static final int NOM_DIM_PERCENT       = 0x0220;
    static final int NOM_DIM_BEAT_PER_MIN  = 0x0AA0;
    static final int NOM_DIM_MMHG          = 0x0F20;
    static final int NOM_DIM_RESP_PER_MIN  = 0x0AE0;
    static final int NOM_DIM_DEGC          = 0x17A0;
    static final int NOM_DIM_DIMLESS       = 0x0200;
    static final int NOM_DIM_MILLI_VOLT    = 0x0F72;

    static final int SEX_UNKNOWN = 0;
    static final int SEX_MALE    = 1;
    static final int SEX_FEMALE  = 2;

    static final int NOM_EVT_HI_HR      = 0x0402;
    static final int NOM_EVT_LO_HR      = 0x0403;
    static final int NOM_EVT_LO_SAT_O2  = 0x0404;
    static final int NOM_EVT_HI_ABP_SYS = 0x0410;

    static final int NOM_VENT_MODE_SIMV = 0x8003;

    static final int METRIC_CAT_MEAS       = 0x0001;
    static final int METRIC_ACCESS_AVAIL   = 0x0002;
    static final int METRIC_ACCESS_RD_ONLY = 0x0004;

    // =====================================================================
    // Waveform constants
    // =====================================================================
    static final int WAVEFORM_BUFFER_SIZE = 600;
    static final Color DARK_BG = new Color(10, 10, 10);
    static final Color PANEL_BG = new Color(25, 25, 25);
    static final Color GRID_COLOR = new Color(40, 40, 40);
    static final int TREND_BUFFER_SIZE = 300;

    // =====================================================================
    // Shared state
    // =====================================================================
    private final AtomicBoolean draegerRunning = new AtomicBoolean(false);
    private final AtomicBoolean philipsRunning = new AtomicBoolean(false);
    private Thread draegerThread;
    private Thread philipsThread;
    private Process draegerSerialProcess;
    private Process philipsSerialProcess;
    private final File draegerControlFile = new File(System.getProperty("java.io.tmpdir"), "critical-insights-draeger-rs232.properties");
    private final File philipsControlFile = new File(System.getProperty("java.io.tmpdir"), "critical-insights-philips-mib-rs232.properties");
    private volatile ServerSocket draegerServerSocket;
    private volatile Socket draegerClientSocket;
    private volatile DatagramSocket philipsSocket;

    // Freeze state
    private volatile boolean waveformFrozen = false;

    // Alarm state
    private volatile boolean alarmMuted = false;
    private volatile String currentAlarmText = "";
    private volatile int currentAlarmPriority = 0;
    private boolean alarmFlashState = false;
    private final JLabel drAlarmBar = new JLabel(" ");
    private final JLabel phAlarmBar = new JLabel(" ");
    private final JButton drMuteBtn = makeDarkButton("\uD83D\uDD0A");
    private final JButton phMuteBtn = makeDarkButton("\uD83D\uDD0A");

    // Fullscreen state
    private boolean isFullscreen = false;
    private Rectangle windowedBounds;

    // Event counter
    private final AtomicLong eventCount = new AtomicLong(0);
    private volatile long lastEventCountSnapshot = 0;
    private volatile double eventsPerSecond = 0;
    private final long startTime = System.currentTimeMillis();
    private volatile long lastPollTimestamp = 0;

    // =====================================================================
    // Draeger vitals sliders
    // =====================================================================
    private final JSlider slTidalVol   = makeDarkSlider(0, 800, 450);
    private final JSlider slRespRate   = makeDarkSlider(0, 40, 16);
    private final JSlider slPeakPres   = makeDarkSlider(0, 50, 22);
    private final JSlider slPeep       = makeDarkSlider(0, 20, 5);
    private final JSlider slFio2       = makeDarkSlider(21, 100, 40);
    private final JSlider slCompliance = makeDarkSlider(10, 100, 45);
    private final JSlider slResistance = makeDarkSlider(3, 40, 12);
    private final JSlider slAirwayTemp = makeDarkSlider(28, 40, 34);

    private final JComboBox<String> ventModeCombo = makeDarkCombo(
            new String[]{"IPPV", "SIMV", "CPAP", "ASB", "BIPAP", "PCV", "PSV", "APRV"});
    private final JCheckBox chkDrWaveforms = makeDarkCheck("Waveforms");

    // =====================================================================
    // Philips vitals sliders
    // =====================================================================
    private final JSlider slHeartRate   = makeDarkSlider(0, 200, 72);
    private final JSlider slSpo2       = makeDarkSlider(0, 100, 97);
    private final JSlider slAbpSys     = makeDarkSlider(0, 220, 120);
    private final JSlider slAbpDia     = makeDarkSlider(0, 130, 80);
    private final JSlider slEtCo2      = makeDarkSlider(0, 65, 38);
    private final JSlider slTemp       = makeDarkSlider(340, 410, 370);
    private final JSlider slPhRespRate = makeDarkSlider(0, 40, 16);
    private final JSlider slNbpSys     = makeDarkSlider(0, 220, 118);
    private final JSlider slNbpDia     = makeDarkSlider(0, 130, 76);
    private final JSlider slCvp        = makeDarkSlider(0, 30, 8);
    private final JSlider slPapSys     = makeDarkSlider(15, 60, 25);
    private final JSlider slPapDia     = makeDarkSlider(5, 30, 10);

    // =====================================================================
    // Alarm toggles
    // =====================================================================
    private final JCheckBox chkPawHigh    = makeDarkCheck("PAW HIGH");
    private final JCheckBox chkO2High     = makeDarkCheck("% O2 HIGH");
    private final JCheckBox chkMinVolLow  = makeDarkCheck("MV LOW");
    private final JCheckBox chkApnea      = makeDarkCheck("APNEA");
    private final JCheckBox chkPhHrAlarm  = makeDarkCheck("HR");
    private final JCheckBox chkPhSpo2Low  = makeDarkCheck("SpO2 Low");
    private final JCheckBox chkPhAbpHigh  = makeDarkCheck("ABP High");

    // =====================================================================
    // Patient demographics
    // =====================================================================
    private final JTextField tfPatientName   = makeDarkField("John Doe", 12);
    private final JTextField tfPatientId     = makeDarkField("P12345", 8);
    private final JTextField tfPatientDob    = makeDarkField("1980-01-15", 10);
    private final JComboBox<String> cbPatientSex = makeDarkCombo(new String[]{"M", "F", "Unknown"});
    private final JTextField tfPatientHeight = makeDarkField("175", 4);
    private final JTextField tfPatientWeight = makeDarkField("75", 4);

    private final JCheckBox chkPhWaveforms = makeDarkCheck("Waveforms");
    private final JCheckBox chkBroadcast   = makeDarkCheck("Broadcast");

    // =====================================================================
    // Connection controls
    // =====================================================================
    private final JTextField draegerPortField = makeDarkField("9100", 5);
    private final JTextField philipsPortField = makeDarkField("24105", 5);
    private final JComboBox<String> draegerTransportCombo = makeDarkCombo(new String[]{"TCP", "RS232"});
    private final JComboBox<String> philipsTransportCombo = makeDarkCombo(new String[]{"LAN/UDP", "MIB-RS232"});
    private final JTextField draegerSerialField = makeDarkField("/tmp/draeger-device", 16);
    private final JTextField philipsSerialField = makeDarkField("/tmp/philips-device", 16);
    private final JComboBox<String> modelCombo = makeDarkCombo(
            new String[]{"evita", "evita2", "evita4", "v500", "savina", "fabius"});

    private final JLabel draegerStatus     = new JLabel("Stopped");
    private final JLabel philipsStatus     = new JLabel("Stopped");
    private final JLabel draegerConnStatus = new JLabel("No connection");
    private final JLabel philipsConnStatus = new JLabel("No connection");

    private final JButton btnDraegerStart = makeDarkButton("Start");
    private final JButton btnDraegerStop  = makeDarkButton("Stop");
    private final JButton btnPhilipsStart = makeDarkButton("Start");
    private final JButton btnPhilipsStop  = makeDarkButton("Stop");

    // =====================================================================
    // Enhanced controls
    // =====================================================================
    private final JComboBox<String> drSweepSpeed = makeDarkCombo(new String[]{"25 mm/s", "12.5 mm/s", "50 mm/s"});
    private final JComboBox<String> phSweepSpeed = makeDarkCombo(new String[]{"25 mm/s", "12.5 mm/s", "50 mm/s"});
    private final JButton drFreezeBtn = makeDarkButton("Freeze");
    private final JButton phFreezeBtn = makeDarkButton("Freeze");
    private final JCheckBox chkNoise = makeDarkCheck("Add Noise");
    private final JCheckBox chkPhNoise = makeDarkCheck("Add Noise");

    // Scenario combos
    private final JComboBox<String> drScenarioCombo = makeDarkCombo(
            new String[]{"-- Scenario --", "Normal", "ARDS", "COPD", "Weaning", "Apnea"});
    private final JComboBox<String> phScenarioCombo = makeDarkCombo(
            new String[]{"-- Scenario --", "Normal", "Tachycardia", "Bradycardia", "Hypotension", "Cardiac Arrest", "Septic Shock"});

    // ECG rhythm combo
    private final JComboBox<String> ecgRhythmCombo = makeDarkCombo(
            new String[]{"Normal Sinus", "Sinus Tachycardia", "Sinus Bradycardia",
                          "Atrial Fibrillation", "Ventricular Tachycardia",
                          "Ventricular Fibrillation", "Asystole"});

    // NBP cycling
    private final JCheckBox chkNbpCycle = makeDarkCheck("NBP Auto-cycle");
    private final JComboBox<String> nbpIntervalCombo = makeDarkCombo(new String[]{"1 min", "3 min", "5 min", "15 min"});
    private final JLabel lblNbpStatus = new JLabel("NBP: --");

    // SpO2 Signal Quality
    private final JLabel lblSpo2SQ = new JLabel("SQ: ----------");

    // =====================================================================
    // Log
    // =====================================================================
    private final JTextArea logArea = new JTextArea();

    // =====================================================================
    // Waveform displays
    // =====================================================================
    private final WaveformPanel drWavePanel;
    private final float[][] drWaveBuffers = new float[3][WAVEFORM_BUFFER_SIZE];
    private int drWaveWritePos = 0;
    private final Object drWaveLock = new Object();
    static final Color[] DR_TRACE_COLORS = {
        new Color(255, 255, 0),
        new Color(255, 255, 255),
        new Color(255, 0, 255)
    };
    static final String[] DR_TRACE_LABELS = {"Airway Pressure", "Flow", "Volume"};

    private final WaveformPanel phWavePanel;
    private final float[][] phWaveBuffers = new float[4][WAVEFORM_BUFFER_SIZE];
    private int phWaveWritePos = 0;
    private final Object phWaveLock = new Object();
    static final Color[] PH_TRACE_COLORS = {
        new Color(0, 255, 0),
        new Color(0, 255, 255),
        new Color(255, 0, 0),
        new Color(255, 255, 0)
    };
    static final String[] PH_TRACE_LABELS = {"ECG Lead II", "SpO2 Pleth", "ABP", "Respiration"};

    // Draeger numeric display labels
    private final JLabel lblDrVt    = new JLabel("450");
    private final JLabel lblDrRr    = new JLabel("16");
    private final JLabel lblDrPaw   = new JLabel("22");
    private final JLabel lblDrPeep  = new JLabel("5");
    private final JLabel lblDrFio2  = new JLabel("40");
    private final JLabel lblDrMv    = new JLabel("7.2");
    private final JLabel lblDrCompl = new JLabel("45");
    private final JLabel lblDrRes   = new JLabel("12");
    private final JLabel lblDrTemp  = new JLabel("34");

    // Philips numeric display labels
    private final JLabel lblPhHr    = new JLabel("72");
    private final JLabel lblPhSpo2  = new JLabel("97");
    private final JLabel lblPhPr    = new JLabel("73");
    private final JLabel lblPhRr    = new JLabel("16");
    private final JLabel lblPhAbp   = new JLabel("120/80");
    private final JLabel lblPhAbpM  = new JLabel("(93)");
    private final JLabel lblPhNbp   = new JLabel("118/76");
    private final JLabel lblPhNbpM  = new JLabel("(90)");
    private final JLabel lblPhEtco2 = new JLabel("38");
    private final JLabel lblPhTemp  = new JLabel("37.0");
    private final JLabel lblPhCvp   = new JLabel("8");
    private final JLabel lblPhPap   = new JLabel("25/10");
    private final JLabel lblPhPapM  = new JLabel("(15)");

    // Trend sparklines (Draeger)
    private final SparklinePanel spDrVt   = new SparklinePanel(new Color(100, 200, 255));
    private final SparklinePanel spDrRr   = new SparklinePanel(new Color(150, 255, 150));
    private final SparklinePanel spDrPaw  = new SparklinePanel(new Color(255, 255, 0));
    private final SparklinePanel spDrPeep = new SparklinePanel(new Color(255, 180, 50));
    private final SparklinePanel spDrFio2 = new SparklinePanel(new Color(200, 150, 255));

    // Trend sparklines (Philips)
    private final SparklinePanel spPhHr    = new SparklinePanel(new Color(0, 255, 0));
    private final SparklinePanel spPhSpo2  = new SparklinePanel(new Color(0, 255, 255));
    private final SparklinePanel spPhRr    = new SparklinePanel(new Color(255, 255, 0));
    private final SparklinePanel spPhAbp   = new SparklinePanel(new Color(255, 0, 0));
    private final SparklinePanel spPhEtco2 = new SparklinePanel(new Color(255, 180, 50));
    private final SparklinePanel spPhTemp  = new SparklinePanel(new Color(200, 150, 255));

    // Waveform generation phase trackers
    private double drPawPhase = 0, drFlowPhase = 0, drVolPhase = 0;
    private double phEcgPhaseLocal = 0, phPlethPhaseLocal = 0, phAbpPhaseLocal = 0, phRespPhaseLocal = 0;
    private final Random waveRng = new Random();

    // Noise state
    private double noiseBaselinePhase = 0;
    private long noiseArtifactTimer = 0;
    private final Random noiseRng = new Random(99);

    // Draeger protocol state
    private volatile boolean drCommunicationInitialized = false;
    private volatile boolean drRealtimeEnabled = false;
    private final Set<Integer> drRealtimeConfiguredCodes = new LinkedHashSet<>();
    private Set<Integer> drConfiguredDataCodes = null;
    private long drWaveformSampleIndex = 0;
    private final Random drRng = new Random();
    private volatile boolean drTransmissionSuspended = false;

    // Philips protocol state
    private volatile boolean phAssociated = false;
    private int phInvokeId = 0;
    private int phPollNumber = 0;
    private int phSequenceNumber = 0;
    private final Random phRng = new Random(42);
    private double phEcgPhase = 0;
    private double phPlethPhase = 0;
    private double phAbpWavePhase = 0;
    private double phRespWavePhase = 0;
    private volatile ScheduledExecutorService broadcastExec;

    // Scenario animation
    private javax.swing.Timer scenarioAnimTimer;
    private Map<JSlider, Integer> scenarioTargets;
    private int scenarioAnimSteps = 0;

    // NBP cycling
    private javax.swing.Timer nbpCycleTimer;
    private javax.swing.Timer nbpInflateTimer;

    // Status bar
    private final JLabel statusBar = new JLabel("  Press F11 for fullscreen | Space to freeze | M to mute");

    // Tabs reference
    private JTabbedPane mainTabs;

    // Screenshot toast
    private final JLabel screenshotToast = new JLabel("");

    // =====================================================================
    // Constructor
    // =====================================================================

    public CriticalInsightsMonitor() {
        super("CriticalInsights Patient Monitor Simulator");
        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        getContentPane().setBackground(DARK_BG);
        setLayout(new BorderLayout(0, 0));

        for (int t = 0; t < 3; t++) Arrays.fill(drWaveBuffers[t], 0.5f);
        for (int t = 0; t < 4; t++) Arrays.fill(phWaveBuffers[t], 0.5f);

        drWavePanel = new WaveformPanel(drWaveBuffers, DR_TRACE_COLORS, DR_TRACE_LABELS, drWaveLock, 3, true);
        phWavePanel = new WaveformPanel(phWaveBuffers, PH_TRACE_COLORS, PH_TRACE_LABELS, phWaveLock, 4, false);

        mainTabs = new JTabbedPane();
        mainTabs.setBackground(new Color(30, 30, 30));
        mainTabs.setForeground(Color.WHITE);
        mainTabs.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 16));

        mainTabs.addTab("Draeger Ventilator", buildDraegerTab());
        mainTabs.addTab("Philips Monitor", buildPhilipsTab());
        mainTabs.addTab("Protocol Log", buildLogTab());

        add(mainTabs, BorderLayout.CENTER);

        // Status bar
        statusBar.setBackground(new Color(20, 20, 20));
        statusBar.setForeground(new Color(120, 120, 120));
        statusBar.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
        statusBar.setOpaque(true);
        statusBar.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(1, 0, 0, 0, new Color(50, 50, 50)),
            BorderFactory.createEmptyBorder(3, 8, 3, 8)));
        add(statusBar, BorderLayout.SOUTH);

        // Button actions
        btnDraegerStart.addActionListener(e -> startDraeger());
        btnDraegerStop.addActionListener(e -> stopDraeger());
        btnPhilipsStart.addActionListener(e -> startPhilips());
        btnPhilipsStop.addActionListener(e -> stopPhilips());
        btnDraegerStop.setEnabled(false);
        btnPhilipsStop.setEnabled(false);
        draegerTransportCombo.addActionListener(e -> updateTransportFields());
        philipsTransportCombo.addActionListener(e -> updateTransportFields());
        updateTransportFields();
        addWindowListener(new WindowAdapter() {
            @Override
            public void windowClosing(WindowEvent e) {
                stopDraeger();
                stopPhilips();
            }
        });
        new javax.swing.Timer(1000, e -> writeSerialControlFiles()).start();

        ventModeCombo.setSelectedItem("SIMV");

        // Freeze buttons
        drFreezeBtn.addActionListener(e -> toggleFreeze());
        phFreezeBtn.addActionListener(e -> toggleFreeze());

        // Mute buttons
        drMuteBtn.addActionListener(e -> toggleMute());
        phMuteBtn.addActionListener(e -> toggleMute());

        // Scenario combos
        drScenarioCombo.addActionListener(e -> applyDraegerScenario());
        phScenarioCombo.addActionListener(e -> applyPhilipsScenario());

        // Slider tooltips
        slTidalVol.setToolTipText("Normal: 400-600 mL");
        slRespRate.setToolTipText("Normal: 12-20 /min");
        slPeakPres.setToolTipText("Normal: 15-30 mbar");
        slPeep.setToolTipText("Normal: 3-8 mbar");
        slFio2.setToolTipText("Normal: 21-40 %");
        slCompliance.setToolTipText("Normal: 30-60 L/bar");
        slResistance.setToolTipText("Normal: 5-15 mbar/L/s");
        slAirwayTemp.setToolTipText("Normal: 32-37 C");
        slHeartRate.setToolTipText("Normal: 60-100 bpm");
        slSpo2.setToolTipText("Normal: 95-100 %");
        slAbpSys.setToolTipText("Normal: 90-140 mmHg");
        slAbpDia.setToolTipText("Normal: 60-90 mmHg");
        slEtCo2.setToolTipText("Normal: 35-45 mmHg");
        slTemp.setToolTipText("Normal: 365-375 (x10)");
        slPhRespRate.setToolTipText("Normal: 12-20 /min");
        slNbpSys.setToolTipText("Normal: 90-140 mmHg");
        slNbpDia.setToolTipText("Normal: 60-90 mmHg");
        slCvp.setToolTipText("Normal: 2-8 mmHg");
        slPapSys.setToolTipText("Normal: 15-30 mmHg");
        slPapDia.setToolTipText("Normal: 4-12 mmHg");

        // NBP cycling setup
        chkNbpCycle.addActionListener(e -> setupNbpCycling());

        // Waveform animation timer (30ms ~33fps)
        javax.swing.Timer animTimer = new javax.swing.Timer(30, e -> {
            if (!waveformFrozen) {
                generateDrWaveformSamples();
                generatePhWaveformSamples();
                drWavePanel.repaint();
                phWavePanel.repaint();
            }
            updateDrNumerics();
            updatePhNumerics();
            updateAlarms();
            noiseArtifactTimer++;
        });
        animTimer.start();

        // Alarm flash timer (500ms)
        javax.swing.Timer alarmFlashTimer = new javax.swing.Timer(500, e -> {
            alarmFlashState = !alarmFlashState;
            updateAlarmBars();
        });
        alarmFlashTimer.start();

        // Trend update timer (1 second)
        javax.swing.Timer trendTimer = new javax.swing.Timer(1000, e -> {
            updateTrends();
            updateStatusBar();
            updateSpo2SignalQuality();
        });
        trendTimer.start();

        // Events per second counter (1 second)
        javax.swing.Timer epsTimer = new javax.swing.Timer(1000, e -> {
            long current = eventCount.get();
            eventsPerSecond = current - lastEventCountSnapshot;
            lastEventCountSnapshot = current;
        });
        epsTimer.start();

        // Keyboard shortcuts
        KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(new KeyEventDispatcher() {
            @Override
            public boolean dispatchKeyEvent(KeyEvent e) {
                if (e.getID() != KeyEvent.KEY_PRESSED) return false;
                // Don't intercept if typing in a text field
                if (e.getSource() instanceof JTextField) return false;

                if (e.getKeyCode() == KeyEvent.VK_SPACE) {
                    toggleFreeze();
                    return true;
                } else if (e.getKeyCode() == KeyEvent.VK_F11) {
                    toggleFullscreen();
                    return true;
                } else if (e.getKeyCode() == KeyEvent.VK_1 && !e.isControlDown()) {
                    mainTabs.setSelectedIndex(0);
                    return true;
                } else if (e.getKeyCode() == KeyEvent.VK_2 && !e.isControlDown()) {
                    mainTabs.setSelectedIndex(1);
                    return true;
                } else if (e.getKeyCode() == KeyEvent.VK_3 && !e.isControlDown()) {
                    mainTabs.setSelectedIndex(2);
                    return true;
                } else if (e.getKeyCode() == KeyEvent.VK_S && e.isControlDown()) {
                    takeScreenshot();
                    return true;
                } else if (e.getKeyCode() == KeyEvent.VK_M && !e.isControlDown()) {
                    toggleMute();
                    return true;
                }
                return false;
            }
        });

        setPreferredSize(new Dimension(1400, 900));
        setMinimumSize(new Dimension(1100, 750));
        pack();
        setLocationRelativeTo(null);
    }

    // =====================================================================
    // Sparkline Panel -- tiny trend graph
    // =====================================================================

    private static class SparklinePanel extends JPanel {
        private final float[] buffer = new float[TREND_BUFFER_SIZE];
        private int writePos = 0;
        private boolean filled = false;
        private final Color lineColor;

        SparklinePanel(Color color) {
            this.lineColor = color;
            setBackground(new Color(15, 15, 15));
            setPreferredSize(new Dimension(150, 50));
            setMinimumSize(new Dimension(80, 30));
            setMaximumSize(new Dimension(150, 50));
            setBorder(BorderFactory.createLineBorder(new Color(50, 50, 50)));
            Arrays.fill(buffer, Float.NaN);
        }

        void addSample(float value) {
            buffer[writePos % TREND_BUFFER_SIZE] = value;
            writePos++;
            if (writePos >= TREND_BUFFER_SIZE) filled = true;
        }

        @Override
        protected void paintComponent(Graphics g0) {
            super.paintComponent(g0);
            Graphics2D g = (Graphics2D) g0;
            g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            int w = getWidth();
            int h = getHeight();

            int count = filled ? TREND_BUFFER_SIZE : writePos;
            if (count < 2) return;

            float min = Float.MAX_VALUE, max = Float.MIN_VALUE;
            int start = filled ? writePos : 0;
            for (int i = 0; i < count; i++) {
                float v = buffer[(start + i) % TREND_BUFFER_SIZE];
                if (Float.isNaN(v)) continue;
                if (v < min) min = v;
                if (v > max) max = v;
            }
            if (max - min < 0.01f) { min -= 1; max += 1; }

            g.setColor(lineColor.darker().darker());
            g.setStroke(new BasicStroke(1.0f));
            for (int i = 1; i < count; i++) {
                float v0 = buffer[(start + i - 1) % TREND_BUFFER_SIZE];
                float v1 = buffer[(start + i) % TREND_BUFFER_SIZE];
                if (Float.isNaN(v0) || Float.isNaN(v1)) continue;
                int x0 = (int)((i - 1.0) / (count - 1) * (w - 1));
                int x1 = (int)((double)i / (count - 1) * (w - 1));
                int y0 = h - 2 - (int)((v0 - min) / (max - min) * (h - 4));
                int y1 = h - 2 - (int)((v1 - min) / (max - min) * (h - 4));
                g.drawLine(x0, y0, x1, y1);
            }
        }
    }

    // =====================================================================
    // Waveform Panel -- custom drawing with sweep line
    // =====================================================================

    private class WaveformPanel extends JPanel {
        private BufferedImage backBuffer;
        private final float[][] buffers;
        private Color[] colors;
        private final String[] labels;
        private final Object lock;
        private final int numTraces;
        private final boolean isDraeger;
        private boolean showGrid = true;
        private final double[] vertScale;

        WaveformPanel(float[][] buffers, Color[] colors, String[] labels, Object lock, int numTraces, boolean isDraeger) {
            this.buffers = buffers;
            this.colors = colors.clone();
            this.labels = labels;
            this.lock = lock;
            this.numTraces = numTraces;
            this.isDraeger = isDraeger;
            this.vertScale = new double[numTraces];
            Arrays.fill(vertScale, 1.0);
            setBackground(DARK_BG);
            setMinimumSize(new Dimension(300, 100));
            setPreferredSize(new Dimension(700, 280));

            // Context menu
            JPopupMenu popup = new JPopupMenu();
            JMenuItem colorItem = new JMenuItem("Change Color...");
            colorItem.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
            colorItem.addActionListener(e -> {
                int traceIdx = getTraceAtY(lastPopupY);
                if (traceIdx >= 0 && traceIdx < numTraces) {
                    Color chosen = JColorChooser.showDialog(this, "Trace Color", this.colors[traceIdx]);
                    if (chosen != null) this.colors[traceIdx] = chosen;
                }
            });
            popup.add(colorItem);

            JMenu scaleMenu = new JMenu("Scale");
            scaleMenu.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
            for (String s : new String[]{"1x", "2x", "4x"}) {
                JMenuItem mi = new JMenuItem(s);
                mi.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
                double sv = s.equals("1x") ? 1.0 : s.equals("2x") ? 2.0 : 4.0;
                mi.addActionListener(ev -> {
                    int traceIdx = getTraceAtY(lastPopupY);
                    if (traceIdx >= 0 && traceIdx < numTraces) vertScale[traceIdx] = sv;
                });
                scaleMenu.add(mi);
            }
            popup.add(scaleMenu);

            JMenuItem gridItem = new JMenuItem("Grid On/Off");
            gridItem.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
            gridItem.addActionListener(e -> showGrid = !showGrid);
            popup.add(gridItem);

            JMenuItem exportItem = new JMenuItem("Export Waveform Data...");
            exportItem.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
            exportItem.addActionListener(e -> exportWaveformData());
            popup.add(exportItem);

            addMouseListener(new MouseAdapter() {
                @Override
                public void mousePressed(MouseEvent e) { maybeShowPopup(e); }
                @Override
                public void mouseReleased(MouseEvent e) { maybeShowPopup(e); }
                private void maybeShowPopup(MouseEvent e) {
                    if (e.isPopupTrigger()) {
                        lastPopupY = e.getY();
                        popup.show(e.getComponent(), e.getX(), e.getY());
                    }
                }
            });
        }

        private int lastPopupY = 0;

        private int getTraceAtY(int y) {
            int h = getHeight();
            if (h <= 0) return 0;
            int traceHeight = h / numTraces;
            return Math.min(y / Math.max(traceHeight, 1), numTraces - 1);
        }

        private void exportWaveformData() {
            JFileChooser fc = new JFileChooser();
            fc.setSelectedFile(new java.io.File("waveform_data.csv"));
            if (fc.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
                try (PrintWriter pw = new PrintWriter(fc.getSelectedFile())) {
                    StringBuilder header = new StringBuilder("Sample");
                    for (String l : labels) header.append(",").append(l);
                    pw.println(header);
                    synchronized (lock) {
                        int wp = isDraeger ? drWaveWritePos : phWaveWritePos;
                        for (int i = 0; i < WAVEFORM_BUFFER_SIZE; i++) {
                            int idx = (wp + i) % WAVEFORM_BUFFER_SIZE;
                            StringBuilder line = new StringBuilder(String.valueOf(i));
                            for (int t = 0; t < numTraces; t++) {
                                line.append(",").append(String.format("%.4f", buffers[t][idx]));
                            }
                            pw.println(line);
                        }
                    }
                } catch (IOException ex) {
                    log("[ERROR] Export failed: " + ex.getMessage());
                }
            }
        }

        int getWritePos() {
            if (buffers == drWaveBuffers) return drWaveWritePos;
            return phWaveWritePos;
        }

        double getSweepSpeedFactor() {
            JComboBox<String> combo = isDraeger ? drSweepSpeed : phSweepSpeed;
            String sel = (String) combo.getSelectedItem();
            if (sel != null && sel.startsWith("12.5")) return 2.0;
            if (sel != null && sel.startsWith("50")) return 0.5;
            return 1.0;
        }

        @Override
        protected void paintComponent(Graphics g0) {
            super.paintComponent(g0);
            int w = getWidth();
            int h = getHeight();
            if (w <= 0 || h <= 0) return;

            if (backBuffer == null || backBuffer.getWidth() != w || backBuffer.getHeight() != h) {
                backBuffer = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
            }

            Graphics2D g = backBuffer.createGraphics();
            g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);

            g.setColor(DARK_BG);
            g.fillRect(0, 0, w, h);

            // Grid lines
            if (showGrid) {
                g.setColor(GRID_COLOR);
                int gridSpacing = 30;
                for (int x = 0; x < w; x += gridSpacing) g.drawLine(x, 0, x, h);
                for (int y = 0; y < h; y += gridSpacing) g.drawLine(0, y, w, y);
            }

            int traceHeight = h / numTraces;
            int writePos = getWritePos();

            synchronized (lock) {
                for (int t = 0; t < numTraces; t++) {
                    int yOffset = t * traceHeight;

                    g.setColor(colors[t]);
                    g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 14));
                    g.drawString(labels[t], 6, yOffset + 18);

                    g.setColor(new Color(60, 60, 60));
                    g.drawLine(0, yOffset, w, yOffset);

                    g.setColor(colors[t]);
                    g.setStroke(new BasicStroke(1.8f));
                    double scale = vertScale[t];
                    for (int i = 0; i < WAVEFORM_BUFFER_SIZE - 1; i++) {
                        int bufIdx = (writePos + i) % WAVEFORM_BUFFER_SIZE;
                        int bufIdxNext = (writePos + i + 1) % WAVEFORM_BUFFER_SIZE;

                        if (bufIdxNext == writePos % WAVEFORM_BUFFER_SIZE) continue;

                        int x1 = (int)((double)i / WAVEFORM_BUFFER_SIZE * w);
                        int x2 = (int)((double)(i + 1) / WAVEFORM_BUFFER_SIZE * w);
                        float val1 = buffers[t][bufIdx];
                        float val2 = buffers[t][bufIdxNext];

                        // Apply vertical scaling around center
                        double center = 0.5;
                        val1 = (float)(center + (val1 - center) * scale);
                        val2 = (float)(center + (val2 - center) * scale);

                        int y1 = yOffset + (int)((1.0f - val1) * traceHeight);
                        int y2 = yOffset + (int)((1.0f - val2) * traceHeight);

                        y1 = Math.max(yOffset + 1, Math.min(yOffset + traceHeight - 1, y1));
                        y2 = Math.max(yOffset + 1, Math.min(yOffset + traceHeight - 1, y2));

                        g.drawLine(x1, y1, x2, y2);
                    }
                }

                // Sweep line
                int sweepX = (int)((double)(writePos % WAVEFORM_BUFFER_SIZE) / WAVEFORM_BUFFER_SIZE * w);
                g.setColor(new Color(255, 255, 255, 180));
                g.setStroke(new BasicStroke(2.0f));
                g.drawLine(sweepX, 0, sweepX, h);

                // Erase zone
                g.setColor(new Color(10, 10, 10, 200));
                int eraseWidth = (int)(w * 0.05);
                g.fillRect(sweepX + 1, 0, eraseWidth, h);
            }

            // Frozen indicator
            if (waveformFrozen) {
                g.setColor(new Color(255, 255, 0, 200));
                g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 24));
                g.drawString("FROZEN", w / 2 - 50, 30);
            }

            g.dispose();
            g0.drawImage(backBuffer, 0, 0, null);
        }
    }

    // =====================================================================
    // Draeger Tab
    // =====================================================================

    private JPanel buildDraegerTab() {
        JPanel tab = new JPanel(new BorderLayout(0, 0));
        tab.setBackground(DARK_BG);

        // North panel: connection bar + alarm bar
        JPanel northPanel = new JPanel(new BorderLayout(0, 0));
        northPanel.setBackground(DARK_BG);
        northPanel.add(buildDraegerConnectionBar(), BorderLayout.NORTH);
        northPanel.add(buildAlarmBar(drAlarmBar, drMuteBtn), BorderLayout.SOUTH);
        tab.add(northPanel, BorderLayout.NORTH);

        JSplitPane hSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, drWavePanel, buildDraegerNumericsPanel());
        hSplit.setBackground(DARK_BG);
        hSplit.setDividerLocation(800);
        hSplit.setResizeWeight(0.75);
        hSplit.setDividerSize(5);
        hSplit.setContinuousLayout(true);

        JPanel controlsArea = buildDraegerControlsArea();
        JSplitPane vSplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT, hSplit, controlsArea);
        vSplit.setBackground(DARK_BG);
        vSplit.setDividerLocation(300);
        vSplit.setResizeWeight(0.4);
        vSplit.setDividerSize(5);
        vSplit.setContinuousLayout(true);

        tab.add(vSplit, BorderLayout.CENTER);

        return tab;
    }

    private JPanel buildAlarmBar(JLabel alarmLabel, JButton muteButton) {
        JPanel bar = new JPanel(new BorderLayout(4, 0));
        bar.setBackground(Color.GRAY);
        bar.setPreferredSize(new Dimension(0, 32));
        bar.setBorder(BorderFactory.createEmptyBorder(2, 8, 2, 8));

        alarmLabel.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 18));
        alarmLabel.setForeground(Color.WHITE);
        alarmLabel.setHorizontalAlignment(SwingConstants.CENTER);

        muteButton.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
        muteButton.setPreferredSize(new Dimension(50, 28));

        bar.add(alarmLabel, BorderLayout.CENTER);
        bar.add(muteButton, BorderLayout.EAST);

        return bar;
    }

    private JPanel buildDraegerConnectionBar() {
        JPanel bar = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 6));
        bar.setBackground(new Color(30, 30, 30));
        bar.setBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, new Color(60, 60, 60)));

        bar.add(makeBoldLabel("Connection:", 14));
        bar.add(makeBoldLabel("Transport", 14));
        bar.add(draegerTransportCombo);
        bar.add(makeBoldLabel("TCP Port", 14));
        bar.add(draegerPortField);
        bar.add(makeBoldLabel("Serial", 14));
        bar.add(draegerSerialField);
        bar.add(makeBoldLabel("Model", 14));
        bar.add(modelCombo);
        bar.add(btnDraegerStart);
        bar.add(btnDraegerStop);

        draegerStatus.setForeground(Color.GRAY);
        draegerStatus.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 13));
        bar.add(draegerStatus);

        draegerConnStatus.setForeground(new Color(100, 100, 100));
        draegerConnStatus.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 13));
        bar.add(draegerConnStatus);

        bar.add(Box.createHorizontalStrut(10));
        bar.add(makeBoldLabel("Speed:", 14));
        bar.add(drSweepSpeed);
        bar.add(drFreezeBtn);

        JButton ssBtn = makeDarkButton("\uD83D\uDCF8 Screenshot");
        ssBtn.addActionListener(e -> takeScreenshot());
        bar.add(ssBtn);

        return bar;
    }

    private JPanel buildDraegerNumericsPanel() {
        JPanel panel = new JPanel();
        panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
        panel.setBackground(DARK_BG);
        panel.setMinimumSize(new Dimension(180, 0));
        panel.setPreferredSize(new Dimension(310, 0));
        panel.setBorder(BorderFactory.createMatteBorder(0, 2, 0, 0, new Color(60, 60, 60)));

        Color yellow  = DR_TRACE_COLORS[0];
        Color white   = DR_TRACE_COLORS[1];
        Color magenta = DR_TRACE_COLORS[2];
        Color cyan    = new Color(100, 200, 255);
        Color lime    = new Color(150, 255, 150);
        Color orange  = new Color(255, 180, 50);
        Color pink    = new Color(255, 150, 200);
        Color lav     = new Color(200, 150, 255);
        Color teal    = new Color(100, 255, 200);

        panel.add(buildNumericBoxWithSparkline("VT",     lblDrVt,    "mL",       cyan,    3, spDrVt));
        panel.add(buildNumericBoxWithSparkline("RR",     lblDrRr,    "/min",     lime,    3, spDrRr));
        panel.add(buildNumericBoxWithSparkline("Paw",    lblDrPaw,   "mbar",     yellow,  3, spDrPaw));
        panel.add(buildNumericBoxWithSparkline("PEEP",   lblDrPeep,  "mbar",     orange,  3, spDrPeep));
        panel.add(buildNumericBoxWithSparkline("FiO2",   lblDrFio2,  "%",        lav,     3, spDrFio2));
        panel.add(buildNumericBox("MV",     lblDrMv,    "L/min",    white,   3));
        panel.add(buildNumericBox("Compl",  lblDrCompl, "L/bar",    lime,    3));
        panel.add(buildNumericBox("Resist", lblDrRes,   "mbar/L/s", pink,    3));
        panel.add(buildNumericBox("Temp",   lblDrTemp,  "\u00B0C",  teal,    3));
        panel.add(Box.createVerticalGlue());
        return panel;
    }

    private JPanel buildDraegerControlsArea() {
        JPanel outer = new JPanel(new BorderLayout(0, 4));
        outer.setBackground(PANEL_BG);
        outer.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(1, 0, 0, 0, new Color(60, 60, 60)),
            BorderFactory.createEmptyBorder(6, 10, 6, 10)
        ));

        // Title row with scenario
        JPanel titleRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        titleRow.setBackground(PANEL_BG);
        JLabel title = new JLabel("Vital Controls");
        title.setForeground(Color.LIGHT_GRAY);
        title.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 15));
        titleRow.add(title);
        titleRow.add(Box.createHorizontalStrut(20));
        titleRow.add(makeBoldLabel("Scenario:", 14));
        titleRow.add(drScenarioCombo);
        titleRow.add(Box.createHorizontalStrut(10));
        titleRow.add(chkNoise);

        JPanel sliders = new JPanel(new GridLayout(4, 2, 16, 6));
        sliders.setBackground(PANEL_BG);
        sliders.setBorder(BorderFactory.createEmptyBorder(4, 0, 4, 0));

        addSliderRow(sliders, "Tidal Vol (mL)",  slTidalVol);
        addSliderRow(sliders, "Resp Rate",       slRespRate);
        addSliderRow(sliders, "Peak Press",      slPeakPres);
        addSliderRow(sliders, "PEEP",            slPeep);
        addSliderRow(sliders, "FiO2 (%)",        slFio2);
        addSliderRow(sliders, "Compliance",      slCompliance);
        addSliderRow(sliders, "Resistance",      slResistance);
        addSliderRow(sliders, "Airway Temp",     slAirwayTemp);

        JPanel bottomRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 12, 4));
        bottomRow.setBackground(PANEL_BG);
        bottomRow.add(makeBoldLabel("Mode:", 14));
        bottomRow.add(ventModeCombo);
        bottomRow.add(chkDrWaveforms);
        bottomRow.add(Box.createHorizontalStrut(20));
        bottomRow.add(makeBoldLabel("Alarms:", 14));
        bottomRow.add(chkPawHigh);
        bottomRow.add(chkApnea);
        bottomRow.add(chkO2High);
        bottomRow.add(chkMinVolLow);

        outer.add(titleRow, BorderLayout.NORTH);
        outer.add(sliders, BorderLayout.CENTER);
        outer.add(bottomRow, BorderLayout.SOUTH);
        return outer;
    }

    // =====================================================================
    // Philips Tab
    // =====================================================================

    private JPanel buildPhilipsTab() {
        JPanel tab = new JPanel(new BorderLayout(0, 0));
        tab.setBackground(DARK_BG);

        JPanel northPanel = new JPanel(new BorderLayout(0, 0));
        northPanel.setBackground(DARK_BG);
        northPanel.add(buildPhilipsConnectionBar(), BorderLayout.NORTH);
        northPanel.add(buildAlarmBar(phAlarmBar, phMuteBtn), BorderLayout.SOUTH);
        tab.add(northPanel, BorderLayout.NORTH);

        JSplitPane hSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, phWavePanel, buildPhilipsNumericsPanel());
        hSplit.setBackground(DARK_BG);
        hSplit.setDividerLocation(800);
        hSplit.setResizeWeight(0.7);
        hSplit.setDividerSize(5);
        hSplit.setContinuousLayout(true);

        JPanel controlsArea = buildPhilipsControlsArea();
        JSplitPane vSplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT, hSplit, controlsArea);
        vSplit.setBackground(DARK_BG);
        vSplit.setDividerLocation(300);
        vSplit.setResizeWeight(0.4);
        vSplit.setDividerSize(5);
        vSplit.setContinuousLayout(true);

        tab.add(vSplit, BorderLayout.CENTER);

        return tab;
    }

    private JPanel buildPhilipsConnectionBar() {
        JPanel bar = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 6));
        bar.setBackground(new Color(30, 30, 30));
        bar.setBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, new Color(60, 60, 60)));

        bar.add(makeBoldLabel("Connection:", 14));
        bar.add(makeBoldLabel("Transport", 14));
        bar.add(philipsTransportCombo);
        bar.add(makeBoldLabel("UDP Port", 14));
        bar.add(philipsPortField);
        bar.add(makeBoldLabel("Serial", 14));
        bar.add(philipsSerialField);
        bar.add(chkBroadcast);
        bar.add(btnPhilipsStart);
        bar.add(btnPhilipsStop);

        philipsStatus.setForeground(Color.GRAY);
        philipsStatus.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 13));
        bar.add(philipsStatus);

        philipsConnStatus.setForeground(new Color(100, 100, 100));
        philipsConnStatus.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 13));
        bar.add(philipsConnStatus);

        bar.add(Box.createHorizontalStrut(10));
        bar.add(makeBoldLabel("Speed:", 14));
        bar.add(phSweepSpeed);
        bar.add(phFreezeBtn);

        JButton ssBtn = makeDarkButton("\uD83D\uDCF8 Screenshot");
        ssBtn.addActionListener(e -> takeScreenshot());
        bar.add(ssBtn);

        return bar;
    }

    private JPanel buildPhilipsNumericsPanel() {
        JPanel panel = new JPanel();
        panel.setLayout(new BoxLayout(panel, BoxLayout.Y_AXIS));
        panel.setBackground(DARK_BG);
        panel.setMinimumSize(new Dimension(180, 0));
        panel.setPreferredSize(new Dimension(330, 0));
        panel.setBorder(BorderFactory.createMatteBorder(0, 2, 0, 0, new Color(60, 60, 60)));

        Color green  = PH_TRACE_COLORS[0];
        Color cyan   = PH_TRACE_COLORS[1];
        Color red    = PH_TRACE_COLORS[2];
        Color yellow = PH_TRACE_COLORS[3];
        Color white  = new Color(220, 220, 220);
        Color blue   = new Color(100, 180, 255);
        Color orange = new Color(255, 180, 50);
        Color purple = new Color(200, 150, 255);

        panel.add(buildNumericBoxWithSparkline("HR",    lblPhHr,    "bpm",    green,  2, spPhHr));
        panel.add(buildNumericBoxWithSparkline("SpO2",  lblPhSpo2,  "%",      cyan,   2, spPhSpo2));
        panel.add(buildNumericBox("PR",    lblPhPr,    "bpm",    cyan,   2));
        panel.add(buildNumericBoxWithSparkline("RR",    lblPhRr,    "/min",   yellow, 2, spPhRr));
        panel.add(buildCompoundNumericBoxWithSparkline("ABP", lblPhAbp, lblPhAbpM, "mmHg", red, 2, spPhAbp));
        panel.add(buildCompoundNumericBox("NBP", lblPhNbp, lblPhNbpM, "mmHg", white, 2));
        panel.add(buildNumericBoxWithSparkline("etCO2", lblPhEtco2, "mmHg",   orange, 2, spPhEtco2));
        panel.add(buildNumericBoxWithSparkline("Temp",  lblPhTemp,  "\u00B0C", purple, 2, spPhTemp));
        panel.add(buildNumericBox("CVP",   lblPhCvp,   "mmHg",   blue,   2));
        panel.add(buildCompoundNumericBox("PAP", lblPhPap, lblPhPapM, "mmHg", blue, 2));

        // SpO2 SQ indicator
        JPanel sqPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 4, 2));
        sqPanel.setBackground(DARK_BG);
        sqPanel.setMaximumSize(new Dimension(300, 24));
        lblSpo2SQ.setFont(new Font(Font.MONOSPACED, Font.BOLD, 12));
        lblSpo2SQ.setForeground(new Color(0, 200, 0));
        sqPanel.add(lblSpo2SQ);
        panel.add(sqPanel);

        // NBP status
        JPanel nbpPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 4, 2));
        nbpPanel.setBackground(DARK_BG);
        nbpPanel.setMaximumSize(new Dimension(300, 24));
        lblNbpStatus.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 11));
        lblNbpStatus.setForeground(new Color(180, 180, 180));
        nbpPanel.add(lblNbpStatus);
        panel.add(nbpPanel);

        panel.add(Box.createVerticalGlue());
        return panel;
    }

    private JPanel buildPhilipsControlsArea() {
        JPanel outer = new JPanel(new BorderLayout(0, 4));
        outer.setBackground(PANEL_BG);
        outer.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(1, 0, 0, 0, new Color(60, 60, 60)),
            BorderFactory.createEmptyBorder(6, 10, 6, 10)
        ));

        // Title row with scenario + rhythm
        JPanel titleRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        titleRow.setBackground(PANEL_BG);
        JLabel title = new JLabel("Vital Controls");
        title.setForeground(Color.LIGHT_GRAY);
        title.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 15));
        titleRow.add(title);
        titleRow.add(Box.createHorizontalStrut(10));
        titleRow.add(makeBoldLabel("Scenario:", 14));
        titleRow.add(phScenarioCombo);
        titleRow.add(Box.createHorizontalStrut(10));
        titleRow.add(makeBoldLabel("Rhythm:", 14));
        titleRow.add(ecgRhythmCombo);
        titleRow.add(Box.createHorizontalStrut(10));
        titleRow.add(chkPhNoise);
        titleRow.add(Box.createHorizontalStrut(10));
        titleRow.add(chkNbpCycle);
        titleRow.add(nbpIntervalCombo);

        JPanel sliders = new JPanel(new GridLayout(4, 3, 16, 6));
        sliders.setBackground(PANEL_BG);
        sliders.setBorder(BorderFactory.createEmptyBorder(4, 0, 4, 0));

        addSliderRow(sliders, "Heart Rate (bpm)", slHeartRate);
        addSliderRow(sliders, "SpO2 (%)",         slSpo2);
        addSliderRow(sliders, "Resp Rate",        slPhRespRate);
        addSliderRow(sliders, "ABP Sys (mmHg)",   slAbpSys);
        addSliderRow(sliders, "ABP Dia (mmHg)",   slAbpDia);
        addSliderRow(sliders, "NBP Sys",          slNbpSys);
        addSliderRow(sliders, "NBP Dia",          slNbpDia);
        addSliderRow(sliders, "etCO2 (mmHg)",     slEtCo2);
        addSliderRow(sliders, "Temp (\u00B0Cx10)",slTemp);
        addSliderRow(sliders, "CVP",              slCvp);
        addSliderRow(sliders, "PAP Sys",          slPapSys);
        addSliderRow(sliders, "PAP Dia",          slPapDia);

        JPanel bottomRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 10, 4));
        bottomRow.setBackground(PANEL_BG);
        bottomRow.add(chkPhWaveforms);
        bottomRow.add(Box.createHorizontalStrut(10));
        bottomRow.add(makeBoldLabel("Alarms:", 14));
        bottomRow.add(chkPhHrAlarm);
        bottomRow.add(chkPhSpo2Low);
        bottomRow.add(chkPhAbpHigh);
        bottomRow.add(Box.createHorizontalStrut(20));
        bottomRow.add(makeBoldLabel("Patient:", 14));
        bottomRow.add(makeBoldLabel("Name", 12));
        bottomRow.add(tfPatientName);
        bottomRow.add(makeBoldLabel("ID", 12));
        bottomRow.add(tfPatientId);
        bottomRow.add(makeBoldLabel("Sex", 12));
        bottomRow.add(cbPatientSex);
        bottomRow.add(makeBoldLabel("Ht", 12));
        bottomRow.add(tfPatientHeight);
        bottomRow.add(makeBoldLabel("Wt", 12));
        bottomRow.add(tfPatientWeight);

        outer.add(titleRow, BorderLayout.NORTH);
        outer.add(sliders, BorderLayout.CENTER);
        outer.add(bottomRow, BorderLayout.SOUTH);
        return outer;
    }

    // =====================================================================
    // Log Tab
    // =====================================================================

    private JPanel buildLogTab() {
        JPanel panel = new JPanel(new BorderLayout());
        panel.setBackground(PANEL_BG);
        logArea.setEditable(false);
        logArea.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 13));
        logArea.setBackground(new Color(15, 15, 15));
        logArea.setForeground(new Color(0, 200, 0));
        logArea.setCaretColor(new Color(0, 200, 0));
        logArea.setLineWrap(true);
        JScrollPane scroll = new JScrollPane(logArea);
        scroll.setBorder(BorderFactory.createEmptyBorder());
        panel.add(scroll, BorderLayout.CENTER);

        JPanel buttons = new JPanel(new FlowLayout(FlowLayout.RIGHT));
        buttons.setBackground(PANEL_BG);
        JButton clear = makeDarkButton("Clear Log");
        clear.addActionListener(e -> logArea.setText(""));
        buttons.add(clear);
        panel.add(buttons, BorderLayout.SOUTH);
        return panel;
    }

    // =====================================================================
    // Numeric box builders
    // =====================================================================

    private JPanel buildNumericBox(String label, JLabel valueLabel, String unit, Color color, int borderWidth) {
        JPanel box = new JPanel(new BorderLayout(2, 0));
        box.setBackground(DARK_BG);
        box.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, borderWidth, 1, 0, color.darker()),
            BorderFactory.createEmptyBorder(2, 8, 2, 8)
        ));
        box.setMaximumSize(new Dimension(330, 58));
        box.setPreferredSize(new Dimension(280, 52));

        JLabel lbl = new JLabel(label);
        lbl.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 14));
        lbl.setForeground(color);

        valueLabel.setFont(new Font(Font.MONOSPACED, Font.BOLD, 40));
        valueLabel.setForeground(color);
        valueLabel.setHorizontalAlignment(SwingConstants.RIGHT);

        JLabel unitLbl = new JLabel(unit);
        unitLbl.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
        unitLbl.setForeground(color.darker());

        JPanel topRow = new JPanel(new BorderLayout());
        topRow.setBackground(DARK_BG);
        topRow.add(lbl, BorderLayout.WEST);
        topRow.add(unitLbl, BorderLayout.EAST);

        box.add(topRow, BorderLayout.NORTH);
        box.add(valueLabel, BorderLayout.CENTER);

        return box;
    }

    private JPanel buildNumericBoxWithSparkline(String label, JLabel valueLabel, String unit, Color color, int borderWidth, SparklinePanel sparkline) {
        JPanel box = new JPanel(new BorderLayout(2, 0));
        box.setBackground(DARK_BG);
        box.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, borderWidth, 1, 0, color.darker()),
            BorderFactory.createEmptyBorder(2, 8, 2, 8)
        ));
        box.setMaximumSize(new Dimension(330, 62));
        box.setPreferredSize(new Dimension(280, 56));

        JLabel lbl = new JLabel(label);
        lbl.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 14));
        lbl.setForeground(color);

        valueLabel.setFont(new Font(Font.MONOSPACED, Font.BOLD, 40));
        valueLabel.setForeground(color);
        valueLabel.setHorizontalAlignment(SwingConstants.RIGHT);

        JLabel unitLbl = new JLabel(unit);
        unitLbl.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
        unitLbl.setForeground(color.darker());

        JPanel topRow = new JPanel(new BorderLayout());
        topRow.setBackground(DARK_BG);
        topRow.add(lbl, BorderLayout.WEST);
        topRow.add(unitLbl, BorderLayout.EAST);

        JPanel center = new JPanel(new BorderLayout(4, 0));
        center.setBackground(DARK_BG);
        center.add(valueLabel, BorderLayout.CENTER);
        sparkline.setPreferredSize(new Dimension(100, 40));
        sparkline.setMaximumSize(new Dimension(100, 40));
        center.add(sparkline, BorderLayout.EAST);

        box.add(topRow, BorderLayout.NORTH);
        box.add(center, BorderLayout.CENTER);

        return box;
    }

    private JPanel buildCompoundNumericBox(String label, JLabel valLabel, JLabel meanLabel, String unit, Color color, int borderWidth) {
        JPanel box = new JPanel(new BorderLayout(2, 0));
        box.setBackground(DARK_BG);
        box.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, borderWidth, 1, 0, color.darker()),
            BorderFactory.createEmptyBorder(2, 8, 2, 8)
        ));
        box.setMaximumSize(new Dimension(330, 66));
        box.setPreferredSize(new Dimension(280, 60));

        JLabel lbl = new JLabel(label);
        lbl.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 14));
        lbl.setForeground(color);

        JLabel unitLbl = new JLabel(unit);
        unitLbl.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
        unitLbl.setForeground(color.darker());

        valLabel.setFont(new Font(Font.MONOSPACED, Font.BOLD, 36));
        valLabel.setForeground(color);
        valLabel.setHorizontalAlignment(SwingConstants.RIGHT);

        meanLabel.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 18));
        meanLabel.setForeground(color);
        meanLabel.setHorizontalAlignment(SwingConstants.RIGHT);

        JPanel topRow = new JPanel(new BorderLayout());
        topRow.setBackground(DARK_BG);
        topRow.add(lbl, BorderLayout.WEST);
        topRow.add(unitLbl, BorderLayout.EAST);

        JPanel valPanel = new JPanel(new BorderLayout());
        valPanel.setBackground(DARK_BG);
        valPanel.add(valLabel, BorderLayout.CENTER);
        valPanel.add(meanLabel, BorderLayout.SOUTH);

        box.add(topRow, BorderLayout.NORTH);
        box.add(valPanel, BorderLayout.CENTER);

        return box;
    }

    private JPanel buildCompoundNumericBoxWithSparkline(String label, JLabel valLabel, JLabel meanLabel, String unit, Color color, int borderWidth, SparklinePanel sparkline) {
        JPanel box = new JPanel(new BorderLayout(2, 0));
        box.setBackground(DARK_BG);
        box.setBorder(BorderFactory.createCompoundBorder(
            BorderFactory.createMatteBorder(0, borderWidth, 1, 0, color.darker()),
            BorderFactory.createEmptyBorder(2, 8, 2, 8)
        ));
        box.setMaximumSize(new Dimension(330, 70));
        box.setPreferredSize(new Dimension(280, 64));

        JLabel lbl = new JLabel(label);
        lbl.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 14));
        lbl.setForeground(color);

        JLabel unitLbl = new JLabel(unit);
        unitLbl.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 12));
        unitLbl.setForeground(color.darker());

        valLabel.setFont(new Font(Font.MONOSPACED, Font.BOLD, 36));
        valLabel.setForeground(color);
        valLabel.setHorizontalAlignment(SwingConstants.RIGHT);

        meanLabel.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 18));
        meanLabel.setForeground(color);
        meanLabel.setHorizontalAlignment(SwingConstants.RIGHT);

        JPanel topRow = new JPanel(new BorderLayout());
        topRow.setBackground(DARK_BG);
        topRow.add(lbl, BorderLayout.WEST);
        topRow.add(unitLbl, BorderLayout.EAST);

        JPanel valPanel = new JPanel(new BorderLayout());
        valPanel.setBackground(DARK_BG);
        valPanel.add(valLabel, BorderLayout.CENTER);
        valPanel.add(meanLabel, BorderLayout.SOUTH);

        JPanel center = new JPanel(new BorderLayout(4, 0));
        center.setBackground(DARK_BG);
        center.add(valPanel, BorderLayout.CENTER);
        sparkline.setPreferredSize(new Dimension(100, 50));
        sparkline.setMaximumSize(new Dimension(100, 50));
        center.add(sparkline, BorderLayout.EAST);

        box.add(topRow, BorderLayout.NORTH);
        box.add(center, BorderLayout.CENTER);

        return box;
    }

    // =====================================================================
    // Dark UI helpers
    // =====================================================================

    private static JLabel makeBoldLabel(String text, int size) {
        JLabel l = new JLabel(text);
        l.setForeground(Color.LIGHT_GRAY);
        l.setFont(new Font(Font.SANS_SERIF, Font.BOLD, size));
        return l;
    }

    private static JButton makeDarkButton(String text) {
        JButton b = new JButton(text);
        b.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 14));
        return b;
    }

    private static JComboBox<String> makeDarkCombo(String[] items) {
        JComboBox<String> cb = new JComboBox<>(items);
        cb.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
        return cb;
    }

    private static JSlider makeDarkSlider(int min, int max, int value) {
        JSlider s = new JSlider(min, max, value);
        s.setPaintTicks(false);
        s.setBackground(PANEL_BG);
        s.setForeground(Color.LIGHT_GRAY);
        return s;
    }

    private static JCheckBox makeDarkCheck(String text) {
        JCheckBox cb = new JCheckBox(text);
        cb.setForeground(Color.LIGHT_GRAY);
        cb.setBackground(PANEL_BG);
        cb.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 14));
        return cb;
    }

    private static JTextField makeDarkField(String text, int cols) {
        JTextField tf = new JTextField(text, cols);
        tf.setBackground(new Color(40, 40, 40));
        tf.setForeground(Color.WHITE);
        tf.setCaretColor(Color.WHITE);
        tf.setFont(new Font(Font.SANS_SERIF, Font.PLAIN, 14));
        return tf;
    }

    private void addSliderRow(JPanel panel, String label, JSlider slider) {
        JPanel row = new JPanel(new BorderLayout(5, 0));
        row.setBackground(PANEL_BG);
        JLabel lbl = makeBoldLabel(label, 14);
        lbl.setPreferredSize(new Dimension(140, 26));
        JLabel val = new JLabel(String.valueOf(slider.getValue()));
        val.setForeground(new Color(0, 220, 0));
        val.setFont(new Font(Font.MONOSPACED, Font.BOLD, 15));
        val.setPreferredSize(new Dimension(50, 26));
        val.setHorizontalAlignment(SwingConstants.RIGHT);
        slider.addChangeListener(e -> val.setText(String.valueOf(slider.getValue())));
        row.add(lbl, BorderLayout.WEST);
        row.add(slider, BorderLayout.CENTER);
        row.add(val, BorderLayout.EAST);
        panel.add(row);
    }

    // =====================================================================
    // Alarm management
    // =====================================================================

    private void updateAlarms() {
        // Draeger alarms
        StringBuilder drAlarms = new StringBuilder();
        int drPriority = 0;
        int pp = slPeakPres.getValue();
        double mv = slTidalVol.getValue() * slRespRate.getValue() / 1000.0;
        int rr = slRespRate.getValue();
        int fio2 = slFio2.getValue();

        if (chkPawHigh.isSelected() || pp > 35) { drAlarms.append("*** PAW HIGH *** "); drPriority = Math.max(drPriority, 27); }
        if (chkApnea.isSelected() || rr < 4) { drAlarms.append("*** APNEA *** "); drPriority = Math.max(drPriority, 27); }
        if (chkO2High.isSelected() || fio2 > 60) { drAlarms.append("** %O2 HIGH ** "); drPriority = Math.max(drPriority, 15); }
        if (chkMinVolLow.isSelected() || mv < 3.0) { drAlarms.append("** MV LOW ** "); drPriority = Math.max(drPriority, 15); }

        final String drText = drAlarms.toString().trim();
        final int drPri = drPriority;
        SwingUtilities.invokeLater(() -> {
            drAlarmBar.setText(drText.isEmpty() ? " " : drText);
            updateSingleAlarmBar(drAlarmBar, (JPanel) drAlarmBar.getParent(), drPri, drText.isEmpty());
        });

        // Philips alarms
        StringBuilder phAlarms = new StringBuilder();
        int phPriority = 0;
        int hr = slHeartRate.getValue();
        int spo2 = slSpo2.getValue();
        int abpSys = slAbpSys.getValue();

        if (chkPhHrAlarm.isSelected() || hr > 120) { phAlarms.append("*** HR HIGH *** "); phPriority = Math.max(phPriority, 25); }
        if (hr < 50 && hr > 0) { phAlarms.append("*** HR LOW *** "); phPriority = Math.max(phPriority, 25); }
        if (chkPhSpo2Low.isSelected() || spo2 < 90) { phAlarms.append("** SpO2 LOW ** "); phPriority = Math.max(phPriority, 22); }
        if (chkPhAbpHigh.isSelected() || abpSys > 160) { phAlarms.append("** ABP HIGH ** "); phPriority = Math.max(phPriority, 15); }

        final String phText = phAlarms.toString().trim();
        final int phPri = phPriority;
        SwingUtilities.invokeLater(() -> {
            phAlarmBar.setText(phText.isEmpty() ? " " : phText);
            updateSingleAlarmBar(phAlarmBar, (JPanel) phAlarmBar.getParent(), phPri, phText.isEmpty());
        });

        // Sound
        if (!alarmMuted && (drPriority > 0 || phPriority > 0)) {
            if (alarmFlashState) {
                Toolkit.getDefaultToolkit().beep();
            }
        }
    }

    private void updateAlarmBars() {
        // Called on flash timer - just repaint
        if (drAlarmBar.getParent() != null) drAlarmBar.getParent().repaint();
        if (phAlarmBar.getParent() != null) phAlarmBar.getParent().repaint();
    }

    private void updateSingleAlarmBar(JLabel label, JPanel bar, int priority, boolean noAlarm) {
        if (bar == null) return;
        if (noAlarm) {
            bar.setBackground(Color.GRAY);
            label.setForeground(Color.WHITE);
        } else if (priority > 20) {
            bar.setBackground(alarmFlashState ? Color.RED : new Color(139, 0, 0));
            label.setForeground(Color.WHITE);
        } else {
            bar.setBackground(alarmFlashState ? Color.YELLOW : new Color(139, 139, 0));
            label.setForeground(Color.BLACK);
        }
    }

    // =====================================================================
    // Freeze / Mute / Fullscreen
    // =====================================================================

    private void toggleFreeze() {
        waveformFrozen = !waveformFrozen;
        String text = waveformFrozen ? "Resume" : "Freeze";
        drFreezeBtn.setText(text);
        phFreezeBtn.setText(text);
    }

    private void toggleMute() {
        alarmMuted = !alarmMuted;
        String icon = alarmMuted ? "\uD83D\uDD07" : "\uD83D\uDD0A";
        drMuteBtn.setText(icon);
        phMuteBtn.setText(icon);
    }

    private void toggleFullscreen() {
        GraphicsDevice gd = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice();
        if (!isFullscreen) {
            windowedBounds = getBounds();
            dispose();
            setUndecorated(true);
            gd.setFullScreenWindow(this);
            isFullscreen = true;
        } else {
            gd.setFullScreenWindow(null);
            dispose();
            setUndecorated(false);
            setBounds(windowedBounds);
            setVisible(true);
            isFullscreen = false;
        }
    }

    // =====================================================================
    // Screenshot
    // =====================================================================

    private void takeScreenshot() {
        WaveformPanel target = mainTabs.getSelectedIndex() == 0 ? drWavePanel : phWavePanel;
        int w = target.getWidth();
        int h = target.getHeight();
        if (w <= 0 || h <= 0) return;

        BufferedImage img = new BufferedImage(w, h, BufferedImage.TYPE_INT_RGB);
        target.paint(img.getGraphics());

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        String filename = "criticalinsights_screenshot_" + timestamp + ".png";
        File outFile = new File(System.getProperty("user.home"), filename);
        try {
            ImageIO.write(img, "png", outFile);
            log("[Screenshot] Saved: " + outFile.getAbsolutePath());

            // Flash effect
            Graphics g = target.getGraphics();
            if (g != null) {
                g.setColor(new Color(255, 255, 255, 180));
                g.fillRect(0, 0, w, h);
                g.dispose();
            }

            // Toast
            SwingUtilities.invokeLater(() -> {
                statusBar.setText("  Saved: " + filename);
                javax.swing.Timer toast = new javax.swing.Timer(3000, e -> {
                    statusBar.setText("  Press F11 for fullscreen | Space to freeze | M to mute");
                    ((javax.swing.Timer) e.getSource()).stop();
                });
                toast.setRepeats(false);
                toast.start();
            });
        } catch (IOException ex) {
            log("[Screenshot] Error: " + ex.getMessage());
        }
    }

    // =====================================================================
    // Scenario presets
    // =====================================================================

    private void applyDraegerScenario() {
        String scenario = (String) drScenarioCombo.getSelectedItem();
        if (scenario == null || scenario.startsWith("--")) return;

        scenarioTargets = new LinkedHashMap<>();
        switch (scenario) {
            case "Normal":
                scenarioTargets.put(slTidalVol, 450); scenarioTargets.put(slRespRate, 16);
                scenarioTargets.put(slPeakPres, 22); scenarioTargets.put(slPeep, 5);
                scenarioTargets.put(slFio2, 40); scenarioTargets.put(slCompliance, 45);
                scenarioTargets.put(slResistance, 12); scenarioTargets.put(slAirwayTemp, 34);
                break;
            case "ARDS":
                scenarioTargets.put(slTidalVol, 300); scenarioTargets.put(slRespRate, 28);
                scenarioTargets.put(slPeakPres, 35); scenarioTargets.put(slPeep, 14);
                scenarioTargets.put(slFio2, 80); scenarioTargets.put(slCompliance, 20);
                scenarioTargets.put(slResistance, 25); scenarioTargets.put(slAirwayTemp, 37);
                break;
            case "COPD":
                scenarioTargets.put(slTidalVol, 500); scenarioTargets.put(slRespRate, 12);
                scenarioTargets.put(slPeakPres, 25); scenarioTargets.put(slPeep, 3);
                scenarioTargets.put(slFio2, 28); scenarioTargets.put(slCompliance, 60);
                scenarioTargets.put(slResistance, 30); scenarioTargets.put(slAirwayTemp, 34);
                break;
            case "Weaning":
                scenarioTargets.put(slTidalVol, 400); scenarioTargets.put(slRespRate, 20);
                scenarioTargets.put(slPeakPres, 15); scenarioTargets.put(slPeep, 5);
                scenarioTargets.put(slFio2, 30); scenarioTargets.put(slCompliance, 50);
                scenarioTargets.put(slResistance, 15); scenarioTargets.put(slAirwayTemp, 35);
                break;
            case "Apnea":
                scenarioTargets.put(slTidalVol, 0); scenarioTargets.put(slRespRate, 0);
                scenarioTargets.put(slPeakPres, 0); scenarioTargets.put(slPeep, 5);
                scenarioTargets.put(slFio2, 40); scenarioTargets.put(slCompliance, 45);
                scenarioTargets.put(slResistance, 12); scenarioTargets.put(slAirwayTemp, 34);
                break;
        }
        startScenarioAnimation();
    }

    private void applyPhilipsScenario() {
        String scenario = (String) phScenarioCombo.getSelectedItem();
        if (scenario == null || scenario.startsWith("--")) return;

        scenarioTargets = new LinkedHashMap<>();
        switch (scenario) {
            case "Normal":
                scenarioTargets.put(slHeartRate, 72); scenarioTargets.put(slSpo2, 97);
                scenarioTargets.put(slPhRespRate, 16); scenarioTargets.put(slAbpSys, 120);
                scenarioTargets.put(slAbpDia, 80); scenarioTargets.put(slNbpSys, 118);
                scenarioTargets.put(slNbpDia, 76); scenarioTargets.put(slEtCo2, 38);
                scenarioTargets.put(slTemp, 370);
                break;
            case "Tachycardia":
                scenarioTargets.put(slHeartRate, 150); scenarioTargets.put(slSpo2, 94);
                scenarioTargets.put(slPhRespRate, 22); scenarioTargets.put(slAbpSys, 100);
                scenarioTargets.put(slAbpDia, 65); scenarioTargets.put(slNbpSys, 98);
                scenarioTargets.put(slNbpDia, 62); scenarioTargets.put(slEtCo2, 32);
                scenarioTargets.put(slTemp, 372);
                break;
            case "Bradycardia":
                scenarioTargets.put(slHeartRate, 42); scenarioTargets.put(slSpo2, 96);
                scenarioTargets.put(slPhRespRate, 14); scenarioTargets.put(slAbpSys, 90);
                scenarioTargets.put(slAbpDia, 60); scenarioTargets.put(slNbpSys, 88);
                scenarioTargets.put(slNbpDia, 58); scenarioTargets.put(slEtCo2, 40);
                scenarioTargets.put(slTemp, 368);
                break;
            case "Hypotension":
                scenarioTargets.put(slHeartRate, 110); scenarioTargets.put(slSpo2, 95);
                scenarioTargets.put(slPhRespRate, 20); scenarioTargets.put(slAbpSys, 75);
                scenarioTargets.put(slAbpDia, 45); scenarioTargets.put(slNbpSys, 72);
                scenarioTargets.put(slNbpDia, 42); scenarioTargets.put(slEtCo2, 30);
                scenarioTargets.put(slTemp, 365);
                break;
            case "Cardiac Arrest":
                scenarioTargets.put(slHeartRate, 0); scenarioTargets.put(slSpo2, 0);
                scenarioTargets.put(slPhRespRate, 0); scenarioTargets.put(slAbpSys, 0);
                scenarioTargets.put(slAbpDia, 0); scenarioTargets.put(slNbpSys, 0);
                scenarioTargets.put(slNbpDia, 0); scenarioTargets.put(slEtCo2, 5);
                scenarioTargets.put(slTemp, 350);
                break;
            case "Septic Shock":
                scenarioTargets.put(slHeartRate, 130); scenarioTargets.put(slSpo2, 88);
                scenarioTargets.put(slPhRespRate, 30); scenarioTargets.put(slAbpSys, 80);
                scenarioTargets.put(slAbpDia, 40); scenarioTargets.put(slNbpSys, 78);
                scenarioTargets.put(slNbpDia, 38); scenarioTargets.put(slEtCo2, 22);
                scenarioTargets.put(slTemp, 395);
                break;
        }
        startScenarioAnimation();
    }

    private void startScenarioAnimation() {
        if (scenarioAnimTimer != null) scenarioAnimTimer.stop();
        scenarioAnimSteps = 0;
        final int totalSteps = 66; // ~2 seconds at 30ms
        final Map<JSlider, Integer> startValues = new LinkedHashMap<>();
        for (JSlider sl : scenarioTargets.keySet()) {
            startValues.put(sl, sl.getValue());
        }

        scenarioAnimTimer = new javax.swing.Timer(30, e -> {
            scenarioAnimSteps++;
            double frac = Math.min(1.0, (double) scenarioAnimSteps / totalSteps);
            // Smooth ease-in-out
            double easeFrac = frac < 0.5 ? 2 * frac * frac : 1 - Math.pow(-2 * frac + 2, 2) / 2;

            for (Map.Entry<JSlider, Integer> entry : scenarioTargets.entrySet()) {
                JSlider sl = entry.getKey();
                int start = startValues.get(sl);
                int target = entry.getValue();
                int current = (int) Math.round(start + (target - start) * easeFrac);
                // Clamp to slider range
                current = Math.max(sl.getMinimum(), Math.min(sl.getMaximum(), current));
                sl.setValue(current);
            }

            if (scenarioAnimSteps >= totalSteps) {
                ((javax.swing.Timer) e.getSource()).stop();
            }
        });
        scenarioAnimTimer.start();
    }

    // =====================================================================
    // NBP Cycling
    // =====================================================================

    private void setupNbpCycling() {
        if (nbpCycleTimer != null) { nbpCycleTimer.stop(); nbpCycleTimer = null; }
        if (nbpInflateTimer != null) { nbpInflateTimer.stop(); nbpInflateTimer = null; }

        if (!chkNbpCycle.isSelected()) {
            lblNbpStatus.setText("NBP: --");
            return;
        }

        String sel = (String) nbpIntervalCombo.getSelectedItem();
        int minutes = 1;
        if (sel != null) {
            if (sel.startsWith("3")) minutes = 3;
            else if (sel.startsWith("5")) minutes = 5;
            else if (sel.startsWith("15")) minutes = 15;
        }
        int intervalMs = minutes * 60 * 1000;

        nbpCycleTimer = new javax.swing.Timer(intervalMs, e -> performNbpCycle());
        nbpCycleTimer.setInitialDelay(5000); // first cycle after 5 seconds
        nbpCycleTimer.start();

        // Trigger first cycle soon
        javax.swing.Timer firstCycle = new javax.swing.Timer(2000, e -> {
            performNbpCycle();
            ((javax.swing.Timer) e.getSource()).stop();
        });
        firstCycle.setRepeats(false);
        firstCycle.start();
    }

    private void performNbpCycle() {
        lblNbpStatus.setText("Inflating...");
        lblNbpStatus.setForeground(new Color(255, 200, 50));

        if (nbpInflateTimer != null) nbpInflateTimer.stop();
        nbpInflateTimer = new javax.swing.Timer(5000, e -> {
            // Vary NBP slightly
            int variation = noiseRng.nextInt(7) - 3;
            int newSys = Math.max(slNbpSys.getMinimum(), Math.min(slNbpSys.getMaximum(), slNbpSys.getValue() + variation));
            int newDia = Math.max(slNbpDia.getMinimum(), Math.min(slNbpDia.getMaximum(), slNbpDia.getValue() + noiseRng.nextInt(5) - 2));
            slNbpSys.setValue(newSys);
            slNbpDia.setValue(newDia);

            String time = LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
            lblNbpStatus.setText("NBP: " + newSys + "/" + newDia + " @ " + time);
            lblNbpStatus.setForeground(new Color(180, 180, 180));
            ((javax.swing.Timer) e.getSource()).stop();
        });
        nbpInflateTimer.setRepeats(false);
        nbpInflateTimer.start();
    }

    // =====================================================================
    // SpO2 Signal Quality
    // =====================================================================

    private void updateSpo2SignalQuality() {
        int spo2 = slSpo2.getValue();
        boolean noisy = chkPhNoise.isSelected();
        int quality = 10;
        if (spo2 < 90) quality -= 3;
        if (spo2 < 80) quality -= 3;
        if (noisy) quality -= 3;
        if (spo2 == 0) quality = 0;
        quality = Math.max(0, Math.min(10, quality));

        StringBuilder sb = new StringBuilder("SQ: ");
        for (int i = 0; i < 10; i++) {
            sb.append(i < quality ? "\u2588" : "\u2591");
        }
        lblSpo2SQ.setText(sb.toString());
        if (quality >= 7) lblSpo2SQ.setForeground(new Color(0, 200, 0));
        else if (quality >= 4) lblSpo2SQ.setForeground(new Color(200, 200, 0));
        else lblSpo2SQ.setForeground(new Color(200, 0, 0));
    }

    // =====================================================================
    // Trend updates
    // =====================================================================

    private void updateTrends() {
        spDrVt.addSample(slTidalVol.getValue());
        spDrRr.addSample(slRespRate.getValue());
        spDrPaw.addSample(slPeakPres.getValue());
        spDrPeep.addSample(slPeep.getValue());
        spDrFio2.addSample(slFio2.getValue());

        spPhHr.addSample(slHeartRate.getValue());
        spPhSpo2.addSample(slSpo2.getValue());
        spPhRr.addSample(slPhRespRate.getValue());
        spPhAbp.addSample(slAbpSys.getValue());
        spPhEtco2.addSample(slEtCo2.getValue());
        spPhTemp.addSample(slTemp.getValue() / 10.0f);

        // Repaint sparklines
        spDrVt.repaint(); spDrRr.repaint(); spDrPaw.repaint(); spDrPeep.repaint(); spDrFio2.repaint();
        spPhHr.repaint(); spPhSpo2.repaint(); spPhRr.repaint(); spPhAbp.repaint(); spPhEtco2.repaint(); spPhTemp.repaint();
    }

    // =====================================================================
    // Status bar
    // =====================================================================

    private void updateStatusBar() {
        long uptimeMs = System.currentTimeMillis() - startTime;
        long uptimeSec = uptimeMs / 1000;
        long h = uptimeSec / 3600;
        long m = (uptimeSec % 3600) / 60;
        long s = uptimeSec % 60;
        String uptime = String.format("%02d:%02d:%02d", h, m, s);

        String lastPoll = lastPollTimestamp > 0
            ? new java.text.SimpleDateFormat("HH:mm:ss").format(new java.util.Date(lastPollTimestamp))
            : "--:--:--";

        statusBar.setText(String.format("  Events/s: %.0f  |  Last poll: %s  |  Uptime: %s  |  Press F11 for fullscreen | Space to freeze | M to mute",
            eventsPerSecond, lastPoll, uptime));
    }

    // =====================================================================
    // Noise generation helpers
    // =====================================================================

    private float addNoise(float value, boolean enabled) {
        if (!enabled) return value;
        noiseBaselinePhase += 0.001;

        // Baseline wander
        float wander = (float)(0.02 * Math.sin(noiseBaselinePhase * 2 * Math.PI * 0.1));

        // High-frequency jitter
        float jitter = (float)(noiseRng.nextGaussian() * 0.02);

        // Occasional motion artifact (every ~10-20 seconds)
        float artifact = 0;
        if (noiseArtifactTimer > 0 && noiseArtifactTimer % (333 + noiseRng.nextInt(333)) == 0) {
            artifact = (float)(noiseRng.nextGaussian() * 0.15);
        }

        return value + wander + jitter + artifact;
    }

    // =====================================================================
    // Waveform generation -- Draeger (3 traces)
    // =====================================================================

    private void generateDrWaveformSamples() {
        int rr = slRespRate.getValue();
        int pp = slPeakPres.getValue();
        int peep = slPeep.getValue();
        int tv = slTidalVol.getValue();
        boolean noisy = chkNoise.isSelected();

        double dt = 0.03;
        double speedFactor = drWavePanel.getSweepSpeedFactor();
        int samplesPerTick = (int)(6 / speedFactor);
        if (samplesPerTick < 1) samplesPerTick = 1;
        double breathInterval = 60.0 / Math.max(rr, 1);

        synchronized (drWaveLock) {
            for (int s = 0; s < samplesPerTick; s++) {
                if (rr == 0 && tv == 0) {
                    // Apnea - flat lines
                    drWaveBuffers[0][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(0.1f, noisy);
                    drWaveBuffers[1][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(0.5f, noisy);
                    drWaveBuffers[2][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(0.05f, noisy);
                } else {
                    double breathFrac = (drPawPhase % breathInterval) / breathInterval;
                    drWaveBuffers[0][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(generatePawSample(breathFrac, pp, peep), noisy);
                    drWaveBuffers[1][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(generateFlowSample(breathFrac), noisy);
                    drWaveBuffers[2][drWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(generateVolumeSample(breathFrac, tv), noisy);
                }

                double sampleDt = dt / samplesPerTick;
                drPawPhase += sampleDt;
                drWaveWritePos++;
            }
        }
    }

    // =====================================================================
    // Waveform generation -- Philips (4 traces)
    // =====================================================================

    private void generatePhWaveformSamples() {
        int hr = slHeartRate.getValue();
        int rr = slPhRespRate.getValue();
        int abpSys = slAbpSys.getValue();
        int abpDia = slAbpDia.getValue();
        boolean noisy = chkPhNoise.isSelected();

        double dt = 0.03;
        double speedFactor = phWavePanel.getSweepSpeedFactor();
        int samplesPerTick = (int)(6 / speedFactor);
        if (samplesPerTick < 1) samplesPerTick = 1;
        double beatInterval = 60.0 / Math.max(hr, 1);
        double breathInterval = 60.0 / Math.max(rr, 1);

        String rhythm = (String) ecgRhythmCombo.getSelectedItem();

        synchronized (phWaveLock) {
            for (int s = 0; s < samplesPerTick; s++) {
                // ECG with rhythm support
                float ecgVal;
                if (hr == 0) {
                    ecgVal = 0.5f + (float)(waveRng.nextGaussian() * 0.002);
                } else {
                    ecgVal = generateEcgWithRhythm(rhythm, beatInterval);
                }
                phWaveBuffers[0][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(ecgVal, noisy);

                // Pleth
                float plethVal;
                if (hr == 0) {
                    plethVal = 0.1f + (float)(waveRng.nextGaussian() * 0.002);
                } else {
                    double plethBeatFrac = (phPlethPhaseLocal % beatInterval) / beatInterval;
                    plethVal = generatePlethSample(plethBeatFrac);
                }
                phWaveBuffers[1][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(plethVal, noisy);

                // ABP
                float abpVal;
                if (hr == 0 || abpSys == 0) {
                    abpVal = 0.05f + (float)(waveRng.nextGaussian() * 0.002);
                } else {
                    double abpBeatFrac = (phAbpPhaseLocal % beatInterval) / beatInterval;
                    abpVal = generateAbpSample(abpBeatFrac, abpSys, abpDia);
                }
                phWaveBuffers[2][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(abpVal, noisy);

                // Resp
                float respVal;
                if (rr == 0) {
                    respVal = 0.5f + (float)(waveRng.nextGaussian() * 0.002);
                } else {
                    double respFrac = (phRespPhaseLocal % breathInterval) / breathInterval;
                    if (respFrac < 0.45) respVal = (float)(0.5 + 0.4 * Math.sin(Math.PI * respFrac / 0.45 * 0.5));
                    else respVal = (float)(0.5 + 0.4 * Math.cos(Math.PI * (respFrac - 0.45) / 0.55 * 0.5));
                    respVal += (float)(waveRng.nextGaussian() * 0.005);
                }
                phWaveBuffers[3][phWaveWritePos % WAVEFORM_BUFFER_SIZE] = addNoise(respVal, noisy);

                double sampleDt = dt / samplesPerTick;
                phEcgPhaseLocal += sampleDt;
                phPlethPhaseLocal += sampleDt;
                phAbpPhaseLocal += sampleDt;
                phRespPhaseLocal += sampleDt;
                phWaveWritePos++;
            }
        }
    }

    // =====================================================================
    // ECG Arrhythmia Simulation
    // =====================================================================

    private float generateEcgWithRhythm(String rhythm, double beatInterval) {
        if (rhythm == null) rhythm = "Normal Sinus";

        switch (rhythm) {
            case "Normal Sinus":
            case "Sinus Tachycardia":
            case "Sinus Bradycardia": {
                double ecgBeatFrac = (phEcgPhaseLocal % beatInterval) / beatInterval;
                return generateEcgSample(ecgBeatFrac);
            }
            case "Atrial Fibrillation": {
                // Irregular R-R, no P wave, fibrillatory baseline
                double irregularInterval = beatInterval * (0.7 + waveRng.nextDouble() * 0.6);
                double ecgBeatFrac = (phEcgPhaseLocal % irregularInterval) / irregularInterval;
                double val = 0.0;
                // No P wave, just QRS and T
                if (ecgBeatFrac >= 0.22 && ecgBeatFrac < 0.25) {
                    val = -0.08 * Math.sin(Math.PI * (ecgBeatFrac - 0.22) / 0.03);
                } else if (ecgBeatFrac >= 0.25 && ecgBeatFrac < 0.30) {
                    val = 0.85 * Math.sin(Math.PI * (ecgBeatFrac - 0.25) / 0.05);
                } else if (ecgBeatFrac >= 0.30 && ecgBeatFrac < 0.33) {
                    val = -0.15 * Math.sin(Math.PI * (ecgBeatFrac - 0.30) / 0.03);
                } else if (ecgBeatFrac >= 0.40 && ecgBeatFrac < 0.55) {
                    val = 0.25 * Math.sin(Math.PI * (ecgBeatFrac - 0.40) / 0.15);
                }
                // Fibrillatory baseline
                val += 0.05 * Math.sin(phEcgPhaseLocal * 50) + waveRng.nextGaussian() * 0.015;
                return (float)(0.5 + val * 0.4);
            }
            case "Ventricular Tachycardia": {
                // Wide QRS, regular, no P waves
                double vtInterval = beatInterval * 0.5;
                double ecgBeatFrac = (phEcgPhaseLocal % vtInterval) / vtInterval;
                double val = 0.0;
                if (ecgBeatFrac < 0.4) {
                    val = 0.7 * Math.sin(Math.PI * ecgBeatFrac / 0.4);
                } else if (ecgBeatFrac < 0.6) {
                    val = -0.3 * Math.sin(Math.PI * (ecgBeatFrac - 0.4) / 0.2);
                }
                val += waveRng.nextGaussian() * 0.01;
                return (float)(0.5 + val * 0.4);
            }
            case "Ventricular Fibrillation": {
                // Chaotic waveform
                double val = 0.3 * Math.sin(phEcgPhaseLocal * 25 + waveRng.nextDouble() * 3)
                           + 0.2 * Math.sin(phEcgPhaseLocal * 37)
                           + 0.15 * Math.sin(phEcgPhaseLocal * 13)
                           + waveRng.nextGaussian() * 0.08;
                return (float)(0.5 + val * 0.35);
            }
            case "Asystole": {
                // Nearly flat line with tiny noise
                return (float)(0.5 + waveRng.nextGaussian() * 0.003);
            }
            default: {
                double ecgBeatFrac = (phEcgPhaseLocal % beatInterval) / beatInterval;
                return generateEcgSample(ecgBeatFrac);
            }
        }
    }

    // =====================================================================
    // Waveform shape generators
    // =====================================================================

    private float generateEcgSample(double beatFrac) {
        double val = 0.0;
        if (beatFrac >= 0.10 && beatFrac < 0.20) {
            val = 0.12 * Math.sin(Math.PI * (beatFrac - 0.10) / 0.10);
        } else if (beatFrac >= 0.22 && beatFrac < 0.25) {
            val = -0.08 * Math.sin(Math.PI * (beatFrac - 0.22) / 0.03);
        } else if (beatFrac >= 0.25 && beatFrac < 0.30) {
            val = 0.85 * Math.sin(Math.PI * (beatFrac - 0.25) / 0.05);
        } else if (beatFrac >= 0.30 && beatFrac < 0.33) {
            val = -0.15 * Math.sin(Math.PI * (beatFrac - 0.30) / 0.03);
        } else if (beatFrac >= 0.40 && beatFrac < 0.55) {
            val = 0.25 * Math.sin(Math.PI * (beatFrac - 0.40) / 0.15);
        }
        val += waveRng.nextGaussian() * 0.005;
        return (float)(0.5 + val * 0.4);
    }

    private float generatePlethSample(double beatFrac) {
        double val;
        if (beatFrac < 0.12) {
            val = beatFrac / 0.12;
        } else if (beatFrac < 0.22) {
            val = 1.0 - 0.25 * ((beatFrac - 0.12) / 0.10);
        } else if (beatFrac < 0.32) {
            val = 0.75 - 0.1 * Math.sin(Math.PI * (beatFrac - 0.22) / 0.10);
        } else {
            val = 0.65 * (1.0 - (beatFrac - 0.32) / 0.68);
        }
        val += waveRng.nextGaussian() * 0.003;
        return (float)(0.1 + val * 0.8);
    }

    private float generateAbpSample(double beatFrac, int sys, int dia) {
        double val;
        double abpRange = 250.0;
        if (beatFrac < 0.10) {
            val = dia + (sys - dia) * (beatFrac / 0.10);
        } else if (beatFrac < 0.20) {
            double d = (beatFrac - 0.10) / 0.10;
            double mean = (sys + 2.0 * dia) / 3.0;
            val = sys - (sys - mean) * 0.4 * d;
        } else if (beatFrac < 0.30) {
            double d = (beatFrac - 0.20) / 0.10;
            double mean = (sys + 2.0 * dia) / 3.0;
            double base = sys - (sys - mean) * 0.4;
            val = base - 5 + 10 * Math.sin(Math.PI * d);
        } else {
            double d = (beatFrac - 0.30) / 0.70;
            double mean = (sys + 2.0 * dia) / 3.0;
            double notchVal = sys - (sys - mean) * 0.4 + 5;
            val = notchVal - (notchVal - dia) * d;
        }
        val += waveRng.nextGaussian() * 0.3;
        return (float)(val / abpRange);
    }

    private float generatePawSample(double breathFrac, int peakPres, int peepVal) {
        double inspRatio = 1.0 / 3.0;
        boolean isInsp = breathFrac < inspRatio;
        double val;
        double range = 60.0;
        if (isInsp) {
            double ip = breathFrac / inspRatio;
            if (ip < 0.1) val = peepVal + (peakPres - peepVal) * (ip / 0.1);
            else if (ip < 0.3) val = peakPres;
            else {
                double plateau = peakPres * 0.82;
                val = plateau + (peakPres - plateau) * Math.exp(-(ip - 0.3) * 5);
            }
        } else {
            double ep = (breathFrac - inspRatio) / (1.0 - inspRatio);
            double plateau = peakPres * 0.82;
            val = peepVal + (plateau - peepVal) * Math.exp(-ep * 4);
        }
        val += waveRng.nextGaussian() * 0.2;
        return (float)((val + 5) / range);
    }

    private float generateFlowSample(double breathFrac) {
        double inspRatio = 1.0 / 3.0;
        boolean isInsp = breathFrac < inspRatio;
        double val;
        if (isInsp) {
            double ip = breathFrac / inspRatio;
            if (ip < 0.05) val = 0.7 * (ip / 0.05);
            else if (ip < 0.95) val = 0.7;
            else val = 0.7 * (1.0 - (ip - 0.95) / 0.05);
        } else {
            double ep = (breathFrac - inspRatio) / (1.0 - inspRatio);
            val = -0.5 * Math.exp(-ep * 3);
        }
        val += waveRng.nextGaussian() * 0.005;
        return (float)(0.5 + val * 0.45);
    }

    private float generateVolumeSample(double breathFrac, int tidalVol) {
        double inspRatio = 1.0 / 3.0;
        boolean isInsp = breathFrac < inspRatio;
        double val;
        if (isInsp) {
            val = breathFrac / inspRatio;
        } else {
            double ep = (breathFrac - inspRatio) / (1.0 - inspRatio);
            val = Math.exp(-ep * 4);
        }
        val += waveRng.nextGaussian() * 0.003;
        return (float)(0.05 + val * 0.85);
    }

    // =====================================================================
    // Update numeric displays
    // =====================================================================

    private void updateDrNumerics() {
        int tv = slTidalVol.getValue();
        int rr = slRespRate.getValue();
        double mv = tv * rr / 1000.0;
        lblDrVt.setText(String.valueOf(tv));
        lblDrRr.setText(String.valueOf(rr));
        lblDrPaw.setText(String.valueOf(slPeakPres.getValue()));
        lblDrPeep.setText(String.valueOf(slPeep.getValue()));
        lblDrFio2.setText(String.valueOf(slFio2.getValue()));
        lblDrMv.setText(String.format("%.1f", mv));
        lblDrCompl.setText(String.valueOf(slCompliance.getValue()));
        lblDrRes.setText(String.valueOf(slResistance.getValue()));
        lblDrTemp.setText(String.valueOf(slAirwayTemp.getValue()));
    }

    private void updatePhNumerics() {
        int hr = slHeartRate.getValue();
        int spo2 = slSpo2.getValue();
        int abpS = slAbpSys.getValue();
        int abpD = slAbpDia.getValue();
        int abpM = abpS > 0 ? (abpS + 2 * abpD) / 3 : 0;
        int nbpS = slNbpSys.getValue();
        int nbpD = slNbpDia.getValue();
        int nbpM = nbpS > 0 ? (nbpS + 2 * nbpD) / 3 : 0;
        int papS = slPapSys.getValue();
        int papD = slPapDia.getValue();
        int papM = (papS + 2 * papD) / 3;
        int rr = slPhRespRate.getValue();
        int etco2 = slEtCo2.getValue();
        double temp = slTemp.getValue() / 10.0;

        lblPhHr.setText(String.valueOf(hr));
        lblPhSpo2.setText(String.valueOf(spo2));
        lblPhPr.setText(String.valueOf(hr > 0 ? hr + 1 : 0));
        lblPhRr.setText(String.valueOf(rr));
        lblPhAbp.setText(abpS + "/" + abpD);
        lblPhAbpM.setText("(" + abpM + ")");
        lblPhNbp.setText(nbpS + "/" + nbpD);
        lblPhNbpM.setText("(" + nbpM + ")");
        lblPhEtco2.setText(String.valueOf(etco2));
        lblPhTemp.setText(String.format("%.1f", temp));
        lblPhCvp.setText(String.valueOf(slCvp.getValue()));
        lblPhPap.setText(papS + "/" + papD);
        lblPhPapM.setText("(" + papM + ")");
    }

    // =====================================================================
    // Logging
    // =====================================================================

    private void log(String msg) {
        SwingUtilities.invokeLater(() -> {
            logArea.append(msg + "\n");
            if (logArea.getDocument().getLength() > 50000) {
                try {
                    logArea.getDocument().remove(0, 10000);
                } catch (BadLocationException ignored) {}
            }
            logArea.setCaretPosition(logArea.getDocument().getLength());
        });
    }

    private void updateTransportFields() {
        boolean draegerSerial = "RS232".equals(draegerTransportCombo.getSelectedItem());
        if (draegerSerial) {
            resolveSerialField(draegerSerialField);
        }
        draegerPortField.setEnabled(!draegerSerial);
        draegerSerialField.setEnabled(draegerSerial);

        boolean philipsSerial = "MIB-RS232".equals(philipsTransportCombo.getSelectedItem());
        if (philipsSerial) {
            resolveSerialField(philipsSerialField);
        }
        philipsPortField.setEnabled(!philipsSerial);
        philipsSerialField.setEnabled(philipsSerial);
        chkBroadcast.setEnabled(!philipsSerial);
    }

    private void resolveSerialField(JTextField field) {
        String path = field.getText().trim();
        if (!path.startsWith("/tmp/")) return;
        try {
            File file = new File(path);
            if (file.exists()) {
                String resolved = file.getCanonicalPath();
                if (!resolved.equals(path) && resolved.startsWith("/dev/")) {
                    field.setText(resolved);
                }
            }
        } catch (IOException ignored) {
        }
    }

    private void writeSerialControlFiles() {
        if (draegerRunning.get() && "RS232".equals(draegerTransportCombo.getSelectedItem())) {
            Properties p = new Properties();
            p.setProperty("tidalVolume", String.valueOf(slTidalVol.getValue()));
            p.setProperty("respRate", String.valueOf(slRespRate.getValue()));
            p.setProperty("peakPressure", String.valueOf(slPeakPres.getValue()));
            p.setProperty("peep", String.valueOf(slPeep.getValue()));
            p.setProperty("fio2", String.valueOf(slFio2.getValue()));
            p.setProperty("compliance", String.valueOf(slCompliance.getValue()));
            p.setProperty("resistance", String.valueOf(slResistance.getValue()));
            p.setProperty("airwayTemp", String.valueOf(slAirwayTemp.getValue()));
            storeProperties(p, draegerControlFile);
        }
        if (philipsRunning.get() && "MIB-RS232".equals(philipsTransportCombo.getSelectedItem())) {
            Properties p = new Properties();
            int abpSys = slAbpSys.getValue();
            int abpDia = slAbpDia.getValue();
            p.setProperty("heartRate", String.valueOf(slHeartRate.getValue()));
            p.setProperty("spo2", String.valueOf(slSpo2.getValue()));
            p.setProperty("pulseRate", String.valueOf(slHeartRate.getValue()));
            p.setProperty("respRate", String.valueOf(slPhRespRate.getValue()));
            p.setProperty("abpSys", String.valueOf(abpSys));
            p.setProperty("abpDia", String.valueOf(abpDia));
            p.setProperty("nbpSys", String.valueOf(slNbpSys.getValue()));
            p.setProperty("nbpDia", String.valueOf(slNbpDia.getValue()));
            p.setProperty("etCo2", String.valueOf(slEtCo2.getValue()));
            p.setProperty("temp", String.valueOf(slTemp.getValue() / 10.0));
            storeProperties(p, philipsControlFile);
        }
    }

    private void storeProperties(Properties p, File file) {
        try (FileOutputStream out = new FileOutputStream(file)) {
            p.store(out, "Critical Insights GUI serial simulator controls");
        } catch (IOException e) {
            log("[GUI] Could not write control file " + file + ": " + e.getMessage());
        }
    }

    private void startProcessLogReader(String prefix, Process process) {
        Thread t = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()))) {
                String line;
                while ((line = reader.readLine()) != null) {
                    log("[" + prefix + "] " + line);
                }
            } catch (IOException e) {
                log("[" + prefix + "] log reader stopped: " + e.getMessage());
            }
        }, prefix.toLowerCase(Locale.ROOT).replace(' ', '-') + "-log");
        t.setDaemon(true);
        t.start();
    }

    private void stopProcess(Process process) {
        if (process == null) return;
        process.destroy();
        try {
            if (!process.waitFor(1500, TimeUnit.MILLISECONDS)) {
                process.destroyForcibly();
                process.waitFor(1500, TimeUnit.MILLISECONDS);
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
        }
    }

    private String javaCommand() {
        String javaHome = System.getenv("JAVA_HOME");
        if (javaHome != null && !javaHome.trim().isEmpty()) {
            return javaHome + File.separator + "bin" + File.separator + "java";
        }
        return "java";
    }

    private String simulatorClasspath() {
        return System.getProperty("java.class.path", ".");
    }

    // =====================================================================
    // Draeger Simulator
    // =====================================================================

    private void startDraeger() {
        if (draegerRunning.get()) return;
        String transport = (String) draegerTransportCombo.getSelectedItem();
        if ("RS232".equals(transport)) {
            startDraegerSerial();
            return;
        }
        int port;
        try {
            port = Integer.parseInt(draegerPortField.getText().trim());
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Invalid port number");
            return;
        }

        draegerRunning.set(true);
        btnDraegerStart.setEnabled(false);
        btnDraegerStop.setEnabled(true);
        draegerStatus.setText("Running");
        draegerStatus.setForeground(new Color(0, 200, 0));

        String model = (String) modelCombo.getSelectedItem();
        log("[Draeger] Starting on TCP port " + port + " model=" + model);

        draegerThread = new Thread(() -> runDraegerServer(port), "draeger-sim-ci");
        draegerThread.setDaemon(true);
        draegerThread.start();
    }

    private void startDraegerSerial() {
        String serialPath = draegerSerialField.getText().trim();
        if (serialPath.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Enter a Draeger serial device path");
            return;
        }
        String model = (String) modelCombo.getSelectedItem();
        draegerRunning.set(true);
        try {
            writeSerialControlFiles();
            ProcessBuilder pb = new ProcessBuilder(
                    javaCommand(), "-cp", simulatorClasspath(), "MedibusSimulatorV2",
                    "--serial", serialPath,
                    "--model", model,
                    "--control-file", draegerControlFile.getAbsolutePath());
            pb.redirectErrorStream(true);
            draegerSerialProcess = pb.start();
            startProcessLogReader("Draeger RS232", draegerSerialProcess);
        } catch (IOException e) {
            draegerRunning.set(false);
            JOptionPane.showMessageDialog(this, "Could not start Draeger RS232 simulator: " + e.getMessage());
            return;
        }

        btnDraegerStart.setEnabled(false);
        btnDraegerStop.setEnabled(true);
        draegerStatus.setText("Running");
        draegerStatus.setForeground(new Color(0, 200, 0));
        draegerConnStatus.setText("Serial: " + serialPath);
        log("[Draeger] Starting RS232 simulator on " + serialPath + " model=" + model);
    }

    private void stopDraeger() {
        draegerRunning.set(false);
        stopProcess(draegerSerialProcess);
        draegerSerialProcess = null;
        try {
            if (draegerServerSocket != null && !draegerServerSocket.isClosed()) {
                draegerServerSocket.close();
            }
        } catch (IOException ignored) {}
        closeDraegerClient();
        btnDraegerStart.setEnabled(true);
        btnDraegerStop.setEnabled(false);
        draegerStatus.setText("Stopped");
        draegerStatus.setForeground(Color.GRAY);
        draegerConnStatus.setText("No connection");
        drRealtimeEnabled = false;
        drRealtimeConfiguredCodes.clear();
        drConfiguredDataCodes = null;
        log("[Draeger] Stopped");
    }

    private void runDraegerServer(int port) {
        try {
            draegerServerSocket = new ServerSocket(port);
            draegerServerSocket.setSoTimeout(1000);
            log("[Draeger] Listening on port " + port);
            while (draegerRunning.get()) {
                try {
                    Socket client = draegerServerSocket.accept();
                    replaceDraegerClient(client);
                } catch (SocketTimeoutException ignored) {
                }
            }
        } catch (IOException e) {
            if (draegerRunning.get()) log("[Draeger] Error: " + e.getMessage());
        }
    }

    private void replaceDraegerClient(Socket client) {
        closeDraegerClient();
        draegerClientSocket = client;
        SwingUtilities.invokeLater(() ->
            draegerConnStatus.setText("Connected: " + client.getRemoteSocketAddress()));
        log("[Draeger] Gateway connected from " + client.getRemoteSocketAddress());
        drCommunicationInitialized = false;
        drRealtimeEnabled = false;
        drRealtimeConfiguredCodes.clear();
        drConfiguredDataCodes = null;
        drWaveformSampleIndex = 0;

        Thread clientThread = new Thread(() -> {
            handleDraegerClient(client);
            if (draegerClientSocket == client) {
                draegerClientSocket = null;
                SwingUtilities.invokeLater(() -> draegerConnStatus.setText("Disconnected"));
            }
        }, "draeger-sim-client");
        clientThread.setDaemon(true);
        clientThread.start();
    }

    private void closeDraegerClient() {
        Socket client = draegerClientSocket;
        draegerClientSocket = null;
        if (client != null) {
            try { client.close(); } catch (IOException ignored) {}
        }
    }

    private void handleDraegerClient(Socket client) {
        try {
            InputStream in = client.getInputStream();
            OutputStream out = client.getOutputStream();
            byte[] buf = new byte[512];
            int pos = 0;
            boolean inFrame = false;

            while (draegerRunning.get() && !client.isClosed()) {
                int b = in.read();
                if (b == -1) break;
                if (b == DC1) {
                    drTransmissionSuspended = true;
                    log("[Draeger] <- DC1 suspend");
                    continue;
                }
                if (b == DC3) {
                    drTransmissionSuspended = false;
                    log("[Draeger] <- DC3 resume");
                    continue;
                }

                if (b == ESC) {
                    inFrame = true;
                    pos = 0;
                    buf[pos++] = (byte) b;
                } else if (b == CR && inFrame) {
                    inFrame = false;
                    if (pos >= 4) handleDraegerCommand(buf, pos, out);
                    pos = 0;
                } else if (inFrame && pos < buf.length) {
                    buf[pos++] = (byte) b;
                }
            }
        } catch (IOException e) {
            if (draegerRunning.get()) log("[Draeger] Client error: " + e.getMessage());
        }
    }

    private void handleDraegerCommand(byte[] buf, int len, OutputStream out) throws IOException {
        int cmd = buf[1] & 0xFF;
        eventCount.incrementAndGet();
        lastPollTimestamp = System.currentTimeMillis();

        int receivedCs = asciiHexDecode(buf[len - 2], buf[len - 1]);
        int calcCs = 0;
        for (int i = 0; i < len - 2; i++) calcCs += buf[i] & 0xFF;
        calcCs &= 0xFF;
        if (receivedCs != calcCs) {
            log("[Draeger] BAD CHECKSUM cmd=0x" + hexByte(cmd));
            sendDraegerNak(out);
            return;
        }

        byte[] argument = null;
        if (len > 4) {
            argument = new byte[len - 4];
            System.arraycopy(buf, 2, argument, 0, argument.length);
        }

        String cmdName = draegerCmdName(cmd);
        log("[Draeger] <- " + cmdName);

        if (!drTransmissionSuspended && drRealtimeEnabled && chkDrWaveforms.isSelected()) {
            sendDraegerRealtimeBurst(out);
        }

        switch (cmd) {
            case CMD_ICC:
                drCommunicationInitialized = true;
                drConfiguredDataCodes = null;
                drRealtimeEnabled = false;
                drRealtimeConfiguredCodes.clear();
                sendDraegerControlResponse(cmd, out);
                log("[Draeger] -> ICC acknowledged");
                break;
            case CMD_NOP:
            case CMD_STOP:
                if (cmd == CMD_STOP) {
                    drCommunicationInitialized = false;
                    drRealtimeEnabled = false;
                    drRealtimeConfiguredCodes.clear();
                }
                sendDraegerControlResponse(cmd, out);
                break;
            case CMD_REQ_DEVICE_ID:
                sendDraegerDeviceId(out);
                break;
            case CMD_REQ_DATETIME:
                sendDraegerDateTime(out);
                break;
            case CMD_REQ_DATA_CP1:
                sendDraegerDataCP1(out);
                break;
            case CMD_REQ_DATA_CP2:
                sendDraegerDataCP2(out);
                break;
            case CMD_REQ_ALARMS_CP1:
            case CMD_REQ_ALARMS_CP3:
            case CMD_REQ_ALARMS_CP2:
                sendDraegerAlarms(cmd, out);
                break;
            case CMD_REQ_LOW_ALRM_CP1:
            case CMD_REQ_HI_ALRM_CP1:
                sendDraegerAlarmLimits(cmd, out);
                break;
            case CMD_REQ_SETTINGS:
                sendDraegerSettings(out);
                break;
            case CMD_REQ_TEXT_MSG:
                sendDraegerTextMessages(out);
                break;
            case CMD_CONFIGURE_RESP:
                handleDraegerConfigureDataResponse(argument, out);
                break;
            case CMD_REQ_RT_CONFIG:
                sendDraegerRealtimeConfig(out);
                break;
            case CMD_CONFIG_RT:
                handleDraegerConfigureRealtime(argument, out);
                break;
            case CMD_REQ_TREND_STATUS:
                sendDraegerTrendStatus(out);
                break;
            case CMD_REQ_TREND_DATA:
                sendDraegerTrendData(argument, out);
                break;
            default:
                sendDraegerControlResponse(cmd, out);
                break;
        }
    }

    private void sendDraegerControlResponse(int cmd, OutputStream out) throws IOException {
        if (drTransmissionSuspended) return;
        int cs = SOH + cmd;
        out.write(SOH); out.write(cmd);
        writeAsciiHexChecksum(out, cs);
        out.write(CR); out.flush();
    }

    private void sendDraegerNak(OutputStream out) throws IOException {
        if (drTransmissionSuspended) return;
        int cs = SOH + NAK;
        out.write(SOH); out.write(NAK);
        writeAsciiHexChecksum(out, cs);
        out.write(CR); out.flush();
    }

    private void sendDraegerDataResponse(int cmd, byte[] data, OutputStream out) throws IOException {
        if (drTransmissionSuspended) return;
        int cs = SOH + cmd;
        for (byte b : data) cs += b & 0xFF;
        out.write(SOH); out.write(cmd); out.write(data);
        writeAsciiHexChecksum(out, cs);
        out.write(CR); out.flush();
    }

    private void sendDraegerDeviceId(OutputStream out) throws IOException {
        String model = (String) modelCombo.getSelectedItem();
        String[][] models = {
            {"evita","8210","Evita","01.00:03.00"}, {"evita2","8200","Evita 2","01.00:03.00"},
            {"evita4","8214","Evita 4","02.00:03.00"}, {"v500","8410","Evita V500","03.20:06.00"},
            {"savina","8310","Savina","01.00:03.00"}, {"fabius","8088","Fabius GS","02.02:04.00"}
        };
        String id = "8210", name = "Evita", rev = "01.00:03.00";
        for (String[] m : models) {
            if (m[0].equals(model)) { id = m[1]; name = m[2]; rev = m[3]; break; }
        }
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        d.write(padTo(id, 4).getBytes());
        d.write(0x27); d.write(name.getBytes()); d.write(0x27);
        d.write(padTo(rev, 11).getBytes());
        sendDraegerDataResponse(CMD_REQ_DEVICE_ID, d.toByteArray(), out);
        log("[Draeger] -> DeviceID: " + id + " '" + name + "' " + rev);
    }

    private void sendDraegerDateTime(OutputStream out) throws IOException {
        LocalDateTime now = LocalDateTime.now();
        String[] m = {"JAN","FEB","MAR","APR","MAI","JUN","JUL","AUG","SEP","OKT","NOV","DEZ"};
        String dt = String.format("%02d:%02d:%02d%02d-%s-%02d",
                now.getHour(), now.getMinute(), now.getSecond(),
                now.getDayOfMonth(), m[now.getMonthValue()-1], now.getYear()%100);
        sendDraegerDataResponse(CMD_REQ_DATETIME, dt.getBytes(), out);
    }

    private void sendDraegerDataCP1(OutputStream out) throws IOException {
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        int tv = slTidalVol.getValue();
        int rr = slRespRate.getValue();
        int pp = slPeakPres.getValue();
        int peep = slPeep.getValue();
        int fio2 = slFio2.getValue();
        int comp = slCompliance.getValue();
        int res = slResistance.getValue();
        int at = slAirwayTemp.getValue();
        double mvol = tv * rr / 1000.0;

        drMaybeAddRecord(d, 0x07, comp);
        drMaybeAddRecord(d, 0x08, res);
        drMaybeAddRecord(d, 0x73, (int)(pp * 0.55));
        drMaybeAddRecord(d, 0x78, peep);
        drMaybeAddRecord(d, 0x7D, pp);
        drMaybeAddRecord(d, 0x82, tv);
        drMaybeAddRecord(d, 0xB5, rr);
        drMaybeAddRecord(d, 0xB9, (int)(mvol * 10));
        drMaybeAddRecord(d, 0xC1, at);
        drMaybeAddRecord(d, 0xF0, fio2);

        sendDraegerDataResponse(CMD_REQ_DATA_CP1, d.toByteArray(), out);
        log("[Draeger] -> CP1: VT=" + tv + " RR=" + rr + " Paw=" + pp +
            " PEEP=" + peep + " FiO2=" + fio2);
    }

    private void drMaybeAddRecord(ByteArrayOutputStream d, int code, int value) throws IOException {
        if (drConfiguredDataCodes == null || drConfiguredDataCodes.contains(code)) {
            addDrRecord(d, code, value);
        }
    }

    private void sendDraegerDataCP2(OutputStream out) throws IOException {
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        drMaybeAddRecord(d, 0xD6, slRespRate.getValue());
        sendDraegerDataResponse(CMD_REQ_DATA_CP2, d.toByteArray(), out);
    }

    private void sendDraegerAlarms(int cmd, OutputStream out) throws IOException {
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        int pp = slPeakPres.getValue();
        double mv = slTidalVol.getValue() * slRespRate.getValue() / 1000.0;
        int rr = slRespRate.getValue();
        int fio2 = slFio2.getValue();

        if (cmd == CMD_REQ_ALARMS_CP1 || cmd == CMD_REQ_ALARMS_CP3) {
            if (chkPawHigh.isSelected())   addAlarmRecord(d, 27, 0x10, "PAW HIGH    ");
            if (chkO2High.isSelected())    addAlarmRecord(d, 23, 0x37, "% O2 HIGH   ");
            if (chkMinVolLow.isSelected()) addAlarmRecord(d, 26, 0x19, "MIN VOL LOW ");
            if (chkApnea.isSelected())     addAlarmRecord(d, 27, 0x98, "APNEA EVITA ");

            if (pp > 35 && !chkPawHigh.isSelected())
                addAlarmRecord(d, 2, 0x10, "PAW HIGH    ");
            if (mv < 3.0 && !chkMinVolLow.isSelected())
                addAlarmRecord(d, 1, 0x0C, "MIN VOL LOW ");
            if (rr < 4 && !chkApnea.isSelected())
                addAlarmRecord(d, 1, 0x1A, "APNEA       ");
            if (fio2 > 60 && !chkO2High.isSelected())
                addAlarmRecord(d, 3, 0x37, "% O2 HIGH   ");
        }
        sendDraegerDataResponse(cmd, d.toByteArray(), out);
        if (d.size() > 0) log("[Draeger] -> " + (d.size()/15) + " alarm(s) active");
    }

    private void sendDraegerAlarmLimits(int cmd, OutputStream out) throws IOException {
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        if (cmd == CMD_REQ_HI_ALRM_CP1) {
            addDrRecord(d, 0x7D, 35);
            addDrRecord(d, 0xB9, 150);
            addDrRecord(d, 0xF0, 100);
        } else {
            addDrRecord(d, 0xB9, 30);
            addDrRecord(d, 0xF0, 18);
        }
        sendDraegerDataResponse(cmd, d.toByteArray(), out);
    }

    private void sendDraegerSettings(OutputStream out) throws IOException {
        int fio2 = slFio2.getValue();
        int tv = slTidalVol.getValue();
        int peep = slPeep.getValue();

        ByteArrayOutputStream d = new ByteArrayOutputStream();
        addDrSettingRecord(d, 0x01, fio2, 0);
        addDrSettingRecord(d, 0x02, 400, 1);
        addDrSettingRecord(d, 0x04, tv, 3);
        addDrSettingRecord(d, 0x07, 10, 1);
        addDrSettingRecord(d, 0x08, 20, 1);
        addDrSettingRecord(d, 0x09, 120, 1);
        addDrSettingRecord(d, 0x0B, peep * 10, 1);
        addDrSettingRecord(d, 0x13, 400, 1);

        sendDraegerDataResponse(CMD_REQ_SETTINGS, d.toByteArray(), out);
        log("[Draeger] -> Settings: O2=" + fio2 + "% VT=" + tv + "mL PEEP=" + peep + " Pmax=40");
    }

    private void addDrSettingRecord(ByteArrayOutputStream d, int code, int value, int decimals) throws IOException {
        d.write(asciiHexHi(code));
        d.write(asciiHexLo(code));
        String valStr;
        if (decimals > 0) {
            StringBuilder fmt = new StringBuilder();
            int intPart = value;
            for (int i = 0; i < decimals; i++) {
                fmt.insert(0, (char)('0' + (intPart % 10)));
                intPart /= 10;
            }
            String intStr = String.valueOf(intPart);
            valStr = intStr + "." + fmt.toString();
        } else {
            valStr = String.valueOf(value);
        }
        while (valStr.length() < 5) valStr = " " + valStr;
        if (valStr.length() > 5) valStr = valStr.substring(valStr.length() - 5);
        d.write(valStr.getBytes());
    }

    private void sendDraegerTextMessages(OutputStream out) throws IOException {
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        String mode = (String) ventModeCombo.getSelectedItem();
        String text = "Mode " + mode;
        d.write(asciiHexHi(0x06));
        d.write(asciiHexLo(0x06));
        int len = Math.min(text.length(), 32);
        d.write(0x30 + len);
        d.write(text.substring(0, len).getBytes());
        d.write(ETX);

        sendDraegerDataResponse(CMD_REQ_TEXT_MSG, d.toByteArray(), out);
        log("[Draeger] -> Text: Mode " + mode);
    }

    private void handleDraegerConfigureDataResponse(byte[] argument, OutputStream out) throws IOException {
        if (argument != null && argument.length >= 2) {
            drConfiguredDataCodes = new LinkedHashSet<>();
            for (int i = 0; i + 1 < argument.length; i += 2) {
                int code = asciiHexDecode(argument[i], argument[i + 1]);
                drConfiguredDataCodes.add(code);
            }
            log("[Draeger] -> Configured " + drConfiguredDataCodes.size() + " data codes");
        } else {
            drConfiguredDataCodes = null;
            log("[Draeger] -> Reset to all data codes");
        }
        sendDraegerControlResponse(CMD_CONFIGURE_RESP, out);
    }

    private void sendDraegerRealtimeConfig(OutputStream out) throws IOException {
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        int[] codes = { RT_CODE_AIRWAY_PRESSURE, RT_CODE_FLOW, RT_CODE_VOLUME, RT_CODE_EXP_VOLUME };
        for (int code : codes) {
            addDraegerRealtimeConfigRecord(d, code);
        }
        sendDraegerDataResponse(CMD_REQ_RT_CONFIG, d.toByteArray(), out);
        log("[Draeger] -> RT config: " + (drRealtimeEnabled ? drRealtimeConfiguredCodes.size() + " codes" : "disabled"));
    }

    private void addDraegerRealtimeConfigRecord(ByteArrayOutputStream d, int code) throws IOException {
        int min = 0;
        int max = 511;
        int maxbin = 511;
        switch (code) {
            case RT_CODE_AIRWAY_PRESSURE:
                min = -10;
                max = 60;
                break;
            case RT_CODE_FLOW:
                min = -60;
                max = 60;
                break;
            case RT_CODE_VOLUME:
            case RT_CODE_EXP_VOLUME:
                min = 0;
                max = 800;
                break;
            default:
                break;
        }
        d.write(asciiHexHi(code));
        d.write(asciiHexLo(code));
        d.write(String.format("%08d", Integer.valueOf(10)).getBytes());
        d.write(String.format("%5d", Integer.valueOf(min)).getBytes());
        d.write(String.format("%5d", Integer.valueOf(max)).getBytes());
        d.write(String.format("%03X", Integer.valueOf(maxbin)).getBytes());
    }

    private void handleDraegerConfigureRealtime(byte[] argument, OutputStream out) throws IOException {
        if (argument != null && argument.length >= 2) {
            drRealtimeConfiguredCodes.clear();
            for (int i = 0; i + 3 < argument.length; i += 4) {
                int code = asciiHexDecode(argument[i], argument[i + 1]);
                drRealtimeConfiguredCodes.add(code);
            }
            if (drRealtimeConfiguredCodes.isEmpty() && argument.length >= 2) {
                int code = asciiHexDecode(argument[0], argument[1]);
                drRealtimeConfiguredCodes.add(code);
            }
            drRealtimeEnabled = !drRealtimeConfiguredCodes.isEmpty();
            drWaveformSampleIndex = 0;
            log("[Draeger] -> Realtime enabled for " + drRealtimeConfiguredCodes.size() + " codes");
        } else {
            drRealtimeEnabled = false;
            drRealtimeConfiguredCodes.clear();
            log("[Draeger] -> Realtime disabled");
        }
        sendDraegerControlResponse(CMD_CONFIG_RT, out);
    }

    private static final int DR_TREND_VT = 0x82;
    private static final int DR_TREND_RR = 0xD6;
    private static final int DR_TREND_PAW = 0x7D;
    private static final int DR_TREND_PEEP = 0x78;
    private static final int DR_TREND_FIO2 = 0xF0;
    private static final int DR_TREND_MV = 0xB9;

    private void sendDraegerTrendStatus(OutputStream out) throws IOException {
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        int[] codes = { DR_TREND_VT, DR_TREND_RR, DR_TREND_PAW, DR_TREND_PEEP, DR_TREND_FIO2, DR_TREND_MV };
        long begin = draegerMedibusTimestamp(System.currentTimeMillis());
        for (int code : codes) {
            d.write('2');
            d.write('4');
            d.write(asciiHexHi(code));
            d.write(asciiHexLo(code));
            d.write("000001".getBytes());
            d.write(String.format("%08X", Long.valueOf(begin)).getBytes());
        }
        sendDraegerDataResponse(CMD_REQ_TREND_STATUS, d.toByteArray(), out);
    }

    private void sendDraegerTrendData(byte[] argument, OutputStream out) throws IOException {
        int codepage = 0x24;
        int code = DR_TREND_RR;
        if (argument != null && argument.length >= 4) {
            codepage = asciiHexDecode(argument[0], argument[1]);
            code = asciiHexDecode(argument[2], argument[3]);
        }
        ByteArrayOutputStream d = new ByteArrayOutputStream();
        d.write(asciiHexHi(codepage));
        d.write(asciiHexLo(codepage));
        d.write(asciiHexHi(code));
        d.write(asciiHexLo(code));
        d.write('0');
        d.write('1');
        d.write(formatDraegerTrendValue(code).getBytes());
        d.write(String.format("%08X", Long.valueOf(draegerMedibusTimestamp(System.currentTimeMillis()))).getBytes());
        sendDraegerDataResponse(CMD_REQ_TREND_DATA, d.toByteArray(), out);
    }

    private String formatDraegerTrendValue(int code) {
        switch (code) {
            case DR_TREND_VT:
                return String.format("%4d", Integer.valueOf(slTidalVol.getValue()));
            case DR_TREND_PAW:
                return String.format("%4d", Integer.valueOf(slPeakPres.getValue()));
            case DR_TREND_PEEP:
                return String.format("%4d", Integer.valueOf(slPeep.getValue()));
            case DR_TREND_FIO2:
                return String.format("%4d", Integer.valueOf(slFio2.getValue()));
            case DR_TREND_MV:
                return String.format("%4.1f", Double.valueOf(slTidalVol.getValue() * slRespRate.getValue() / 1000.0));
            case DR_TREND_RR:
            default:
                return String.format("%4d", Integer.valueOf(slRespRate.getValue()));
        }
    }

    private long draegerMedibusTimestamp(long epochMillis) {
        Calendar c = Calendar.getInstance();
        c.setTimeInMillis(epochMillis);
        int year = Math.max(0, c.get(Calendar.YEAR) - 1980) & 0x7F;
        int month = (c.get(Calendar.MONTH) + 1) & 0x0F;
        int day = c.get(Calendar.DAY_OF_MONTH) & 0x1F;
        int hour = c.get(Calendar.HOUR_OF_DAY) & 0x1F;
        int minute = c.get(Calendar.MINUTE) & 0x3F;
        int second = (c.get(Calendar.SECOND) / 2) & 0x1F;
        return ((long) year << 25) | ((long) month << 21) | ((long) day << 16) |
                ((long) hour << 11) | ((long) minute << 5) | second;
    }

    private void sendDraegerRealtimeBurst(OutputStream out) throws IOException {
        if (drTransmissionSuspended || !drRealtimeEnabled || drRealtimeConfiguredCodes.isEmpty()) return;
        if (!chkDrWaveforms.isSelected()) return;

        int rr = slRespRate.getValue();
        int pp = slPeakPres.getValue();
        int peep = slPeep.getValue();
        int tv = slTidalVol.getValue();
        double breathPeriod = 60.0 / Math.max(rr, 1);
        double ieI = 1.0, ieE = 2.0;
        double inspRatio = ieI / (ieI + ieE);

        int samplesPerBurst = 10;
            for (int s = 0; s < samplesPerBurst; s++) {
                double t = drWaveformSampleIndex / 100.0;
                double breathPhase = (t % breathPeriod) / breathPeriod;
                int streamCount = Math.min(drRealtimeConfiguredCodes.size(), 4);
                int sync = RT_SYNC_BYTE;
                for (int i = 0; i < streamCount; i++) {
                    sync |= 1 << i;
                }
                out.write(sync);

                if (drWaveformSampleIndex > 0) {
                double prevT = (drWaveformSampleIndex - 1) / 100.0;
                double prevPhase = (prevT % breathPeriod) / breathPeriod;
                if (prevPhase > 0.9 && breathPhase < 0.1) {
                    out.write(RT_SYNC);
                    out.write(RT_SYNC_INSP_START);
                }
            }

            for (int code : drRealtimeConfiguredCodes) {
                int sampleValue = generateDrWaveformSample(code, breathPhase, inspRatio, pp, peep, tv);
                int hiByte = 0x80 | ((sampleValue >> 6) & 0x3F);
                int loByte = 0x80 | (sampleValue & 0x3F);
                out.write(hiByte);
                out.write(loByte);
            }
            drWaveformSampleIndex++;
        }
        out.flush();
    }

    private int generateDrWaveformSample(int code, double breathPhase, double inspRatio,
                                          int peakPres, int peepVal, int tidalVol) {
        boolean isInsp = breathPhase < inspRatio;
        double plateauPres = peakPres * 0.82;
        double value = 0;

        switch (code) {
            case RT_CODE_AIRWAY_PRESSURE:
                if (isInsp) {
                    double ip = breathPhase / inspRatio;
                    if (ip < 0.1) value = peepVal + (peakPres - peepVal) * (ip / 0.1);
                    else if (ip < 0.3) value = peakPres;
                    else value = plateauPres + (peakPres - plateauPres) * Math.exp(-(ip - 0.3) * 5);
                } else {
                    double ep = (breathPhase - inspRatio) / (1.0 - inspRatio);
                    value = peepVal + (plateauPres - peepVal) * Math.exp(-ep * 4);
                }
                value += drRng.nextGaussian() * 0.3;
                value = Math.max(-10, Math.min(60, value));
                return (int) Math.round((value + 10) * 511.0 / 70.0);

            case RT_CODE_FLOW:
                double inspFlow = 40;
                if (isInsp) {
                    double ip = breathPhase / inspRatio;
                    if (ip < 0.05) value = inspFlow * (ip / 0.05);
                    else if (ip < 0.95) value = inspFlow;
                    else value = inspFlow * (1.0 - (ip - 0.95) / 0.05);
                } else {
                    double ep = (breathPhase - inspRatio) / (1.0 - inspRatio);
                    value = -inspFlow * 0.8 * Math.exp(-ep * 3);
                }
                value += drRng.nextGaussian() * 0.5;
                value = Math.max(-60, Math.min(60, value));
                return (int) Math.round((value + 60) * 511.0 / 120.0);

            case RT_CODE_VOLUME:
                if (isInsp) {
                    value = tidalVol * (breathPhase / inspRatio);
                } else {
                    double ep = (breathPhase - inspRatio) / (1.0 - inspRatio);
                    value = tidalVol * Math.exp(-ep * 4);
                }
                value += drRng.nextGaussian() * 2;
                value = Math.max(0, Math.min(800, value));
                return (int) Math.round(value * 511.0 / 800.0);

            case RT_CODE_EXP_VOLUME:
                if (isInsp) {
                    value = 0;
                } else {
                    double ep = (breathPhase - inspRatio) / (1.0 - inspRatio);
                    value = tidalVol * (1.0 - Math.exp(-ep * 4));
                }
                value += drRng.nextGaussian() * 2;
                value = Math.max(0, Math.min(800, value));
                return (int) Math.round(value * 511.0 / 800.0);

            default:
                return 0;
        }
    }

    private void addDrRecord(ByteArrayOutputStream d, int code, int value) throws IOException {
        d.write(asciiHexHi(code)); d.write(asciiHexLo(code));
        String v = String.valueOf(Math.abs(value));
        while (v.length() < 4) v = " " + v;
        if (v.length() > 4) v = v.substring(v.length()-4);
        d.write(v.getBytes());
    }

    private void addAlarmRecord(ByteArrayOutputStream d, int priority, int code, String phrase) throws IOException {
        d.write(0x30 + priority);
        d.write(asciiHexHi(code)); d.write(asciiHexLo(code));
        String p = padTo(phrase, 12);
        d.write(p.substring(0, 12).getBytes());
    }

    // =====================================================================
    // Philips Simulator
    // =====================================================================

    private void startPhilips() {
        if (philipsRunning.get()) return;
        String transport = (String) philipsTransportCombo.getSelectedItem();
        if ("MIB-RS232".equals(transport)) {
            startPhilipsSerial();
            return;
        }
        int port;
        try {
            port = Integer.parseInt(philipsPortField.getText().trim());
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Invalid port number");
            return;
        }

        philipsRunning.set(true);
        btnPhilipsStart.setEnabled(false);
        btnPhilipsStop.setEnabled(true);
        philipsStatus.setText("Running");
        philipsStatus.setForeground(new Color(0, 200, 0));
        log("[Philips] Starting on UDP port " + port);

        philipsThread = new Thread(() -> runPhilipsServer(port), "philips-sim-ci");
        philipsThread.setDaemon(true);
        philipsThread.start();
    }

    private void startPhilipsSerial() {
        String serialPath = philipsSerialField.getText().trim();
        if (serialPath.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Enter a Philips serial device path");
            return;
        }
        philipsRunning.set(true);
        try {
            writeSerialControlFiles();
            ProcessBuilder pb = new ProcessBuilder(
                    javaCommand(), "-cp", simulatorClasspath(), "IntellivueSerialSimulator",
                    "--serial", serialPath,
                    "--control-file", philipsControlFile.getAbsolutePath());
            pb.redirectErrorStream(true);
            philipsSerialProcess = pb.start();
            startProcessLogReader("Philips MIB-RS232", philipsSerialProcess);
        } catch (IOException e) {
            philipsRunning.set(false);
            JOptionPane.showMessageDialog(this, "Could not start Philips MIB-RS232 simulator: " + e.getMessage());
            return;
        }

        btnPhilipsStart.setEnabled(false);
        btnPhilipsStop.setEnabled(true);
        philipsStatus.setText("Running");
        philipsStatus.setForeground(new Color(0, 200, 0));
        philipsConnStatus.setText("Serial: " + serialPath);
        log("[Philips] Starting MIB-RS232 simulator on " + serialPath);
    }

    private void stopPhilips() {
        philipsRunning.set(false);
        stopProcess(philipsSerialProcess);
        philipsSerialProcess = null;
        if (broadcastExec != null) {
            broadcastExec.shutdownNow();
            broadcastExec = null;
        }
        try {
            if (philipsSocket != null && !philipsSocket.isClosed()) {
                philipsSocket.close();
            }
        } catch (Exception ignored) {}
        phAssociated = false;
        btnPhilipsStart.setEnabled(true);
        btnPhilipsStop.setEnabled(false);
        philipsStatus.setText("Stopped");
        philipsStatus.setForeground(Color.GRAY);
        philipsConnStatus.setText("No connection");
        log("[Philips] Stopped");
    }

    private void runPhilipsServer(int port) {
        try {
            philipsSocket = new DatagramSocket(port);
            philipsSocket.setSoTimeout(500);
            philipsSocket.setBroadcast(true);
            log("[Philips] Listening on UDP port " + port);

            broadcastExec = Executors.newSingleThreadScheduledExecutor(r -> {
                Thread t = new Thread(r, "ph-broadcast-ci");
                t.setDaemon(true);
                return t;
            });
            broadcastExec.scheduleAtFixedRate(() -> {
                if (!phAssociated && philipsRunning.get() && chkBroadcast.isSelected()) {
                    try {
                        sendPhilipsConnectIndication(philipsSocket, port);
                        SwingUtilities.invokeLater(() -> philipsConnStatus.setText("Broadcasting..."));
                    } catch (Exception ignored) {}
                }
            }, 0, 5, TimeUnit.SECONDS);

            byte[] recvBuf = new byte[8192];
            phAssociated = false;
            phPollNumber = 0;
            phSequenceNumber = 0;

            while (philipsRunning.get()) {
                try {
                    DatagramPacket pkt = new DatagramPacket(recvBuf, recvBuf.length);
                    philipsSocket.receive(pkt);
                    ByteBuffer in = ByteBuffer.wrap(pkt.getData(), 0, pkt.getLength());
                    in.order(ByteOrder.BIG_ENDIAN);
                    InetSocketAddress client = new InetSocketAddress(pkt.getAddress(), pkt.getPort());

                    int first = in.get(0) & 0xFF;
                    eventCount.incrementAndGet();
                    lastPollTimestamp = System.currentTimeMillis();

                    if (first == CN_SPDU_SI) {
                        log("[Philips] <- Association Request from " + client);
                        SwingUtilities.invokeLater(() ->
                            philipsConnStatus.setText("Connected: " + client));

                        sendPhilipsAssocResponse(philipsSocket, client);
                        phAssociated = true;
                        log("[Philips] -> Association Response (accepted)");

                        sendPhilipsMdsEvent(philipsSocket, client);
                        log("[Philips] -> MDS Create Event");

                    } else if (first == DATA_EXPORT_SPDU && phAssociated) {
                        in.position(0);
                        // 4-byte SPpdu header: session_id(u16) + p_context_id(u16)
                        int sessionId = in.getShort() & 0xFFFF;  // 0xE100
                        int pContextId = in.getShort() & 0xFFFF; // 2
                        int roType = in.getShort() & 0xFFFF;

                        if (roType == ROIV_APDU) {
                            in.getShort();
                            int invokeId = in.getShort() & 0xFFFF;
                            int cmdType = in.getShort() & 0xFFFF;
                            int cmdLen = in.getShort() & 0xFFFF;

                            if (cmdType == CMD_CONFIRMED_ACTION) {
                                if (in.remaining() >= 6) in.position(in.position() + 6);
                                int scope = in.remaining() >= 4 ? in.getInt() : 0;
                                int actionType = in.remaining() >= 2 ? (in.getShort() & 0xFFFF) : 0;
                                int actionPayloadLen = in.remaining() >= 2 ? (in.getShort() & 0xFFFF) : 0;
                                int requestedObjectClass = 0;
                                if (actionPayloadLen >= 8 && in.remaining() >= 8) {
                                    int requestStart = in.position();
                                    in.getShort(); // poll number
                                    in.getShort(); // object partition
                                    requestedObjectClass = in.getShort() & 0xFFFF;
                                    in.position(requestStart);
                                }

                                phPollNumber++;
                                log("[Philips] <- Poll Request #" + phPollNumber);

                                if (actionType == (NOM_ACT_POLL_MDIB_DATA_EXT & 0xFFFF)) {
                                    sendPhilipsExtendedPoll(philipsSocket, client, invokeId);
                                } else if (requestedObjectClass == NOM_MOC_VMO_METRIC_SA_RT && chkPhWaveforms.isSelected()) {
                                    sendPhilipsWaveformPollResult(philipsSocket, client, invokeId);
                                    log("[Philips]   -> Waveform poll result");
                                } else {
                                    sendPhilipsPollResult(philipsSocket, client, invokeId);
                                }

                                log("[Philips] -> Poll Result: HR=" + slHeartRate.getValue() +
                                    " SpO2=" + slSpo2.getValue() + " RR=" + slPhRespRate.getValue() +
                                    " ABP=" + slAbpSys.getValue() + "/" + slAbpDia.getValue() +
                                    " CVP=" + slCvp.getValue() +
                                    " PAP=" + slPapSys.getValue() + "/" + slPapDia.getValue());
                            } else if (cmdType == CMD_GET) {
                                log("[Philips] <- Get Request");
                                sendPhilipsGetResult(philipsSocket, client, invokeId);
                            } else {
                                sendPhilipsEmptyResult(philipsSocket, client, invokeId, cmdType);
                            }
                        }
                    } else if (first == FN_SPDU_SI) {
                        phAssociated = false;
                        log("[Philips] <- Release Request");
                        ByteBuffer resp = ByteBuffer.allocate(4);
                        resp.order(ByteOrder.BIG_ENDIAN);
                        resp.put((byte) DN_SPDU_SI);
                        resp.put((byte) 0x00);
                        resp.putShort((short) 0);
                        resp.flip();
                        sendUdp(philipsSocket, client, resp);
                        SwingUtilities.invokeLater(() -> philipsConnStatus.setText("Disconnected"));
                    }
                } catch (SocketTimeoutException ignored) {
                }
            }
        } catch (IOException e) {
            if (philipsRunning.get()) log("[Philips] Error: " + e.getMessage());
        }
    }

    private void sendPhilipsConnectIndication(DatagramSocket socket, int listenPort) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(128);
        buf.order(ByteOrder.BIG_ENDIAN);

        buf.put((byte) CN_SPDU_SI);
        int lenPos = buf.position();
        buf.putShort((short) 0);

        buf.putInt(0x80000000);

        try {
            InetAddress localAddr = InetAddress.getLocalHost();
            byte[] addrBytes = localAddr.getAddress();
            buf.putShort((short) 0x0001); buf.putShort((short) 4); buf.put(addrBytes);
            buf.putShort((short) 0x0002); buf.putShort((short) 2); buf.putShort((short) listenPort);
        } catch (UnknownHostException e) {
            buf.putShort((short) 0x0001); buf.putShort((short) 4); buf.putInt(0);
            buf.putShort((short) 0x0002); buf.putShort((short) 2); buf.putShort((short) listenPort);
        }

        buf.putShort((short) 0x0003); buf.putShort((short) 4); buf.putInt(0x40000000);
        buf.putShort((short) 0x0004); buf.putShort((short) 4); buf.putInt(0x40000000);

        int totalLen = buf.position() - lenPos - 2;
        buf.putShort(lenPos, (short) totalLen);

        buf.flip();
        byte[] data = new byte[buf.remaining()];
        buf.get(data);
        try {
            socket.send(new DatagramPacket(data, data.length,
                    InetAddress.getByName("255.255.255.255"), BROADCAST_PORT));
        } catch (IOException ignored) {}
    }

    private void sendPhilipsAssocResponse(DatagramSocket socket, InetSocketAddress addr) throws IOException {
        // Build MDSEUserInfoStd payload (matches IntellivueSimulatorV2.buildAssociationResponse)
        ByteBuffer userInfo = ByteBuffer.allocate(256);
        userInfo.order(ByteOrder.BIG_ENDIAN);

        // MDSEUserInfoStd fields
        userInfo.putInt((int) 0x80000000L); // protocolVersion = MDDL_VERSION1
        userInfo.putInt((int) 0x40000000L); // nomenclatureVersion = NOMEN_VERSION
        userInfo.putInt(0x00000000);        // functionalUnits
        userInfo.putInt((int) 0x00800000L); // systemType = SYST_SERVER
        userInfo.putInt((int) 0x20000000L); // startupMode = COLD_START

        // optionList (AttributeValueList — empty: count=0, length=0)
        userInfo.putShort((short) 0); // count
        userInfo.putShort((short) 0); // length

        // supportedAProfiles (AttributeValueList with PollProfileSupport + MdibObjectSupport)
        ByteBuffer attrs = ByteBuffer.allocate(200);
        attrs.order(ByteOrder.BIG_ENDIAN);

        // Attribute 1: PollProfileSupport (attribute_id=0x0001)
        ByteBuffer pps = ByteBuffer.allocate(64);
        pps.order(ByteOrder.BIG_ENDIAN);
        pps.putInt(0x80000000);       // pollProfileRevision
        pps.putInt(80000);            // minPollPeriod (RelativeTime): 80000 x 125us = 10s
        pps.putInt(1456);             // maxMtuRx
        pps.putInt(1456);             // maxMtuTx
        pps.putInt(0x00000000);       // maxBwTx
        pps.putInt(0x60000000);       // poll profile options
        // optionalPackages (AttributeValueList: count=0, length=0)
        pps.putShort((short) 0);      // count
        pps.putShort((short) 0);      // length
        pps.flip();

        attrs.putShort((short) 0x0001); // attribute_id for PollProfileSupport
        attrs.putShort((short) pps.remaining()); // length
        attrs.put(pps);

        // Attribute 2: MdibObjectSupport (attribute_id=0x0102)
        ByteBuffer mos = ByteBuffer.allocate(100);
        mos.order(ByteOrder.BIG_ENDIAN);
        int numObjClasses = 6;
        mos.putShort((short) numObjClasses);
        mos.putShort((short) (numObjClasses * 8));
        // MDS
        mos.putShort((short) NOM_MOC_VMS_MDS); mos.putShort((short) 0); mos.putInt(1);
        // Numeric
        mos.putShort((short) NOM_MOC_VMO_METRIC_NU); mos.putShort((short) 0); mos.putInt(64);
        // SampleArray
        mos.putShort((short) NOM_MOC_VMO_METRIC_SA_RT); mos.putShort((short) 0); mos.putInt(16);
        // Alarm Monitor
        mos.putShort((short) NOM_MOC_VMO_AL_MON); mos.putShort((short) 0); mos.putInt(2);
        // Patient Demographics
        mos.putShort((short) NOM_MOC_PT_DEMOG); mos.putShort((short) 0); mos.putInt(1);
        // Enumeration
        mos.putShort((short) NOM_MOC_VMO_METRIC_ENUM); mos.putShort((short) 0); mos.putInt(16);
        mos.flip();

        attrs.putShort((short) 0x0102); // attribute_id for MdibObjectSupport
        attrs.putShort((short) mos.remaining());
        attrs.put(mos);
        attrs.flip();

        // supportedAProfiles: count=2, length=attrs.remaining()
        userInfo.putShort((short) 2);
        userInfo.putShort((short) attrs.remaining());
        userInfo.put(attrs);

        userInfo.flip();
        int userInfoLen = userInfo.remaining();

        // ASN.1 BER length encoding for userInfo
        byte[] asnLen;
        if (userInfoLen < 128) {
            asnLen = new byte[] { (byte) userInfoLen };
        } else if (userInfoLen < 256) {
            asnLen = new byte[] { (byte) 0x81, (byte) userInfoLen };
        } else {
            asnLen = new byte[] { (byte) 0x82, (byte) ((userInfoLen >> 8) & 0xFF), (byte) (userInfoLen & 0xFF) };
        }

        // Presentation header — exact bytes from AssociationAcceptImpl in driver
        byte[] presHeader = new byte[] {
            0x31, (byte)0x80, (byte)0xA0, (byte)0x80, (byte)0x80, 0x01, 0x01, 0x00, 0x00,
            (byte)0xA2, (byte)0x80, (byte)0xA0, 0x03, 0x00, 0x00, 0x01, (byte)0xA5, (byte)0x80,
            0x30, (byte)0x80, (byte)0x80, 0x01, 0x00, (byte)0x81, 0x02, 0x51, 0x01, 0x00, 0x00,
            0x30, (byte)0x80, (byte)0x80, 0x01, 0x00, (byte)0x81, 0x0C, 0x2A, (byte)0x86, 0x48,
            (byte)0xCE, 0x14, 0x02, 0x01, 0x00, 0x00, 0x00, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00,
            0x61, (byte)0x80, 0x30, (byte)0x80, 0x02, 0x01, 0x01, (byte)0xA0, (byte)0x80, 0x61,
            (byte)0x80, (byte)0xA1, (byte)0x80, 0x06, 0x0C, 0x2A, (byte)0x86, 0x48, (byte)0xCE,
            0x14, 0x02, 0x01, 0x00, 0x00, 0x00, 0x03, 0x01, 0x00, 0x00, (byte)0xA2, 0x03, 0x02,
            0x01, 0x00, (byte)0xA3, 0x05, (byte)0xA1, 0x03, 0x02, 0x01, 0x00, (byte)0xBE,
            (byte)0x80, 0x28, (byte)0x80, 0x02, 0x01, 0x02, (byte)0x81
        };

        // Presentation trailer — 16 zero bytes
        byte[] presTrailer = new byte[16];

        // Session parameters (PI 0x05 and PI 0x14)
        byte[] sessionPI = new byte[] {
            0x05, 0x08, 0x13, 0x01, 0x00, 0x16, 0x01, 0x02, (byte)0x80, 0x00,
            0x14, 0x02, 0x00, 0x02
        };

        // Presentation context (0xC1 + length)
        int presContentLen = presHeader.length + asnLen.length + userInfoLen + presTrailer.length;

        // Total session body length
        int bodyLen = sessionPI.length + 1 /* 0xC1 tag */ + phLengthInfoBytes(presContentLen).length + presContentLen;

        ByteBuffer buf = ByteBuffer.allocate(512);
        buf.order(ByteOrder.BIG_ENDIAN);

        // Session header: type=0x0E (Accept)
        buf.put((byte) AC_SPDU_SI);
        // Session length: uses LengthInformation encoding (0xFF + 2 bytes if >= 255)
        if (bodyLen < 255) {
            buf.put((byte) bodyLen);
        } else {
            buf.put((byte) 0xFF);
            buf.putShort((short) bodyLen);
        }

        // Session parameters
        buf.put(sessionPI);

        // Presentation context: 0xC1 + length (LengthInformation format)
        buf.put((byte) 0xC1);
        if (presContentLen < 255) {
            buf.put((byte) presContentLen);
        } else {
            buf.put((byte) 0xFF);
            buf.putShort((short) presContentLen);
        }

        // Presentation header
        buf.put(presHeader);

        // ASN.1 length + user info
        buf.put(asnLen);
        buf.put(userInfo);

        // Presentation trailer
        buf.put(presTrailer);

        buf.flip();
        sendUdp(socket, addr, buf);
    }

    private static byte[] phLengthInfoBytes(int len) {
        if (len < 128) {
            return new byte[] { (byte) len };
        } else if (len < 256) {
            return new byte[] { (byte) 0x81, (byte) len };
        } else {
            return new byte[] { (byte) 0x82, (byte) ((len >> 8) & 0xFF), (byte) (len & 0xFF) };
        }
    }

    private void sendPhilipsMdsEvent(DatagramSocket socket, InetSocketAddress addr) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(512);
        buf.order(ByteOrder.BIG_ENDIAN);

        // 4-byte SPpdu header
        buf.putShort((short) 0xE100); // session_id
        buf.putShort((short) 0x0002); // p_context_id (fixed at 2)

        buf.putShort((short) ROIV_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);
        buf.putShort((short) phInvokeId++);
        buf.putShort((short) CMD_CONFIRMED_EVENT);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);

        // Managed Object: MDS
        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0); buf.putShort((short) 0);

        // Event time (relative) + event type
        buf.putInt(0); // relative time
        buf.putShort((short) NOM_NOTI_MDS_CREAT); // event type
        int eventLenPos = buf.position();
        buf.putShort((short) 0); // event length placeholder

        // MDS Create Event data: ManagedObject + AttributeList
        // (per MdsCreateEventImpl.parse: managedObject first, then attrs)
        buf.putShort((short) NOM_MOC_VMS_MDS); // managed object oidType
        buf.putShort((short) 0);               // context
        buf.putShort((short) 0);               // handle

        // Attribute list — minimal: just System Type
        buf.putShort((short) 1);  // count = 1
        int attrListLenPos = buf.position();
        buf.putShort((short) 0);  // attr list length placeholder
        int attrStart = buf.position();

        // Attribute: System Type
        buf.putShort((short) NOM_ATTR_SYS_TYPE); // 0x0986
        buf.putShort((short) 4);
        buf.putShort((short) NOM_PART_OBJ);
        buf.putShort((short) NOM_MOC_VMS_MDS);

        int attrEnd = buf.position();

        // Fix lengths
        buf.putShort(attrListLenPos, (short) (attrEnd - attrStart));
        buf.putShort(eventLenPos, (short) (attrEnd - eventLenPos - 2));
        int end = buf.position();
        buf.putShort(roLenPos, (short) (end - roLenPos - 2));
        buf.putShort(cmdLenPos, (short) (end - cmdLenPos - 2));

        buf.flip();
        sendUdp(socket, addr, buf);
    }

    private void sendPhilipsExtendedPoll(DatagramSocket socket, InetSocketAddress addr, int invokeId) throws IOException {
        ByteBuffer numSeg = buildPhNumericsSegment(invokeId);
        sendUdp(socket, addr, numSeg);
        log("[Philips]   -> ROLRS: Numerics segment");

        if (chkPhWaveforms.isSelected()) {
            ByteBuffer waveSeg = buildPhWaveformSegment(invokeId);
            sendUdp(socket, addr, waveSeg);
            log("[Philips]   -> ROLRS: Waveform segment");
        }

        ByteBuffer finalSeg = buildPhFinalSegment(invokeId);
        sendUdp(socket, addr, finalSeg);
        log("[Philips]   -> RORS: Final segment (alarms, patient, enums)");
    }

    private ByteBuffer buildPhNumericsSegment(int clientInvokeId) {
        ByteBuffer buf = ByteBuffer.allocate(2048);
        buf.order(ByteOrder.BIG_ENDIAN);

        // 4-byte SPpdu header
        buf.putShort((short) 0xE100); // session_id
        buf.putShort((short) 0x0002); // p_context_id (fixed at 2)

        // RORS final result. The dashboard driver handles each extended-poll
        // result independently, which keeps the simulator deterministic.
        buf.putShort((short) RORS_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) clientInvokeId);

        buf.putShort((short) CMD_CONFIRMED_ACTION);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);

        // Managed Object
        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0); buf.putShort((short) 0);

        // Action type
        buf.putShort((short)(NOM_ACT_POLL_MDIB_DATA_EXT & 0xFFFF));
        int actionLenPos = buf.position();
        buf.putShort((short) 0);

        // Extended poll number, sequence number, and timestamps
        writePhExtendedPollHeader(buf);

        // Poll info list
        buf.putShort((short) 1); // 1 context
        int pollInfoLenPos = buf.position();
        buf.putShort((short) 0);
        int pollInfoStart = buf.position();

        // SingleContextPoll
        buf.putShort((short) 0); // mds_context

        // Observation list: all numerics
        int[][] numerics = getPhNumerics();
        buf.putShort((short) numerics.length);
        int obsListLenPos = buf.position();
        buf.putShort((short) 0);
        int obsStart = buf.position();

        for (int[] n : numerics) {
            writePhNumericObs(buf, n);
        }

        int obsEnd = buf.position();
        buf.putShort(obsListLenPos, (short)(obsEnd - obsStart));
        buf.putShort(pollInfoLenPos, (short)(obsEnd - pollInfoStart));

        // Fix lengths
        fixPhLengths3(buf, roLenPos, cmdLenPos, actionLenPos);

        buf.flip();
        return buf;
    }

    private ByteBuffer buildPhWaveformSegment(int clientInvokeId) {
        ByteBuffer buf = ByteBuffer.allocate(8192);
        buf.order(ByteOrder.BIG_ENDIAN);

        // 4-byte SPpdu header
        buf.putShort((short) 0xE100); // session_id
        buf.putShort((short) 0x0002); // p_context_id (fixed at 2)

        // RORS final result.
        buf.putShort((short) RORS_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) clientInvokeId);

        buf.putShort((short) CMD_CONFIRMED_ACTION);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0); buf.putShort((short) 0);

        buf.putShort((short)(NOM_ACT_POLL_MDIB_DATA_EXT & 0xFFFF));
        int actionLenPos = buf.position();
        buf.putShort((short) 0);

        writePhExtendedPollHeader(buf);

        // Poll info list
        buf.putShort((short) 1);
        int pollInfoLenPos = buf.position();
        buf.putShort((short) 0);
        int pollInfoStart = buf.position();

        buf.putShort((short) 0); // mds_context

        // 4 waveforms
        buf.putShort((short) 4);
        int obsListLenPos = buf.position();
        buf.putShort((short) 0);
        int obsStart = buf.position();

        writePhWaveformObs(buf, 0x50, NOM_ECG_ELEC_POTL_II_SA, generatePhEcgSamples(), 500, NOM_DIM_MILLI_VOLT);
        writePhWaveformObs(buf, 0x51, NOM_PLETH_PULS_OXIM_SA, generatePhPlethSamples(), 125, NOM_DIM_DIMLESS);
        writePhWaveformObs(buf, 0x52, NOM_PRESS_BLD_ART_ABP_SA, generatePhAbpSamples(), 125, NOM_DIM_MMHG);
        writePhWaveformObs(buf, 0x53, NOM_RESP_SA, generatePhRespSamples(), 62, NOM_DIM_DIMLESS);

        int obsEnd = buf.position();
        buf.putShort(obsListLenPos, (short)(obsEnd - obsStart));
        buf.putShort(pollInfoLenPos, (short)(obsEnd - pollInfoStart));

        // Fix lengths
        fixPhLengths3(buf, roLenPos, cmdLenPos, actionLenPos);

        buf.flip();
        return buf;
    }

    private ByteBuffer buildPhFinalSegment(int clientInvokeId) {
        ByteBuffer buf = ByteBuffer.allocate(2048);
        buf.order(ByteOrder.BIG_ENDIAN);

        // 4-byte SPpdu header
        buf.putShort((short) 0xE100); // session_id
        buf.putShort((short) 0x0002); // p_context_id (fixed at 2)

        // RORS (final result)
        buf.putShort((short) RORS_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) clientInvokeId);
        buf.putShort((short) CMD_CONFIRMED_ACTION);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0); buf.putShort((short) 0);

        buf.putShort((short)(NOM_ACT_POLL_MDIB_DATA_EXT & 0xFFFF));
        int actionLenPos = buf.position();
        buf.putShort((short) 0);

        writePhExtendedPollHeader(buf);

        // Poll info list: 1 context with multiple obs
        buf.putShort((short) 1);
        int pollInfoLenPos = buf.position();
        buf.putShort((short) 0);
        int pollInfoStart = buf.position();

        buf.putShort((short) 0); // mds_context

        // 3 observations: alarm monitor, patient demog, enum
        buf.putShort((short) 3);
        int obsListLenPos = buf.position();
        buf.putShort((short) 0);
        int obsStart = buf.position();

        writePhAlarmMonitorObs(buf, 0x60);
        writePhPatientDemogObs(buf, 0x70);
        writePhEnumObs(buf, 0x80);

        int obsEnd = buf.position();
        buf.putShort(obsListLenPos, (short)(obsEnd - obsStart));
        buf.putShort(pollInfoLenPos, (short)(obsEnd - pollInfoStart));

        // Fix lengths
        fixPhLengths3(buf, roLenPos, cmdLenPos, actionLenPos);

        buf.flip();
        return buf;
    }

    private void sendPhilipsPollResult(DatagramSocket socket, InetSocketAddress addr, int invokeId) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(2048);
        buf.order(ByteOrder.BIG_ENDIAN);

        // 4-byte SPpdu header
        buf.putShort((short) 0xE100); // session_id
        buf.putShort((short) 0x0002); // p_context_id (fixed at 2)

        buf.putShort((short) RORS_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) invokeId);
        buf.putShort((short) CMD_CONFIRMED_ACTION);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0); buf.putShort((short) 0);

        buf.putShort((short)(NOM_ACT_POLL_MDIB_DATA & 0xFFFF));
        int actionLenPos = buf.position();
        buf.putShort((short) 0);

        writePhPollHeader(buf);

        // Poll info list
        buf.putShort((short) 1);
        int pollInfoLenPos = buf.position();
        buf.putShort((short) 0);
        int pollInfoStart = buf.position();

        buf.putShort((short) 0); // mds_context

        int[][] numerics = getPhNumerics();
        buf.putShort((short) numerics.length);
        int obsListLenPos = buf.position();
        buf.putShort((short) 0);
        int obsStart = buf.position();

        for (int[] n : numerics) {
            writePhNumericObs(buf, n);
        }

        int obsEnd = buf.position();
        buf.putShort(obsListLenPos, (short)(obsEnd - obsStart));
        buf.putShort(pollInfoLenPos, (short)(obsEnd - pollInfoStart));

        // Fix lengths
        fixPhLengths3(buf, roLenPos, cmdLenPos, actionLenPos);

        buf.flip();
        sendUdp(socket, addr, buf);
    }

    private void sendPhilipsWaveformPollResult(DatagramSocket socket, InetSocketAddress addr, int invokeId) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(8192);
        buf.order(ByteOrder.BIG_ENDIAN);

        buf.putShort((short) 0xE100);
        buf.putShort((short) 0x0002);

        buf.putShort((short) RORS_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) invokeId);
        buf.putShort((short) CMD_CONFIRMED_ACTION);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0); buf.putShort((short) 0);

        buf.putShort((short)(NOM_ACT_POLL_MDIB_DATA & 0xFFFF));
        int actionLenPos = buf.position();
        buf.putShort((short) 0);

        writePhPollHeader(buf);

        buf.putShort((short) 1);
        int pollInfoLenPos = buf.position();
        buf.putShort((short) 0);
        int pollInfoStart = buf.position();

        buf.putShort((short) 0);

        buf.putShort((short) 4);
        int obsListLenPos = buf.position();
        buf.putShort((short) 0);
        int obsStart = buf.position();

        writePhWaveformObs(buf, 0x50, NOM_ECG_ELEC_POTL_II_SA, generatePhEcgSamples(), 500, NOM_DIM_MILLI_VOLT);
        writePhWaveformObs(buf, 0x51, NOM_PLETH_PULS_OXIM_SA, generatePhPlethSamples(), 125, NOM_DIM_DIMLESS);
        writePhWaveformObs(buf, 0x52, NOM_PRESS_BLD_ART_ABP_SA, generatePhAbpSamples(), 125, NOM_DIM_MMHG);
        writePhWaveformObs(buf, 0x53, NOM_RESP_SA, generatePhRespSamples(), 62, NOM_DIM_DIMLESS);

        int obsEnd = buf.position();
        buf.putShort(obsListLenPos, (short)(obsEnd - obsStart));
        buf.putShort(pollInfoLenPos, (short)(obsEnd - pollInfoStart));

        fixPhLengths3(buf, roLenPos, cmdLenPos, actionLenPos);

        buf.flip();
        sendUdp(socket, addr, buf);
    }

    private void sendPhilipsGetResult(DatagramSocket socket, InetSocketAddress addr, int invokeId) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(512);
        buf.order(ByteOrder.BIG_ENDIAN);

        // 4-byte SPpdu header
        buf.putShort((short) 0xE100); // session_id
        buf.putShort((short) 0x0002); // p_context_id (fixed at 2)

        buf.putShort((short) RORS_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);

        buf.putShort((short) invokeId);
        buf.putShort((short) CMD_GET);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);

        // Managed Object
        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0); buf.putShort((short) 0);

        // Attribute list
        buf.putShort((short) 3);
        int attrListLenPos = buf.position();
        buf.putShort((short) 0);
        int attrStart = buf.position();

        // Attribute: System Type
        buf.putShort((short) NOM_ATTR_SYS_TYPE);
        buf.putShort((short) 4);
        buf.putShort((short) NOM_PART_OBJ);
        buf.putShort((short) NOM_MOC_VMS_MDS);

        // Attribute: System Model
        writePhSystemModel(buf);

        // Attribute: Absolute Time
        buf.putShort((short) NOM_ATTR_TIME_ABS);
        buf.putShort((short) 8);
        writePhAbsoluteTime(buf);

        int attrEnd = buf.position();
        buf.putShort(attrListLenPos, (short)(attrEnd - attrStart));

        // Fix lengths
        int end = buf.position();
        buf.putShort(roLenPos, (short)(end - roLenPos - 2));
        buf.putShort(cmdLenPos, (short)(end - cmdLenPos - 2));

        buf.flip();
        sendUdp(socket, addr, buf);
    }

    private void sendPhilipsEmptyResult(DatagramSocket socket, InetSocketAddress addr, int invokeId, int cmdType) throws IOException {
        ByteBuffer buf = ByteBuffer.allocate(32);
        buf.order(ByteOrder.BIG_ENDIAN);
        // 4-byte SPpdu header
        buf.putShort((short) 0xE100); // session_id
        buf.putShort((short) 0x0002); // p_context_id
        buf.putShort((short) RORS_APDU);
        int roLenPos = buf.position();
        buf.putShort((short) 0);
        buf.putShort((short) invokeId);
        buf.putShort((short) cmdType);
        int cmdLenPos = buf.position();
        buf.putShort((short) 0);
        buf.putShort((short) NOM_MOC_VMS_MDS);
        buf.putShort((short) 0);
        buf.putShort((short) 0);
        if (cmdType == CMD_SET || cmdType == CMD_CONFIRMED_SET) {
            buf.putShort((short) 0); // AttributeValueList count
            buf.putShort((short) 0); // AttributeValueList length
        }
        int end = buf.position();
        buf.putShort(cmdLenPos, (short) (end - cmdLenPos - 2));
        buf.putShort(roLenPos, (short) (end - roLenPos - 2));
        buf.flip();
        sendUdp(socket, addr, buf);
    }

    private int[][] getPhNumerics() {
        int hr = slHeartRate.getValue();
        int spo2 = slSpo2.getValue();
        int rr = slPhRespRate.getValue();
        int abpS = slAbpSys.getValue();
        int abpD = slAbpDia.getValue();
        int abpM = abpS > 0 ? (abpS + 2 * abpD) / 3 : 0;
        int nbpS = slNbpSys.getValue();
        int nbpD = slNbpDia.getValue();
        int nbpM = nbpS > 0 ? (nbpS + 2 * nbpD) / 3 : 0;
        int etco2 = slEtCo2.getValue();
        int temp = slTemp.getValue();
        int cvp = slCvp.getValue();
        int papS = slPapSys.getValue();
        int papD = slPapDia.getValue();
        int papM = (papS + 2 * papD) / 3;

        return new int[][] {
            {NOM_ECG_CARD_BEAT_RATE,     hr,    NOM_DIM_BEAT_PER_MIN, 0x01, 0},
            {NOM_PULS_OXIM_SAT_O2,       spo2,  NOM_DIM_PERCENT,      0x02, 0},
            {NOM_PULS_OXIM_PULS_RATE,    hr > 0 ? hr+1 : 0,  NOM_DIM_BEAT_PER_MIN, 0x03, 0},
            {NOM_RESP_RATE,              rr,    NOM_DIM_RESP_PER_MIN, 0x04, 0},
            {NOM_PRESS_BLD_ART_ABP_SYS,  abpS,  NOM_DIM_MMHG,         0x10, 0},
            {NOM_PRESS_BLD_ART_ABP_DIA,  abpD,  NOM_DIM_MMHG,         0x11, 0},
            {NOM_PRESS_BLD_ART_ABP_MEAN, abpM,  NOM_DIM_MMHG,         0x12, 0},
            {NOM_PRESS_BLD_NONINV_SYS,   nbpS,  NOM_DIM_MMHG,         0x20, 0},
            {NOM_PRESS_BLD_NONINV_DIA,   nbpD,  NOM_DIM_MMHG,         0x21, 0},
            {NOM_PRESS_BLD_NONINV_MEAN,  nbpM,  NOM_DIM_MMHG,         0x22, 0},
            {NOM_CO2_ET,                 etco2, NOM_DIM_MMHG,         0x30, 0},
            {NOM_TEMP_BLD,               temp,  NOM_DIM_DEGC,         0x40, 1},
            {NOM_PRESS_BLD_VEN_CENT,     cvp,   NOM_DIM_MMHG,         0x41, 0},
            {NOM_PRESS_BLD_ART_PULM_SYS, papS,  NOM_DIM_MMHG,         0x42, 0},
            {NOM_PRESS_BLD_ART_PULM_DIA, papD,  NOM_DIM_MMHG,         0x43, 0},
            {NOM_PRESS_BLD_ART_PULM_MEAN,papM,  NOM_DIM_MMHG,         0x44, 0},
        };
    }

    private void writePhNumericObs(ByteBuffer buf, int[] n) {
        int physioId = n[0], value = n[1], unitCode = n[2], handle = n[3], isTemp = n[4];

        // Per ObservationPollImpl.parse(): handle(u16) + AttributeList(count+length+AVAs)
        buf.putShort((short) handle);

        // Attribute list: just NOM_ATTR_NU_VAL_OBS for reliability
        buf.putShort((short) 1); // count = 1 attribute
        int attrListLenPos = buf.position();
        buf.putShort((short) 0); // length placeholder
        int attrStart = buf.position();

        // NOM_ATTR_NU_VAL_OBS (NumericObservedValue) — 10 bytes value
        // Per NumericObservedValue.parse(): physioId(u16) + msmtState(u16) + unitCode(u16) + value(u32)
        buf.putShort((short) NOM_ATTR_NU_VAL_OBS); // attribute_id
        buf.putShort((short) 10);                   // value length
        buf.putShort((short) physioId);              // physio_id (OIDType, u16)
        buf.putShort((short) 0x0000);               // measurement state (valid)
        buf.putShort((short) unitCode);              // unit_code (OIDType, u16)
        buf.putInt(encodePhFloat(value, isTemp != 0)); // FLOATType value

        int attrEnd = buf.position();
        buf.putShort(attrListLenPos, (short)(attrEnd - attrStart));
    }

    private String getPhLabel(int physioId) {
        switch (physioId) {
            case NOM_ECG_CARD_BEAT_RATE: return "HR";
            case NOM_PULS_OXIM_SAT_O2: return "SpO2";
            case NOM_PULS_OXIM_PULS_RATE: return "Pulse";
            case NOM_RESP_RATE: return "RR";
            case NOM_PRESS_BLD_ART_ABP_SYS: return "ABP Sys";
            case NOM_PRESS_BLD_ART_ABP_DIA: return "ABP Dia";
            case NOM_PRESS_BLD_ART_ABP_MEAN: return "ABP Mean";
            case NOM_PRESS_BLD_NONINV_SYS: return "NBP Sys";
            case NOM_PRESS_BLD_NONINV_DIA: return "NBP Dia";
            case NOM_PRESS_BLD_NONINV_MEAN: return "NBP Mean";
            case NOM_CO2_ET: return "etCO2";
            case NOM_TEMP_BLD: return "Temp";
            case NOM_PRESS_BLD_VEN_CENT: return "CVP";
            case NOM_PRESS_BLD_ART_PULM_SYS: return "PAP Sys";
            case NOM_PRESS_BLD_ART_PULM_DIA: return "PAP Dia";
            case NOM_PRESS_BLD_ART_PULM_MEAN: return "PAP Mean";
            default: return "Unknown";
        }
    }

    private void writePhWaveformObs(ByteBuffer buf, int handle, int physioId,
                                     short[] samples, int sampleRate, int unitCode) {
        PhilipsWaveformFrame.writeObservation(buf, handle, physioId, samples, sampleRate, unitCode);
    }

    private byte sampleByte(short sample) {
        return PhilipsWaveformFrame.sampleByte(sample);
    }

    private void writePhAlarmMonitorObs(ByteBuffer buf, int handle) {
        buf.putShort((short) handle);

        int hr = slHeartRate.getValue();
        int spo2 = slSpo2.getValue();
        int abpSys = slAbpSys.getValue();

        boolean hrHigh = hr > 120 || chkPhHrAlarm.isSelected();
        boolean hrLow  = hr < 50;
        boolean spo2Lo = spo2 < 90 || chkPhSpo2Low.isSelected();
        boolean abpHi  = abpSys > 160 || chkPhAbpHigh.isSelected();

        int physAlarmCount = 0;
        if (hrHigh) physAlarmCount++;
        if (hrLow) physAlarmCount++;
        if (spo2Lo) physAlarmCount++;
        if (abpHi) physAlarmCount++;

        buf.putShort((short) 3);
        int attrListLenPos = buf.position();
        buf.putShort((short) 0);
        int attrStart = buf.position();

        buf.putShort((short) NOM_ATTR_DEV_AL_COND);
        buf.putShort((short) 2);
        int alertState = physAlarmCount > 0 ? 0x0002 : 0;
        buf.putShort((short) alertState);

        buf.putShort((short) NOM_ATTR_AL_MON_T_AL_LIST);
        int tAlLenPos = buf.position();
        buf.putShort((short) 0);
        int tAlStart = buf.position();
        buf.putShort((short) 0);
        buf.putShort((short) 0);
        int tAlEnd = buf.position();
        buf.putShort(tAlLenPos, (short)(tAlEnd - tAlStart));

        buf.putShort((short) NOM_ATTR_AL_MON_P_AL_LIST);
        int pAlLenPos = buf.position();
        buf.putShort((short) 0);
        int pAlStart = buf.position();
        buf.putShort((short) physAlarmCount);
        buf.putShort((short)(physAlarmCount * 18));

        if (hrHigh) writePhAlarmEntry(buf, NOM_EVT_HI_HR, 3, NOM_ECG_CARD_BEAT_RATE);
        if (hrLow)  writePhAlarmEntry(buf, NOM_EVT_LO_HR, 3, NOM_ECG_CARD_BEAT_RATE);
        if (spo2Lo) writePhAlarmEntry(buf, NOM_EVT_LO_SAT_O2, 3, NOM_PULS_OXIM_SAT_O2);
        if (abpHi)  writePhAlarmEntry(buf, NOM_EVT_HI_ABP_SYS, 2, NOM_PRESS_BLD_ART_ABP_SYS);

        int pAlEnd = buf.position();
        buf.putShort(pAlLenPos, (short)(pAlEnd - pAlStart));

        int attrEnd = buf.position();
        buf.putShort(attrListLenPos, (short)(attrEnd - attrStart));
    }

    private void writePhAlarmEntry(ByteBuffer buf, int alarmCode, int priority, int sourceId) {
        buf.putShort((short) sourceId);
        buf.putShort((short) alarmCode);
        buf.putShort((short) (priority >= 3 ? 0x0400 : 0x0200));
        buf.putShort((short) 0x0008);
        buf.putShort((short) NOM_MOC_VMO_METRIC_NU);
        buf.putShort((short) 0);
        buf.putShort((short) sourceId);
        buf.putShort((short) 0);
        buf.putShort((short) 0);
    }

    private void writePhPatientDemogObs(ByteBuffer buf, int handle) {
        buf.putShort((short) handle);

        String fullName = tfPatientName.getText().trim();
        String[] parts = fullName.split("\\s+", 2);
        String givenName = parts[0];
        String familyName = parts.length > 1 ? parts[1] : "";
        String patId = tfPatientId.getText().trim();

        int dobYear = 1980, dobMonth = 1, dobDay = 15;
        try {
            String dob = tfPatientDob.getText().trim();
            String[] dp = dob.split("-");
            if (dp.length == 3) {
                dobYear = Integer.parseInt(dp[0]);
                dobMonth = Integer.parseInt(dp[1]);
                dobDay = Integer.parseInt(dp[2]);
            }
        } catch (Exception ignored) {}

        int sex = SEX_UNKNOWN;
        String sexStr = (String) cbPatientSex.getSelectedItem();
        if ("M".equals(sexStr)) sex = SEX_MALE;
        else if ("F".equals(sexStr)) sex = SEX_FEMALE;

        double height = 175;
        double weight = 75;
        try { height = Double.parseDouble(tfPatientHeight.getText().trim()); } catch (Exception ignored) {}
        try { weight = Double.parseDouble(tfPatientWeight.getText().trim()); } catch (Exception ignored) {}
        double bsa = 0.007184 * Math.pow(height, 0.725) * Math.pow(weight, 0.425);

        buf.putShort((short) 8);
        int attrListLenPos = buf.position();
        buf.putShort((short) 0);
        int attrStart = buf.position();

        writePhStringAttr(buf, NOM_ATTR_PT_NAME_GIVEN, givenName);
        writePhStringAttr(buf, NOM_ATTR_PT_NAME_FAMILY, familyName);
        writePhStringAttr(buf, NOM_ATTR_PT_ID, patId);

        buf.putShort((short) NOM_ATTR_PT_DOB);
        buf.putShort((short) 8);
        writePhBcdTime(buf, dobYear, dobMonth, dobDay, 0, 0, 0, 0);

        buf.putShort((short) NOM_ATTR_PT_SEX);
        buf.putShort((short) 2);
        buf.putShort((short) sex);

        buf.putShort((short) NOM_ATTR_PT_HEIGHT);
        buf.putShort((short) 4);
        buf.putInt(encodePhFloat((int)height, false));

        buf.putShort((short) NOM_ATTR_PT_WEIGHT);
        buf.putShort((short) 4);
        buf.putInt(encodePhFloat((int)weight, false));

        buf.putShort((short) NOM_ATTR_PT_BSA);
        buf.putShort((short) 4);
        buf.putInt(encodePhFloatDouble(bsa));

        int attrEnd = buf.position();
        buf.putShort(attrListLenPos, (short)(attrEnd - attrStart));
    }

    private void writePhEnumObs(ByteBuffer buf, int handle) {
        buf.putShort((short) handle);

        buf.putShort((short) 2);
        int attrListLenPos = buf.position();
        buf.putShort((short) 0);
        int attrStart = buf.position();

        buf.putShort((short) NOM_ATTR_ENUM_OBS_VAL);
        buf.putShort((short) 8);
        buf.putShort((short) NOM_PART_SCADA);
        buf.putShort((short) 0x5020);
        buf.putShort((short) 0x0000);
        buf.putShort((short) NOM_VENT_MODE_SIMV);

        buf.putShort((short) NOM_ATTR_ID_TYPE);
        buf.putShort((short) 4);
        buf.putShort((short) NOM_PART_SCADA);
        buf.putShort((short) 0x5020);

        int attrEnd = buf.position();
        buf.putShort(attrListLenPos, (short)(attrEnd - attrStart));
    }

    private short[] generatePhEcgSamples() {
        int numSamples = 500;
        short[] samples = new short[numSamples];
        int hr = slHeartRate.getValue();
        double beatInterval = 60.0 / Math.max(hr, 1);
        double samplePeriod = 1.0 / 500.0;
        String rhythm = (String) ecgRhythmCombo.getSelectedItem();

        for (int i = 0; i < numSamples; i++) {
            double t = phEcgPhase + i * samplePeriod;
            double val = 0;

            if (hr == 0 || "Asystole".equals(rhythm)) {
                val = phRng.nextGaussian() * 0.005;
            } else if ("Ventricular Fibrillation".equals(rhythm)) {
                val = 0.3 * Math.sin(t * 25 + phRng.nextDouble() * 3)
                    + 0.2 * Math.sin(t * 37) + phRng.nextGaussian() * 0.08;
            } else if ("Ventricular Tachycardia".equals(rhythm)) {
                double vtInterval = beatInterval * 0.5;
                double beatFrac = (t % vtInterval) / vtInterval;
                if (beatFrac < 0.4) val = 0.7 * Math.sin(Math.PI * beatFrac / 0.4);
                else if (beatFrac < 0.6) val = -0.3 * Math.sin(Math.PI * (beatFrac - 0.4) / 0.2);
                val += phRng.nextGaussian() * 0.01;
            } else if ("Atrial Fibrillation".equals(rhythm)) {
                double irregularInterval = beatInterval * (0.7 + 0.6 * Math.abs(Math.sin(t * 0.3)));
                double beatFrac = (t % irregularInterval) / irregularInterval;
                if (beatFrac >= 0.22 && beatFrac < 0.25) val = -0.1 * Math.sin(Math.PI * (beatFrac - 0.22) / 0.03);
                else if (beatFrac >= 0.25 && beatFrac < 0.30) val = 1.0 * Math.sin(Math.PI * (beatFrac - 0.25) / 0.05);
                else if (beatFrac >= 0.30 && beatFrac < 0.33) val = -0.2 * Math.sin(Math.PI * (beatFrac - 0.30) / 0.03);
                else if (beatFrac >= 0.40 && beatFrac < 0.55) val = 0.3 * Math.sin(Math.PI * (beatFrac - 0.40) / 0.15);
                val += 0.05 * Math.sin(t * 50) + phRng.nextGaussian() * 0.015;
            } else {
                double beatFrac = (t % beatInterval) / beatInterval;
                if (beatFrac >= 0.10 && beatFrac < 0.20) val = 0.15 * Math.sin(Math.PI * (beatFrac - 0.10) / 0.10);
                else if (beatFrac >= 0.22 && beatFrac < 0.25) val = -0.1 * Math.sin(Math.PI * (beatFrac - 0.22) / 0.03);
                else if (beatFrac >= 0.25 && beatFrac < 0.30) val = 1.0 * Math.sin(Math.PI * (beatFrac - 0.25) / 0.05);
                else if (beatFrac >= 0.30 && beatFrac < 0.33) val = -0.2 * Math.sin(Math.PI * (beatFrac - 0.30) / 0.03);
                else if (beatFrac >= 0.40 && beatFrac < 0.55) val = 0.3 * Math.sin(Math.PI * (beatFrac - 0.40) / 0.15);
                val += phRng.nextGaussian() * 0.01;
            }
            samples[i] = (short) Math.max(-32768, Math.min(32767, (int)(val * 2000)));
        }
        phEcgPhase += numSamples / 500.0;
        return samples;
    }

    private short[] generatePhPlethSamples() {
        int numSamples = 125;
        short[] samples = new short[numSamples];
        int hr = slHeartRate.getValue();
        double beatInterval = 60.0 / Math.max(hr, 1);
        double samplePeriod = 1.0 / 125.0;

        for (int i = 0; i < numSamples; i++) {
            double t = phPlethPhase + i * samplePeriod;
            double val;
            if (hr == 0) {
                val = phRng.nextGaussian() * 0.005;
            } else {
                double beatFrac = (t % beatInterval) / beatInterval;
                if (beatFrac < 0.15) val = beatFrac / 0.15;
                else if (beatFrac < 0.25) val = 1.0 - 0.3 * ((beatFrac - 0.15) / 0.10);
                else if (beatFrac < 0.35) val = 0.7 - 0.15 * Math.sin(Math.PI * (beatFrac - 0.25) / 0.10);
                else val = 0.55 * (1.0 - (beatFrac - 0.35) / 0.65);
                val += phRng.nextGaussian() * 0.005;
            }
            samples[i] = (short) Math.max(0, Math.min(4095, (int)(val * 3000)));
        }
        phPlethPhase += numSamples / 125.0;
        return samples;
    }

    private short[] generatePhAbpSamples() {
        int numSamples = 125;
        short[] samples = new short[numSamples];
        int hr = slHeartRate.getValue();
        double beatInterval = 60.0 / Math.max(hr, 1);
        double samplePeriod = 1.0 / 125.0;
        double abpS = slAbpSys.getValue();
        double abpD = slAbpDia.getValue();
        double abpM = (abpS + 2 * abpD) / 3.0;

        for (int i = 0; i < numSamples; i++) {
            double t = phAbpWavePhase + i * samplePeriod;
            double val;
            if (hr == 0 || abpS == 0) {
                val = phRng.nextGaussian() * 0.5;
            } else {
                double beatFrac = (t % beatInterval) / beatInterval;
                if (beatFrac < 0.10) val = abpD + (abpS - abpD) * (beatFrac / 0.10);
                else if (beatFrac < 0.20) { double d = (beatFrac - 0.10) / 0.10; val = abpS - (abpS - abpM) * 0.4 * d; }
                else if (beatFrac < 0.30) {
                    double d = (beatFrac - 0.20) / 0.10;
                    double base = abpS - (abpS - abpM) * 0.4;
                    val = base - 5 + 10 * Math.sin(Math.PI * d);
                } else {
                    double d = (beatFrac - 0.30) / 0.70;
                    double notchVal = abpS - (abpS - abpM) * 0.4 + 5;
                    val = notchVal - (notchVal - abpD) * d;
                }
                val += phRng.nextGaussian() * 0.5;
            }
            samples[i] = (short) Math.max(-32768, Math.min(32767, (int)(val * 100)));
        }
        phAbpWavePhase += numSamples / 125.0;
        return samples;
    }

    private short[] generatePhRespSamples() {
        int numSamples = 62;
        short[] samples = new short[numSamples];
        int rr = slPhRespRate.getValue();
        double breathInterval = 60.0 / Math.max(rr, 1);
        double samplePeriod = 1.0 / 62.0;

        for (int i = 0; i < numSamples; i++) {
            double t = phRespWavePhase + i * samplePeriod;
            double val;
            if (rr == 0) {
                val = phRng.nextGaussian() * 0.005;
            } else {
                double phase = (t % breathInterval) / breathInterval;
                if (phase < 0.45) val = Math.sin(Math.PI * phase / 0.45 * 0.5);
                else val = Math.cos(Math.PI * (phase - 0.45) / 0.55 * 0.5);
                val += phRng.nextGaussian() * 0.02;
            }
            samples[i] = (short) Math.max(-32768, Math.min(32767, (int)(val * 2000)));
        }
        phRespWavePhase += numSamples / 62.0;
        return samples;
    }

    // --- Philips Helpers ---

    private void writePhPollHeader(ByteBuffer buf) {
        // Per SinglePollDataResultImpl.parse():
        // pollNumber(u16) + relativeTime(u32) + absoluteTime(8 BCD) +
        // polledObjectType(Type: partition(u16)+code(u16)) + polledAttrGroup(u16)
        buf.putShort((short) phPollNumber);
        buf.putInt((int)((System.currentTimeMillis() & 0xFFFFFFFFL) * 8)); // relativeTime
        writePhAbsoluteTime(buf); // 8 bytes BCD
        buf.putShort((short) NOM_PART_OBJ);         // polledObjectType.partition
        buf.putShort((short) NOM_MOC_VMO_METRIC_NU); // polledObjectType.code
        buf.putShort((short) 0x0000);                // polledAttrGroup = all
    }

    private void writePhExtendedPollHeader(ByteBuffer buf) {
        // Per ExtendedPollDataResultImpl.parse():
        // pollNumber(u16) + sequenceNumber(u16) + relativeTime(u32) + absoluteTime(8 BCD) +
        // polledObjectType(Type: partition(u16)+code(u16)) + polledAttrGroup(u16)
        int seq = ++phSequenceNumber;
        buf.putShort((short) phPollNumber);
        buf.putShort((short) seq);
        buf.putInt((int)(((System.currentTimeMillis() + seq) & 0xFFFFFFFFL) * 8));
        writePhAbsoluteTime(buf);
        buf.putShort((short) NOM_PART_OBJ);
        buf.putShort((short) NOM_MOC_VMO_METRIC_NU);
        buf.putShort((short) 0x0000);
    }

    private void writePhAbsoluteTime(ByteBuffer buf) {
        Calendar cal = Calendar.getInstance();
        int year = cal.get(Calendar.YEAR);
        buf.put(toBcd(year / 100));
        buf.put(toBcd(year % 100));
        buf.put(toBcd(cal.get(Calendar.MONTH) + 1));
        buf.put(toBcd(cal.get(Calendar.DAY_OF_MONTH)));
        buf.put(toBcd(cal.get(Calendar.HOUR_OF_DAY)));
        buf.put(toBcd(cal.get(Calendar.MINUTE)));
        buf.put(toBcd(cal.get(Calendar.SECOND)));
        buf.put(toBcd(cal.get(Calendar.MILLISECOND) / 10));
    }

    private void writePhBcdTime(ByteBuffer buf, int year, int month, int day,
                                 int hour, int minute, int second, int hundredths) {
        buf.put(toBcd(year / 100));
        buf.put(toBcd(year % 100));
        buf.put(toBcd(month));
        buf.put(toBcd(day));
        buf.put(toBcd(hour));
        buf.put(toBcd(minute));
        buf.put(toBcd(second));
        buf.put(toBcd(hundredths));
    }

    private byte toBcd(int value) {
        value = Math.max(0, Math.min(99, value));
        return (byte)(((value / 10) << 4) | (value % 10));
    }

    private void writePhSystemModel(ByteBuffer buf) {
        byte[] mfr = "Philips".getBytes();
        byte[] mdl = "IntelliVue MX800".getBytes();
        int mfrPadded = (mfr.length + 1) & ~1;
        int mdlPadded = (mdl.length + 1) & ~1;
        int totalLen = 2 + mfrPadded + 2 + mdlPadded;

        buf.putShort((short) NOM_ATTR_ID_MODEL);
        buf.putShort((short) totalLen);
        buf.putShort((short) mfr.length);
        buf.put(mfr);
        if (mfr.length % 2 != 0) buf.put((byte) 0);
        buf.putShort((short) mdl.length);
        buf.put(mdl);
        if (mdl.length % 2 != 0) buf.put((byte) 0);
    }

    private void writePhStringAttr(ByteBuffer buf, int attrId, String value) {
        byte[] strBytes = value.getBytes();
        int padded = (strBytes.length + 1) & ~1;
        buf.putShort((short) attrId);
        buf.putShort((short)(2 + padded));
        buf.putShort((short) strBytes.length);
        buf.put(strBytes);
        if (strBytes.length % 2 != 0) buf.put((byte) 0);
    }

    private int encodePhFloat(int value, boolean isTempX10) {
        if (isTempX10) {
            return (0xFF << 24) | (value & 0x00FFFFFF);
        }
        return (value & 0x00FFFFFF);
    }

    private int encodePhFloatDouble(double value) {
        if (value == 0.0) return 0;
        int bestExp = 0;
        int bestMantissa = (int) Math.round(value);
        for (int exp = -4; exp <= 4; exp++) {
            double scale = Math.pow(10, -exp);
            long mantissa = Math.round(value * scale);
            if (mantissa >= -8388607 && mantissa <= 8388607) {
                bestExp = exp;
                bestMantissa = (int) mantissa;
                if (exp <= 0) break;
            }
        }
        return ((bestExp & 0xFF) << 24) | (bestMantissa & 0x00FFFFFF);
    }

    private void fixPhLengths3(ByteBuffer buf, int roLenPos, int cmdLenPos, int actionLenPos) {
        int end = buf.position();
        buf.putShort(actionLenPos, (short)(end - actionLenPos - 2));
        buf.putShort(cmdLenPos, (short)(end - cmdLenPos - 2));
        buf.putShort(roLenPos, (short)(end - roLenPos - 2));
    }

    private void sendUdp(DatagramSocket socket, InetSocketAddress addr, ByteBuffer buf) throws IOException {
        byte[] data = new byte[buf.remaining()];
        buf.get(data);
        socket.send(new DatagramPacket(data, data.length, addr.getAddress(), addr.getPort()));
    }

    // =====================================================================
    // Shared Helpers
    // =====================================================================

    private static int asciiHexHi(int v) {
        int n = (v >> 4) & 0x0F;
        return n < 10 ? '0' + n : 'A' + n - 10;
    }

    private static int asciiHexLo(int v) {
        int n = v & 0x0F;
        return n < 10 ? '0' + n : 'A' + n - 10;
    }

    private static int asciiHexDecode(int hi, int lo) {
        return (hexVal(hi) << 4) | hexVal(lo);
    }

    private static int hexVal(int c) {
        if (c >= '0' && c <= '9') return c - '0';
        if (c >= 'A' && c <= 'F') return 10 + c - 'A';
        if (c >= 'a' && c <= 'f') return 10 + c - 'a';
        return 0;
    }

    private void writeAsciiHexChecksum(OutputStream out, int sum) throws IOException {
        int lsb = sum & 0xFF;
        out.write(asciiHexHi(lsb));
        out.write(asciiHexLo(lsb));
    }

    private static String hexByte(int v) {
        return String.format("%02X", v & 0xFF);
    }

    private static String padTo(String s, int len) {
        StringBuilder sb = new StringBuilder(s);
        while (sb.length() < len) sb.append(' ');
        return sb.toString();
    }

    private static String draegerCmdName(int cmd) {
        switch (cmd) {
            case CMD_ICC: return "ICC";
            case CMD_NOP: return "NOP";
            case CMD_STOP: return "STOP";
            case CMD_REQ_DEVICE_ID: return "ReqDeviceId";
            case CMD_REQ_DATETIME: return "ReqDateTime";
            case CMD_REQ_DATA_CP1: return "ReqDataCP1";
            case CMD_REQ_DATA_CP2: return "ReqDataCP2";
            case CMD_REQ_LOW_ALRM_CP1: return "ReqLowAlarmLimitsCP1";
            case CMD_REQ_HI_ALRM_CP1: return "ReqHighAlarmLimitsCP1";
            case CMD_REQ_ALARMS_CP3: return "ReqAlarmsCP3";
            case CMD_REQ_ALARMS_CP1: return "ReqAlarmsCP1";
            case CMD_REQ_ALARMS_CP2: return "ReqAlarmsCP2";
            case CMD_REQ_SETTINGS: return "ReqDeviceSettings";
            case CMD_REQ_TEXT_MSG: return "ReqTextMessages";
            case CMD_CONFIGURE_RESP: return "ConfigureDataResponse";
            case CMD_REQ_RT_CONFIG: return "ReqRealtimeConfig";
            case CMD_CONFIG_RT: return "ConfigureRealtime";
            case CMD_REQ_TREND_STATUS: return "ReqTrendDataStatus";
            case CMD_REQ_TREND_DATA: return "ReqTrendData";
            default: return "Cmd(0x" + hexByte(cmd) + ")";
        }
    }

    // =====================================================================
    // Main
    // =====================================================================

    public static void main(String[] args) {
        final boolean autoStart = args != null && Arrays.asList(args).contains("--autostart");
        try {
            UIManager.setLookAndFeel(UIManager.getCrossPlatformLookAndFeelClassName());
        } catch (Exception ignored) {}

        SwingUtilities.invokeLater(() -> {
            CriticalInsightsMonitor monitor = new CriticalInsightsMonitor();
            monitor.setVisible(true);
            if (autoStart) {
                monitor.startDraeger();
                monitor.startPhilips();
            }
        });
    }
}

 ...
 *   java -cp build/install/simulator/lib/* IntellivueSimulatorV2 [--port 24105] [--patient "John Doe" --patient-id "P12345"]
 */
public class IntellivueSimulatorV2 {
    ...
    private boolean waveEcgEnabled = true;
    private boolean wavePlethEnabled = true;
    private boolean waveAbpEnabled = true;
    private boolean waveRespEnabled = true;
        ...
        if (roType == ROIV_APDU) {
            ...
            if (cmdType == CMD_CONFIRMED_ACTION) {
                ...
                if (actionTypeCode == (NOM_ACT_POLL_MDIB_DATA & 0xFFFF) ||
                    ...
                    } else if (requestedObjectClass == NOM_MOC_VMO_METRIC_SA_RT) {
                        ByteBuffer result = buildWaveformPollResult(clientInvokeId);
                        sendPacket(socket, clientAddr, result);
                        System.out.println("    -> Waveform poll result");
                    } else {
    ...
    private ByteBuffer buildWaveformSegment(int clientInvokeId) {
        ...
        buf.putShort((short) enabledWaveformCount());
        int obsListLenPos = buf.position();
        buf.putShort((short) 0);
        int obsStart = buf.position();

        writeEnabledWaveforms(buf);
    ...
    private ByteBuffer buildWaveformPollResult(int clientInvokeId) {
        ByteBuffer buf = ByteBuffer.allocate(4096);
        buf.order(ByteOrder.BIG_ENDIAN);
    ...
    private int enabledWaveformCount() {
        int count = 0;
        if (waveEcgEnabled) { count++; }
        if (wavePlethEnabled) { count++; }
        if (waveAbpEnabled) { count++; }
        if (waveRespEnabled) { count++; }
        return count;
    }

    private void writeEnabledWaveforms(ByteBuffer buf) {
        if (waveEcgEnabled) {
        ...
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                ...
                case "--help":
                    ...
                    System.out.println("  --waves <list>        Enabled waves: ecg,pleth,abp,resp or all/none");
    ...
    private void configureWaves(String value) {
        waveEcgEnabled = false;
        wavePlethEnabled = false;
        waveAbpEnabled = false;
        waveRespEnabled = false;
        if ("all".equalsIgnoreCase(value)) {
            waveEcgEnabled = true;
            wavePlethEnabled = true;
            waveAbpEnabled = true;
            waveRespEnabled = true;
            return;
        }
        ...
        for (String token : value.split(",")) {
            String wave = token.trim().toLowerCase(Locale.ROOT);
            if ("ecg".equals(wave)) { waveEcgEnabled = true; }
            else if ("pleth".equals(wave) || "spo2".equals(wave)) { wavePlethEnabled = true; }
            else if ("abp".equals(wave)) { waveAbpEnabled = true; }
            else if ("resp".equals(wave)) { waveRespEnabled = true; }
            else if (!wave.isEmpty()) { throw new IllegalArgumentException("Unknown wave: " + token); }
        }
    }
}
